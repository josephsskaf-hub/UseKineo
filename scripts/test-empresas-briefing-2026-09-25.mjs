...
ok(/trackEvent\('dfy_brief_viewed'/.test(form) && !/trackEvent\([^)]*sessionId/.test(form) && !/My footage/i.test(semComentarios(form)) && !/receipt/i.test(form) && /we will write to you there/.test(form), '8c. evento de cliente sem o session_id; nada de prometer My footage (402 para conta grátis); a saída é o e-mail do pagamento — o recibo da Stripe está DESLIGADO na conta (Cowork 25/09), então nada manda responder ao recibo')
