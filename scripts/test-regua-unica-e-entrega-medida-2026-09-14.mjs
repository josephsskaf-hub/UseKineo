...
checa('as 3 medições do portão verbatim usam narrationFitAt com a régua da família', (rc.match(/narrationFitAt\(parsedScript\.narration, duration, narrationRate\)/g) || []).length === 4 /* KINEO-DURACAO-SEGUE-O-ROTEIRO-2026-09-19: +1 remedição ao descer para a duração que a fala enche */ && !/narrationFit\(parsedScript\.narration, duration\)/.test(rc))
...
const DFS = roda(rd('lib/durationFollowsScript.ts')) // 22/09: o portão do Kineo 1 desce a duração sozinho (roteiro próprio)
...
checa('só o roteiro próprio (verbatim) passa pelo portão; a IA escreve do tamanho certo', /if \(ownScript\) \{[^\n]*\n\s+\/\/ KINEO-TETO-EXPLICITO-2026-09-14[\s\S]{0,1400}const falaDoAutor = falaPropria/.test(rf) && rf.includes("const ownScript = verbatim || body.script_mode === 'verbatim'")) // KINEO1-VERBATIM-ESTICA-2026-09-22: roteiro próprio = marcadores OU prosa com script_mode verbatim; a IA continua fora do portão
checa('régua clássica com a velocidade do roteiro, UMA para o portão e para o relatório', rf.includes("const fastRate = speechRateFor({ family: 'classic', speed: parsedScript.speed, language: narrationLanguage.language, voice: fastPersona?.voice, personaSpeed: fastPersona?.defaultSpeed })") && rf.includes('const narrationRate = fastRate') /* 15/09 KINEO-RITMO-POR-VOZ-KINEO1: a mesma régua, com a voz da persona */ && rf.includes('wordsPerSecond: narrationRate.wordsPerSecond, // Board 14/09'))
...
const base = (extra) => { const c = { verbatim: true, parsedScript: { segments: segs(5), speed: null }, narrationLanguage: { language: 'en' }, speechRateFor: SRX.speechRateFor, narrationFitAt: SRX.narrationFitAt, largestFittingDuration: (s) => [90, 60, 35].find((d) => s >= d * 0.95) ?? null, SUPPORTED_DURATIONS: [35, 60, 90], narrationTooShortMessage: () => 'Your script is short.', writeServerEvent: (e) => { eventos.push(e.name); return Promise.resolve(true) }, NextResponse: { json: (b, i) => ({ status: i?.status ?? 200, body: b }) }, user: { id: 'u', email: 'x@y.z' }, isDryRunAccount: () => false, duration: 35, decideDurationFollowsScript: DFS.decideDurationFollowsScript, DURATION_FOLLOWED_SCRIPT_EVENT: 'duration_followed_script' /* KINEO-DURACAO-SEGUE-O-ROTEIRO-KINEO1-2026-09-22 */, ...extra }; c.fastRate = SRX.speechRateFor({ family: 'classic', speed: c.parsedScript?.speed ?? null, language: c.narrationLanguage?.language }); c.ownScript = c.ownScript ?? c.verbatim; c.falaPropria = c.falaPropria ?? c.parsedScript.segments.map((s) => s.voiceover ?? '').join(' '); /* KINEO1-VERBATIM-ESTICA-2026-09-22 */ return c } // 15/09: fastRate (régua da voz) deriva da velocidade do roteiro do caso
{
  ...
  // KINEO-DURACAO-SEGUE-O-ROTEIRO-KINEO1-2026-09-22: roteiro PRÓPRIO + duração do seletor que cabe → desce sozinho (era recusa 422; a parede
  // que zerou no Seedance em 19/09 tinha migrado para o Kineo 1: 11 em 3 dias). Recusa honesta só quando NENHUMA duração cabe.
  checa('SEM consentimento, roteiro próprio que cabe em 35 s: desce para 35, grava duration_followed_script e libera', r5.status === undefined && r5.duration === 35 && r5.portao?.blocked === false && r5.portao?.autofit_applied === true && eventos.join(',') === 'duration_followed_script')
  ...
  checa('180 palavras a 60 s: passam a 1× (58 s); a 1,2× (48 s) descem para 35 s em vez de recusar (22/09)', r7.status === undefined && r7.portao?.blocked === false && r7.duration === 60 && r6.status === undefined && r6.duration === 35 && Math.abs(r6.portao?.speech_seconds - 48) <= 1)
...
console.log('== 6b) terceira passada do Kineo 1 (modo IA): coerência fala/legenda; resposta curta e erro não tocam a cena ==')
{
  ...
  const executa = async (expand, cenas) => { const js = ts.transpileModule('export async function run() {' + slice + '\n return scenes }', { compilerOptions: { module: 1, target: 9 } }).outputText; const exp = {}; vm.runInNewContext(js, { exports: exp, console: { log: () => {}, warn: () => {} }, scenes: cenas, duration: 90, targetWordCount: (d) => Math.round(d * 3.1), fastRate: { wordsPerSecond: 3.1 } /* 15/09: a 3ª passada mede na régua da voz; aqui 3,1 mantém o caso (279) */, narrationLanguage: { language: 'en' }, prompt: 'topic', expandVoiceoversToTargets: expand, shortCaptionFromVoiceover: (t) => 'CAP:' + t.split(' ').slice(0, 3).join(' ') }); return exp.run() }
...
console.log('== 9) terceira revisão do Board: dry-run não autorizado é rejeitado; relatório nunca PASS com portão bloqueando; narração extraída ==')
{
  ...
  const segs = (n, w = 10) => Array(n).fill(null).map((_, i) => ({ pexelsQuery: 'q' + i, voiceover: Array(w).fill('w').join(' ') }))
  const g = (extra) => { const c = { body: {}, user: { id: 'u', email: 'x@y.z' }, isDryRunAccount: () => false, verbatim: true, parsedScript: { segments: segs(12), speed: null }, narrationLanguage: { language: 'en' }, speechRateFor: SRX.speechRateFor, narrationFitAt: SRX.narrationFitAt, largestFittingDuration: (s) => [90, 60, 35].find((d) => s >= d * 0.95) ?? null, SUPPORTED_DURATIONS: [35, 60, 90], narrationTooShortMessage: () => 'short', writeServerEvent: (e) => { ev.push(e.name); return Promise.resolve(true) }, NextResponse: { json: (b, i) => ({ status: i?.status ?? 200, body: b }) }, duration: 35, decideDurationFollowsScript: DFS.decideDurationFollowsScript, DURATION_FOLLOWED_SCRIPT_EVENT: 'duration_followed_script', scenes: [{ voiceover: 'a', description: 'd' }], clipCount: 4, classicDryRunReport: () => ({ verdict: 'PASS — tudo certo', pass: true, problems: [] }), wordsPerSceneFor: () => [27, 35], ...extra }; c.fastRate = SRX.speechRateFor({ family: 'classic', speed: c.parsedScript?.speed ?? null, language: c.narrationLanguage?.language }); c.ownScript = c.ownScript ?? c.verbatim; c.falaPropria = c.falaPropria ?? c.parsedScript.segments.map((s) => s.voiceover ?? '').join(' '); /* KINEO1-VERBATIM-ESTICA-2026-09-22 */ return c } // 15/09: fastRate (régua da voz) deriva da velocidade do roteiro do caso
  ...
  // 22/09: no ensaio a mesma decisão do render real — desce para 35 s (autofit_applied) e o relatório diz PASS para 35 s, não FAIL para 60.
  checa('180 pal / 1,2× / 60 s no ensaio: o portão desce para 35 s (autofit_applied) e o relatório não diz FAIL', r4.status === 200 && r4.body.gate?.blocked === false && r4.body.gate?.autofit_applied === true && r4.body.gate?.target_seconds === 35 && r4.body.pass !== false)
