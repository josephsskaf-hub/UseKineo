...
// guarda a porta de $1 no modal de upgrade. Se a fonte religar, o preâmbulo deixa de disparar e as verificações voltam.
...
import { fileURLToPath as __f } from 'node:url'
{
  ...
  if (/export const CARD_TRIAL_LIVE = false/.test(__src)) {
    console.log('APOSENTADO (restauração 09/09/2026): a porta de $1 no modal de upgrade está desligado na fonte (lib/checkoutPricing.ts). Volta a valer sozinho no dia em que religar.')
    process.exit(0)
  }
}
