...
checa('só no primeiro filme de conta gratuita (o mesmo sinal do hook) e com o interruptor', ft.includes('if (aiHookHandle && FIRST_FILM_AI_CLIPS_ENABLED) {\n      aiStillsMax = Math.min(aiStillsMax, FIRST_FILM_STILLS_WITH_CLIPS_MAX)'))
...
checa('a espera de 15 s só sobrevive com o interruptor DESLIGADO', ft.includes('const deferAiClipsToCompose = !!aiHookHandle && FIRST_FILM_AI_CLIPS_ENABLED') && ft.includes('if (aiHookHandle && !deferAiClipsToCompose) {'))
