...
import {
  ...
  AEO_HOOK_WINDOW_DAYS,
  buildAnswerEngineHookSubscriptionReport,
  isExactAeoHookLanding,
...
export async function collectAnswerEngineHookSubscription({ db, generatedAt = new Date() }) {
  ...
  const windowStart = new Date(now.getTime() - AEO_HOOK_WINDOW_DAYS * DAY_MS).toISOString()
  ...
  const profileFields = 'id,email,created_at,signup_utm_source,signup_utm_medium,signup_utm_campaign,signup_referrer,has_paid,is_pro,plan,stripe_subscription_id,paypal_subscription_id,paddle_subscription_id'
  ...
  const [landings, nullLandings, profiles, nullProfiles, videos, financial, nullFinancial] = await Promise.all([
    paged('landings', (from, to) => db.from('events').select(eventFields)
      .eq('name', 'landing_session_started').eq('path', AEO_HOOK_LANDING_PATH)
  ...
  const sessionIds = [...new Set(landingEvents.filter(isExactAeoHookLanding)
  ...
  return buildAnswerEngineHookSubscriptionReport({
...
async function main() {
  ...
  process.stdout.write(JSON.stringify(await collectAnswerEngineHookSubscription({ db }), null, 2) + '\n')
}
