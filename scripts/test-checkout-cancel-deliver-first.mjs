...
ok(page.includes('ref={objectionRef}'), 'the existing objection container is measured without changing its layout')
ok(page.includes("CHECKOUT_CANCEL_OBJECTION_VIEW_EVENT"), 'actual objection exposure has a dedicated denominator')
ok(page.includes('shouldObserveCheckoutCancelObjection({'), 'the tested state policy blocks checking and first delivery')
ok(page.includes('shouldRecordCheckoutCancelObjectionView({'), 'the tested combined policy owns actual visibility')
