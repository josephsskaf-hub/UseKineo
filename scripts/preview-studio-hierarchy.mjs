...
const cache = new Map()
const react = { ...React, useEffect: () => {}, useMemo: (fn) => fn(), useRef: (value) => ({ current: value }), useState: (value) => [typeof value === 'function' ? value() : value, () => {}] }
function load(file) {
  ...
  const shim = (name) => {
    ...
    if (name === '@/lib/seriesDoorImpressions') return { useSeriesDoorSeen: () => () => () => {} }
...
const [idea, camera, steps] = elements(right)
if (!idea || !camera || !steps || !cost || cost.props.className !== 'cost') throw new Error('Baseline shape changed: review before rendering')
...
`))
// Steps remain available in a disclosure. Runtime and video continuation
// sections are untouched. No media/real account is in initial SSR.
