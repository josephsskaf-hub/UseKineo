...
console.log('== item 3b: modo história (medido no render 2141336f "Benny") ==')
{
  ...
  checa('telemetria: o prompt FINAL por cena vai para cinematic_dispatch_result (clássico e hollywood)', /submitted_prompts: ctx\.submittedPrompts\.map/.test(cin) && /c\.submittedPrompts\[sceneIndex\] = cinematic\.slice\(0, 240\)/.test(cin) && (cin.match(/ctxDespacho\(\)\.submittedPrompts\[hs\.index - 1\] = submittedPrompt\.slice\(0, 240\)/g) || []).length === 3) // KINEO-JUIZ-VE-A-CENA-1-2026-09-22: hs.index é 1-based; o juiz e scene_index são 0-based
}
