...
checa('a sobra entra frase a frase na ÚLTIMA cena narrada (14/09: nunca para trás — auditoria item 2), teto = segundos × 2,3', r.includes('const capW = Math.floor(capSecs * 2.3)') && r.includes("while (si < sentences.length && wordsIn(ultima.voiceover ?? '') + wordsIn(sentences[si]) <= capW) {"))
