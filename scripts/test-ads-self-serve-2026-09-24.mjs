...
ok(/while \(slots\.length > listLength && listLength < maxList\) \{/.test(post) && /let listLength = Math\.max\(2, Math\.ceil\(total \/ 4\.5\)\)/.test(post) && /beatStartTimes\((input\.beats|spokenBeats)\.map\(countWords\), words, narrationSeconds\)/.test(post), // 26/09: spokenBeats = o que a voz fala (telefone/site por extenso), medição ainda mais fiel
