...
console.log('== 1b. o brief do ChatGPT do fundador (H3, 13/09 22:34) ==')
const GPT = 'Create a 90-second YouTube video entirely in English.\n\nTITLE:\n“iPhone 18 Pro: What Would Tech Creators Test First?”\n\nVIDEO SETTINGS:\n- Language: English only.\n- Voiceover: Natural American English, energetic and conversational.\n- Captions and on-screen text: English only.\n- Format: Horizontal, 16:9.\n- Style: Cinematic tech video with close-ups, clean graphics, purposeful transitions, and subtle electronic background music.\n- Present this as an imagined hands-on scenario. Label concept visuals “CONCEPT.” Do not invent specifications, test results, or endorsements from real creators.\n\nUse the following voiceover:\n\n“Five minutes with the iPhone 18 Pro. What would you test first?\n\nImagine handing it to tech creators with one challenge: put it straight to work.\n\nFirst up—the camera. Forget the perfectly lit studio. Head outside. Shoot into the sun. Film while walking. Capture a moving subject. Then try it after dark.\n\nYour turn. Camera, battery, or performance—which test comes first? Tell us in the comments, and subscribe for more tech!”\n\nEDITING:\nOpen immediately with “YOU HAVE 5 MINUTES.”\nMatch each scene to the narration.\nUse chapter text: “CAMERA,” “CREATOR TEST,” and “GAMING.”\nEnd with “WHAT WOULD YOU TEST FIRST?”'
const nGpt = P.parseUserScript(GPT).narration
checa('a narração é só o que está entre aspas (começa em "Five minutes", termina em "more tech!")', /^Five minutes with the iPhone 18 Pro/.test(nGpt) && /subscribe for more tech!/.test(nGpt))
checa('título, "Use the following voiceover:", ficha de voz e bloco EDITING não são lidos', !/Use the following|What Would Tech Creators Test First|Open immediately|Match each scene|chapter text|Natural American|CONCEPT/.test(nGpt))
checa('"Voiceover: Natural American English, energetic…" é ficha de voz (produção), não fala', P.isVoiceSpecLine('- Voiceover: Natural American English, energetic and conversational.') && !P.isVoiceSpecLine('Voiceover: “Five minutes with the iPhone 18 Pro, what would you test first today?”'))
const GPT2 = GPT.replace('“Five minutes', 'Five minutes').replace('more tech!”', 'more tech!')
const nGpt2 = P.parseUserScript(GPT2).narration
checa('sem aspas: TITLE:/VIDEO SETTINGS:/EDITING: viram seções de produção e a fala sob "Use the following voiceover:" sobrevive', /^Five minutes with the iPhone 18 Pro/.test(nGpt2) && /subscribe for more tech!/.test(nGpt2) && !/Open immediately|Match each scene|What Would Tech Creators Test First|Natural American/.test(nGpt2))
checa('cabeçalho de FALA em caixa alta continua abrindo narração ("NARRATION:" não é produção)', /The lamp burned/.test(P.parseUserScript('TITLE:\nThe Keeper\n\nNARRATION:\nThe lamp burned every night for thirty years.').narration) && !/The Keeper/.test(P.parseUserScript('TITLE:\nThe Keeper\n\nNARRATION:\nThe lamp burned every night for thirty years.').narration))
...
console.log('== aviso honesto na entrada (sem tocar na tela) ==')
const mirror = (src) => { const a = src.indexOf('// ═══ MIRROR: looksLikeBriefLite'); const b = src.indexOf('// ═══ END MIRROR ═══'); return a > 0 && b > a ? src.slice(a, b) : null }
const mt = rd('lib/momentumTopic.ts')
const nt = rd('lib/growth/instructionPasteNotice.ts')
checa('o detector lite é byte-idêntico nos dois arquivos puros (mexeu num, mexe no outro)', mirror(mt) !== null && mirror(mt) === mirror(nt))
checa('os dois arquivos continuam sem import de scriptParser (os guardiões deles carregam crus)', !/from '\.\/scriptParser'|@\/lib\/scriptParser/.test(mt) && !/^import /m.test(nt))
const N = roda(nt)
checa('brief do Lumi → aviso "brief_two_voices" (duas vozes pedidas)', N.classifyInstructionPaste(LUMI) === 'brief_two_voices')
checa('brief sem vozes → aviso "brief"', N.classifyInstructionPaste('Zog: criatura azul com olhos enormes e mochila vermelha.\nCrie um vídeo infantil sobre o Zog aprendendo as cores.') === 'brief')
checa('ordem de uma linha continua "command_to_chatbot"', N.classifyInstructionPaste('Create a 35-second cinematic YouTube Short about the Moon.') === 'command_to_chatbot')
checa('resposta de chatbot com STYLE: continua "labeled_script"', N.classifyInstructionPaste('Absolutely. Below is a complete package.\nSTYLE: Bright, colourful\nVisual: a red car\nHOOK: The car that never stops') === 'labeled_script')
checa('as copies novas dizem a verdade: uma voz, Seedance para personagem fixo', /one voice/.test(N.instructionPasteNoticeFor('brief').body) && /Seedance 1\.5/.test(N.instructionPasteNoticeFor('brief').body) && /single voice/.test(N.instructionPasteNoticeFor('brief_two_voices').body))
