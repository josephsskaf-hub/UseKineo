...
const iAiBuild = src.indexOf('const generated = await generateScenes(prompt.slice(0, 1200), clipCount, hollywoodPath ? undefined : classicVisualPolicy)')
...
checa('verbatim vazio -> 2a tentativa: planner de IA sobre o texto cru com o mesmo contrato visual', /via = 'unbracket'[\s\S]*?\} else \{[\s\S]*?generateScenes\(prompt\.slice\(0, 1200\), clipCount, hollywoodPath \? undefined : classicVisualPolicy\)[\s\S]*?via = 'ai_planner'/.test(bloco))
