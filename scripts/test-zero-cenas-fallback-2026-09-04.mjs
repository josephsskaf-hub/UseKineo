...
const iAiBuild = src.indexOf('const generated = await generateScenes(prompt.slice(0, SCENE_WRITER_INPUT_MAX_CHARS), clipCount, hollywoodPath ? undefined : classicVisualPolicy, hollywoodPath ? undefined : classicWriterOptions)') // KINEO-2026-09-12: 4º arg = faixa de palavras + idioma
...
checa('verbatim vazio -> 2a tentativa: planner de IA sobre o texto cru com o mesmo contrato visual', /via = 'unbracket'[\s\S]*?\} else \{[\s\S]*?generateScenes\(prompt\.slice\(0, SCENE_WRITER_INPUT_MAX_CHARS\), clipCount, hollywoodPath \? undefined : classicVisualPolicy, hollywoodPath \? undefined : classicWriterOptions\)[\s\S]*?via = 'ai_planner'/.test(bloco)) // KINEO-2026-09-12: 4º arg = faixa de palavras + idioma
