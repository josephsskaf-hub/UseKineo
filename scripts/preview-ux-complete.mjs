...
export function renderPage(entry, before = false, fixture = {}) {
  const cache = new Map()
  ...
  walk(sf)
  let index=0
  const react={...React,useEffect:()=>{},useCallback:fn=>fn,useMemo:fn=>fn(),useRef:current=>({current}),useState:value=>{
...
if(process.argv.includes('--preview')){
 ...
 const pairs=PAGES.map(([name,file,fixture])=>`<section><h2>${name}</h2>${[1280,390].map(width=>`<h3>${width===390?'Mobile':'Desktop'}</h3><div class="pair">${[true,false].map(before=>`<article><h4>${before?'BEFORE · '+BASE:'AFTER · working code'}</h4><iframe title="${name} ${before?'before':'after'} ${width}" width="${width}" height="${name==='Home'?2400:1050}" sandbox="allow-same-origin" srcdoc="${escape('<!doctype html><html lang="en"><meta charset="utf-8"><style>body{margin:0;background:#0b0e13;font-family:Arial,sans-serif}a{color:inherit}</style><body>'+renderPage(file,before,fixture)+'</body></html>')}"></iframe></article>`).join('')}</div>`).join('')}</section>`).join('')
