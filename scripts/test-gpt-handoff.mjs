...
console.log('\n(J) public/gpt/openapi.json amarrado ao servidor')
{
  ...
  if (schema) {
    ...
    // GPT-V31-FATOS: 1.3.0 intentionally added GET /api/facts; retain only these two public operations.
    ok(opIds.length === 2 && sameSet(opIds, ['createKineoHandoff', 'getKineoFacts']) && schema.paths['/api/facts']?.get?.operationId === 'getKineoFacts', `(J9) exatamente 2 operações autorizadas (${opIds.join(', ')})`)
