...
// KINEO-ORIGEM-HONESTA-2026-09-23 — reancorado: o link de cadastro NÃO pode cravar utm_source/utm_medium.
// `utm_source=google` para todo mundo fazia quem vinha do ChatGPT virar 'google' em profiles.signup_utm_source;
// sem utm, captureSourceOnce (lib/analytics.ts) grava o referrer externo real do pouso. Só utm_campaign + intent_campaign ficam.
checa('CTA abre o Studio com o prompt do nicho, engine, intent_campaign=intent_<slug>; cadastro SEM utm_source/utm_medium cravados (origem real vem do referrer)', pg.includes("const studio = new URLSearchParams({ engine, prompt, duration: '60', script_mode: 'ai', intent_campaign: campaign })") && !/utm_source\s*:/.test(pg) && !/utm_medium\s*:/.test(pg) && pg.includes("utm_campaign: campaign, intent_campaign: campaign, redirect:") && pg.includes('const campaign = `intent_${slug}`'))
