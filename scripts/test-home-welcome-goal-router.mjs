...
const preview = source('docs/previews/HOME-WELCOME-OUTCOME-SELECTOR-V3-2026-09-01.html')
...
ok(component.includes("trackEvent('viral_onboarding_goal_selected'"), 'every explicit outcome selection is measured')
ok(component.includes('HOME_FIRST_WIN_VIEW_MARKER'), 'view event is latched once per tab')
ok(component.includes('useState<OnboardingGoalId | null>(null)'), 'the view starts neutral instead of silently selecting creator')
ok(!component.includes('selected_goal: DEFAULT_ONBOARDING_GOAL.id'), 'a view is never mislabeled as a creator choice')
ok(component.includes('ONBOARDING_GOALS.map'), 'all three bounded outcomes are selectable')
ok(component.includes('aria-pressed={active}'), 'the outcome selector exposes its current state')
ok(component.includes('selectedGoal ? ('), 'the exact brief appears only after a human choice')
ok(component.includes('buildHomeWelcomeGoalHref(selectedGoal)'), 'one contextual CTA owns the handoff after selection')
...
ok(css.includes('.goalGrid') && css.includes('.brief') && css.includes('.primaryAction'), 'selector, preview and handoff have explicit hierarchy')
ok(css.includes('@media (max-width: 700px)') && css.includes('.goalGrid'), 'mobile stacks all three outcomes')
...
ok(preview.includes('Before · creator-first card'), 'preview contains the measured zero-click hierarchy')
ok(preview.includes('After · choose outcome, then preview'), 'preview contains the new two-step hierarchy')
