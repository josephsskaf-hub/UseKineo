...
const comparisonPage = read('app/vs/[pair]/page.tsx')
...
console.log('3b · comparativo neutral com visitante de roteiro')
const neutralStart = comparisons.indexOf("slug: 'descript-vs-pictory'")
const neutralEnd = comparisons.indexOf("slug: 'quso-vs-submagic'", neutralStart)
const neutralRecord = comparisons.slice(neutralStart, neutralEnd)
check('o comparativo neutral alvo existe', neutralStart >= 0 && neutralEnd > neutralStart)
check('a nota admite tópico ou roteiro curto', neutralRecord.includes('typed topic or a short production script of up to 1,000 characters'))
check('a nota preserva o limite de URL/documento', neutralRecord.includes('does not import a URL or document'))
