...
checa('SET do saldo, nunca soma (no rollover), no mesmo UPDATE da renovação', /update\(\{ video_credits: credits, is_pro: true, plan: tier, cinematic_tokens: annualRefillCinematicTokens\(tier\) \}\)/.test(cron) && !/video_credits \+|video_credits: .*\+ credits/.test(cron))
checa('razão em events ANTES do grant, com id determinístico da chave', cron.indexOf("from('events').insert(") < cron.indexOf('.update({ video_credits: credits') && /eventIdFor\(annualRefillEventKey\(sub\.id, periodStart, month\)\)/.test(cron))
