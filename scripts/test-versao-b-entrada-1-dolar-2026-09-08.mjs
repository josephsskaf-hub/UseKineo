...
checa('Creator mensal custa 1500', /basic:\s*\{[^}]*usd:\s*1500/.test(cp) || /basic\s*=\s*1500/.test(cp))
checa('copy da porta fala $1, 7 days, 80 credits e $15', ['$1', '7 days', '80 credits', '$15'].every((t) => P.CARD_ENTRY_COPY.sentence.includes(t) || P.CARD_ENTRY_COPY.headline.includes(t)))
...
checa('layout lê trial_status e has_paid', /select\('is_pro, email, trial_status, has_paid'\)/.test(lay))
checa('layout monta o CardEntryBanner', /<CardEntryBanner/.test(lay))
const ban = rd('components/CardEntryBanner.tsx')
checa('banner só aparece para card_required sem pagamento', /status === CARD_ENTRY_TRIAL_STATUS && !hasPaid/.test(ban))
