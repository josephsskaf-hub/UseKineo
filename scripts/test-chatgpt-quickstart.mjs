...
ok(banner.includes("import styles from './ChatGptWelcomeBanner.module.css'"), 'link styles use a CSS Module and survive the Next Link boundary')
ok(banner.includes("trackEvent('chatgpt_quickstart_selected'"), 'selection event is emitted')
ok(banner.includes('onClick={() => onSelect(option.choice)}'), 'the card can submit only an allow-listed choice')
