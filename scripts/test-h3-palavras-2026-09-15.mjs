...
console.log('== (g) KINEO-FALA-NO-TETO: cena cinematic de 8 s com uma frase de 23 palavras (ensaio do S25, 15/09) ==')
{
  ...
  // KINEO-ULTIMA-ENCHIDA (ensaio do Omni com gpt-4o, deploy cfbe20b1): a cauda recusada uma vez pelo modelo fica com 4 palavras em 4 s
  // (2,3 s muda) — a passada final, depois do teto-rede e do esticão, pede de novo e enche; a régua final é julgada DEPOIS dela.
  let recusou = false
  const appendUmaRecusa = async (items) => { if (items.length === 1 && wordsOf(items[0].text) === 4 && !recusou) { recusou = true; return items.map((it) => it.text) } return appendFiel(items) }
  const logsU = []
  const planU = { characterSheet: '', environmentSheet: 'ice', styleSheet: 'cinematic', scenes: [] }
  for (let i = 0; i < 6; i++) planU.scenes.push({ index: i + 1, type: 'support', seconds: 10, prompt: `u${i + 1}`, voiceover: frase(24, `u${i + 1}_`) + '.', caption: '' })
  planU.scenes.splice(4, 0, { index: 5, type: 'cinematic', seconds: 8, prompt: 'wide', voiceover: frase(21, 'ult_') + '.', caption: '' })
  planU.scenes.forEach((sc, i) => { sc.index = i + 1 })
  await executar(ctxBase(planU, { expandVoiceoversToTargets: async (items) => items.map((it) => it.text), appendNarrationToTargets: appendUmaRecusa, SCENE_CAP: 12, console: { log: (...a) => logsU.push(a.join(' ')), warn: (...a) => logsU.push(a.join(' ')), error: (...a) => logsU.push(a.join(' ')) } }))
  const cU = planU.scenes.find((sc) => /ult_1\b/.test(sc.voiceover))
  const tU = planU.scenes[planU.scenes.indexOf(cU) + 1]
  checa(`KINEO-ULTIMA-ENCHIDA: a cauda recusada na hora do teto-rede (4 palavras) é enchida pela passada final (${wordsOf(tU?.voiceover)} palavras em ${tU?.seconds}s), começando pelas 4 originais`, recusou && !!tU && /^ult_18 ult_19 ult_20 ult_21\.? fato1\b/.test(tU.voiceover) && wordsOf(tU.voiceover) >= 7 && logsU.some((l) => /KINEO-ULTIMA-ENCHIDA: 1 cena\(s\)[^\n]*1 cresceram/.test(l)))
  checa('rota: a passada final roda depois do teto-rede e do esticão ao piso e antes das duas réguas (ensaio e render), só em modo IA, e só aceita continuação que começa pela fala original', fatia.indexOf('KINEO-ULTIMA-ENCHIDA-2026-09-15') > fatia.indexOf('KINEO-FALA-NO-TETO-2026-09-15') && fatia.indexOf('KINEO-ULTIMA-ENCHIDA-2026-09-15') > fatia.indexOf('plan.scenes = fitCinematicPlanFloor(plan.scenes, duration, SCENE_CAP)') && fatia.indexOf('KINEO-ULTIMA-ENCHIDA-2026-09-15') < fatia.indexOf("const dryRunEmails = new Set(") && fatia.indexOf('KINEO-ULTIMA-ENCHIDA-2026-09-15') < fatia.indexOf('KINEO-DATA-INVENTADA-2026-09-15 (varredura final)') && fatia.indexOf('KINEO-ULTIMA-ENCHIDA-2026-09-15') > fatia.indexOf('KINEO-CAUDA-CHEIA-2026-09-15') && fatia.indexOf('KINEO-ULTIMA-ENCHIDA-2026-09-15') < fatia.indexOf('KINEO-SILENCIO-NA-CENA-2026-09-11 — aprovado pelo fundador') && fatia.includes('.filter((x) => x.mudo > 1.0 && x.cabe - x.w >= 2)') && fatia.includes('for (let rodada = 1; rodada <= 2; rodada++) {') && fatia.includes('if (depois.ok) break') && fatia.includes("addWords: Math.max(3, x.cabe - x.w), maxWords: x.cabe + 2") && fatia.includes("wn <= x.cabe + 2 && nova.startsWith((x.sc.voiceover ?? '').trim().replace(") && /if \(!verbatim && plan\.scenes\.length > 0\) \{\s*\n\s*try \{\s*\n\s*const wordsN/.test(fatia))
  checa('KINEO-APARA-NO-RITMO-DA-VOZ: só em verbatim, depois de ritmoVoz e antes do enche-silêncio; tira 1 s da cena mais folgada (> 4 s) enquanto total > 7,5 s ou alguma cena > 1,4 s', fatia.indexOf('KINEO-APARA-NO-RITMO-DA-VOZ-2026-09-16') > fatia.indexOf('const ritmoVoz = (() => {') && fatia.indexOf('KINEO-APARA-NO-RITMO-DA-VOZ-2026-09-16') < fatia.indexOf('KINEO-ENCHE-SILENCIO-2026-09-13 — modo IA') && fatia.includes('if (verbatim && plan.scenes.length > 0) {\n        const mudoDe') && fatia.includes('(totalDe() > 7.5 || plan.scenes.some((sc) => mudoDe(sc) > 1.4))') && fatia.includes("(sc.seconds || 0) > 4 && mudoDe(sc) > 1.0"))
  // KINEO-CORTE-NA-FRASE (16/09): duas frases (12 + 9) numa cena cinematic de 8 s → a cabeça fica com a primeira frase inteira e a cauda começa na segunda
  const planF = { characterSheet: '', environmentSheet: 'ice', styleSheet: 'cinematic', scenes: [] }
  for (let i = 0; i < 6; i++) planF.scenes.push({ index: i + 1, type: 'support', seconds: 10, prompt: `f${i + 1}`, voiceover: frase(24, `f${i + 1}_`) + '.', caption: '' })
  planF.scenes.splice(4, 0, { index: 5, type: 'cinematic', seconds: 8, prompt: 'wide', voiceover: frase(12, 'primeira_') + '. ' + frase(9, 'segunda_') + '.', caption: '' })
  planF.scenes.forEach((sc, i) => { sc.index = i + 1 })
  await executar(ctxBase(planF, { expandVoiceoversToTargets: async (items) => items.map((it) => it.text), appendNarrationToTargets: appendNulo, SCENE_CAP: 12 }))
  const cF = planF.scenes.find((sc) => /primeira_1\b/.test(sc.voiceover))
  const tF = planF.scenes[planF.scenes.indexOf(cF) + 1]
  checa(`KINEO-CORTE-NA-FRASE: cabeça = primeira frase inteira (${wordsOf(cF.voiceover)} palavras) e a cauda começa em "segunda_1" (${tF ? wordsOf(tF.voiceover) : '-'} palavras)`, /primeira_12\.$/.test(cF.voiceover.trim()) && !!tF && /^segunda_1\b/.test(tF.voiceover.trim()))
  checa('KINEO-SOBRA-VIRA-CAUDA: nenhuma palavra da cena de 21 sumiu (12 na cabeça + 9 na cauda)', wordsOf(cF.voiceover) + (tF ? wordsOf(tF.voiceover) : 0) >= 21 && !fatia.includes('frases.pop()'))
  checa('rota: o corte por frase vem antes do corte por palavra e só vale com cabeça ≥ 4 palavras', fatia.includes('KINEO-CORTE-NA-FRASE-2026-09-16') && fatia.includes('const corte = corteFrase >= 4 ? corteFrase : Math.max(1, Math.min(fits, speech.length - 4))'))
