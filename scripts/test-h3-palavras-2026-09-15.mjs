...
console.log('== (g) KINEO-FALA-NO-TETO: cena cinematic de 8 s com uma frase de 23 palavras (ensaio do S25, 15/09) ==')
{
  ...
  checa('rota: a passada final roda depois do teto-rede e do esticão ao piso e antes das duas réguas (ensaio e render), só em modo IA, e só aceita continuação que começa pela fala original', fatia.indexOf('KINEO-ULTIMA-ENCHIDA-2026-09-15') > fatia.indexOf('KINEO-FALA-NO-TETO-2026-09-15') && fatia.indexOf('KINEO-ULTIMA-ENCHIDA-2026-09-15') > fatia.indexOf('plan.scenes = fitCinematicPlanFloor(plan.scenes, duration, SCENE_CAP)') && fatia.indexOf('KINEO-ULTIMA-ENCHIDA-2026-09-15') < fatia.indexOf("const dryRunEmails = new Set(") && fatia.indexOf('KINEO-ULTIMA-ENCHIDA-2026-09-15') < fatia.indexOf('KINEO-SILENCIO-NA-CENA-2026-09-11 — aprovado pelo fundador') && fatia.includes('.filter((x) => x.mudo > 1.0 && x.cabe - x.w >= 2)') && fatia.includes('for (let rodada = 1; rodada <= 2; rodada++) {') && fatia.includes('if (depois.ok || cresceram === 0) break') && fatia.includes("wn <= x.cabe + 2 && nova.startsWith((x.sc.voiceover ?? '').trim().replace(") && /if \(!verbatim && plan\.scenes\.length > 0\) \{\s*\n\s*try \{\s*\n\s*const wordsN/.test(fatia))
