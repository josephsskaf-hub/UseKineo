...
import {
  B2B_PROOF_CANDIDATE_REPORT_VERSION,
  B2B_PROOF_CONTEXT_DAYS,
  B2B_PROOF_EVENT_NAMES,
  B2B_PROOF_WINDOW_DAYS,
  buildB2bProofCandidateReport,
} from './b2b-proof-candidate-report.mjs'
...
async function main() {
  ...
  const windowStart = new Date(generatedAt.getTime() - B2B_PROOF_WINDOW_DAYS * 86_400_000)
  const queryStart = new Date(windowStart.getTime() - B2B_PROOF_CONTEXT_DAYS * 86_400_000)
  ...
  const [events, profiles, videos] = await Promise.all([
    ...
    paged('videos', (from, to) => db.from('videos')
      ...
      .lte('created_at', generatedAt.toISOString())
      .order('created_at', { ascending: true })
      .order('id', { ascending: true })
      .range(from, to)),
  ])

  const report = buildB2bProofCandidateReport({
  ...
  if (report.schemaVersion !== B2B_PROOF_CANDIDATE_REPORT_VERSION) throw new Error('Unexpected report schema')
