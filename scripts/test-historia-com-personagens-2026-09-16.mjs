...
checa('a história é detectada uma vez, antes do laço, sobre AS FALAS (não prompt + falas: contava em dobro); o teto de stills sobe até o número de cenas (máx. 8)', ft.includes("const falas = scenes.map((sc) => sc.voiceover ?? '').join(' ').trim()") && ft.includes('const personagem = characterStoryName(falas || prompt)') && !ft.includes('characterStoryName(`${prompt} ') && ft.includes('if (personagem || filmeDesenhado) aiStillsMax = Math.max(aiStillsMax, Math.min(scenes.length, CHARACTER_STORY_MAX_STILLS))') /* KINEO1-FILME-DESENHADO-2026-09-21: desenho também sobe o teto */ && ft.includes('let aiStillsMax = fastAiScenesMax()'))
checa('com personagem, TODA cena decide still (character_story) — e o VÍDEO de stock SEGUE entrando na cena (17/09: sem `continue`; 7 fotos e 0 clipe era slideshow)', ft.includes("? { ai: true, reason: 'character_story' as const, entity: null }") && ft.includes("clipSources.push('aiStill')\n              cenasComClipeIA.add(sceneNo) // KINEO1-MUNDO-DA-ENTIDADE — a cena já tem visual gerado\n") && ft.includes('// KINEO-HISTORIA-COM-PERSONAGENS (16/09) excluía o stock') && !/if \(personagem\)\s*continue/.test(ft) && /if \(filmeDesenhado\) continue/.test(ft)) // KINEO1-FILME-DESENHADO-2026-09-21: o único `continue` é o do DESENHO (o banco nunca é a cena certa); história com personagem segue com stock
checa('o evento fast_ai_still leva o personagem (medição: quantos filmes de história por dia)', ft.includes('max: aiStillsMax, character: personagem, first_film: primeiroFilmeDaConta, drawn_look: desenhoLook?.look ?? null, log:')) // KINEO1-FILME-DESENHADO-2026-09-21: + o look desenhado
