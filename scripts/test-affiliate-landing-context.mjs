...
const businessPage = source('app/business-video-content-plan/page.tsx')
const businessClient = source('app/business-video-content-plan/BusinessContentPlanClient.tsx')
...
ok(businessPage.includes("affiliateLandingContext(searchParams, 'business')"), 'business planner uses strict business context')
ok(businessPage.includes('affiliateContext={partnerContext}'), 'business page passes only server-approved context')
ok(businessClient.includes('context={affiliateContext}'), 'business planner renders the shared recommendation card')
ok(businessClient.includes('targetId="business-plan-tool"'), 'business recommendation targets the real planner')
ok(businessClient.includes('affiliateContext ? ('), 'ordinary business traffic gets no empty partner card spacing')
