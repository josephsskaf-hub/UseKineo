// Package the refined mixed Kineo proposal for direct file access. No network calls.
// Usage: node scripts/package-kineo-mixed-refined.cjs [output.html] [--check]
// Discovers index.html, prototype pages, previous-*.html and before-*.html.
...
function publicURL(raw) {
  let url = new URL(decode(raw), origin + '/design/mixed-refined-20260924/');
...
function inlineAsset(raw) {
  ...
  if (Object.hasOwn(assetData, raw)) return raw;
  ...
  if (!assetCache.has(filename)) {
    const token = `__KINEO_INLINE_ASSET_${assetCache.size}__`;
    assetCache.set(filename, token);
    assetData[token] = `data:${type};base64,${fs.readFileSync(filename).toString('base64')}`;
  }
  return assetCache.get(filename);
}

function materializeAssets(html) {
  return html.replace(/__KINEO_INLINE_ASSET_\d+__/g, token => {
    assert.ok(Object.hasOwn(assetData, token), 'Unresolved embedded asset: ' + token);
...
const pageSelect = index.match(/<select\b[^>]*id=["']page-select["'][^>]*>([\s\S]*?)<\/select>/i)?.[1];
assert.ok(pageSelect, 'Index must declare its page selector');
const labels = Object.fromEntries([...pageSelect.matchAll(/<option\b[^>]*value=["']([a-z0-9_-]+)["'][^>]*>([\s\S]*?)<\/option>/gi)].map(match => [match[1], decode(match[2].replace(/<[^>]+>/g, '').trim())]));
assert.ok(Object.keys(labels).length, 'Index must declare prototype options');
...
function packageDocument(source, name) {
  ...
  html = html.replace(/\b(data-src|data-video|src|poster)=("|')([\s\S]*?)\2/gi, (_, attr, quote, value) => `${attr}=${quote}${inlineAsset(value)}${quote}`);
  html = html.replace(/\bstyle=("|')([\s\S]*?)\1/gi, (_, quote, css) => `style=${quote}${inlineCSS(css).replaceAll(quote, quote === '"' ? '&quot;' : '&#39;')}${quote}`);
  if (name.startsWith('before-') || name.startsWith('previous-')) {
  ...
  html = html.replace(/<script\b([^>]*)>([\s\S]*?)<\/script>/gi, (_, attrs, code) => {
    ...
    code = code.replace(/const theme=new URLSearchParams\(location\.search\)\.get\('theme'\)\|\|'(?:light|adaptive)'/, "const theme=document.body.dataset.theme||'adaptive'");
...
// Store each asset once in this file, then expand it into the selected srcdoc.
// The review bar itself needs its two font data URLs immediately.
index = materializeAssets(index);

const controller = `
const pages=JSON.parse(document.getElementById('offline-pages').textContent),assets=JSON.parse(document.getElementById('offline-assets').textContent),labels=${json(labels)};
const originalTemplate=document.documentElement.outerHTML;
const root=document.documentElement,theme='adaptive';let page=Object.hasOwn(labels,root.dataset.offlinePage)?root.dataset.offlinePage:${json(Object.keys(labels)[0])};
const view=document.getElementById('view'),before=document.getElementById('before'),stage=document.getElementById('stage'),choice=document.getElementById('page-select'),comparison=document.getElementById('compare-select');
...
function render(key){return pages[key].replace(/__KINEO_INLINE_ASSET_\\d+__/g,token=>assets[token]).replace(/(<body\\b[^>]*\\bdata-theme=")[^"]*(")/i,'$1'+theme+'$2')}
function compare(){const original=comparison.value==='original',key=(original?'before-':'previous-')+page;comparison.querySelector('[value="original"]').disabled=!Object.hasOwn(pages,'before-'+page);comparison.querySelector('[value="previous"]').disabled=!Object.hasOwn(pages,'previous-'+page);if(!Object.hasOwn(pages,key))comparison.value='';stage.classList.toggle('compare',!!comparison.value);if(comparison.value){before.srcdoc=render(key);document.getElementById('before-label').textContent=original?'Site atual · 24/09':'Proposta anterior · Misto'}else before.removeAttribute('srcdoc')}
function show(hash=''){view.onload=()=>{if(hash)view.contentDocument.getElementById(hash.slice(1))?.scrollIntoView()};view.srcdoc=render(page);choice.value=page;document.getElementById('label').textContent=labels[page]+' · refinamento';compare()}
choice.onchange=()=>{if(Object.hasOwn(labels,choice.value)){page=choice.value;show()}};comparison.onchange=compare;document.querySelectorAll('[data-page]').forEach(b=>b.onclick=()=>{page=b.dataset.page;show()});document.getElementById('mobile').onclick=function(){this.setAttribute('aria-pressed',String(stage.classList.toggle('mobile')))};
...
const payload = `<script type="application/json" id="offline-assets">${json(assetData)}</script><script type="application/json" id="offline-pages">${json(pages)}</script><script>${controller}</script>`;
...
for (const [name, html] of Object.entries(pages)) {
  for (const match of html.matchAll(/\b(data-src|data-video|src|poster)=("|')([\s\S]*?)\2/gi)) {
    assert.ok(match[3].startsWith('data:') || Object.hasOwn(assetData, match[3]) || /^https:\/\/www\.usekineo\.com\/[^?#]*\.mp4(?:[?#]|$)/i.test(match[3]), name + ': unresolved media ' + match[3]);
  }
  if (Object.hasOwn(labels, name) && fs.readFileSync(path.join(sourceDir, name + '.html'), 'utf8').includes('IntersectionObserver')) {
    assert.ok(html.includes('IntersectionObserver') && html.includes('v.dataset.src'), name + ': ambient video behavior preserved');
  }
}
