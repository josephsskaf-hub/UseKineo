...
**FATO CONFIRMADO — atualização 07/09/2026:** `next.config.js` ainda usa `ignoreBuildErrors: true` e ignora lint no build. A tentativa de ativar a validação completa encontrou export inválido preexistente em `app/api/admin/send-checkout-recovery/route.ts` (ver handoff das cinco melhorias); não confundir typecheck bruto com tipos de rotas gerados no build. `.github/workflows/guardiao.yml` roda typecheck bruto e os testes offline `test-sharing-safety.mjs` e `test-five-improvements.mjs` em pushes main/codex e pull requests, sem tolerar falhas no job crítico. Existem testes legados: inventário não significa validação. Rode `npx tsc --noEmit --incremental false` e a bateria pertinente antes de entregar. Required checks/proteção de main no GitHub não foram confirmados: o workflow sozinho não prova que qualquer merge é bloqueado.
