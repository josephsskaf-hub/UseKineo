...
export default function GenerateClient({
  ...
  const [expandState, setExpandState] = useState<
    ...
    | { kind: 'transient_failure'; message: string; canRetry: true; retryOp: 'expand' | 'authoring' }
    | { kind: 'auth_required' }
  >(null)
  /**
  ...
  /**
   * KINEO-351 — o candidato da 1ª rodada, usado APENAS como ponto de partida
   * da 2ª. Não é `structuredScriptRef`: aquele é o que vai para a GERAÇÃO, e
   * texto de IA não aprovado nunca pode virar fonte de render (Contrato C1).
   */
  const expandCandidateRef = useRef<string | null>(null)
  ...
  /** KINEO-351 — roteiro escrito pela IA que ainda não cobre a duração pedida. */
  const [authoredStillShort, setAuthoredStillShort] = useState<{ words: number; minWords: number } | null>(null)
...
// Push #095 — player resilience. When the B2/Creatomate CDN returns a 503
  ...
  async function handleExpandScript() {
    ...
    try {
      const res = await fetch('/api/expand-script', {
        ...
        body: JSON.stringify({
          // 2ª rodada continua de onde a 1ª parou; a 1ª parte do texto da pessoa.
          script: expandCandidateRef.current ?? structuredScriptRef.current ?? prompt,
      ...
      // ── already_fits: a fala JÁ enche. Não é falha nem "ainda curto". ─────
      // KINEO-351 — o #350 deixava este caso escorrer para o ramo do
      // `still_short` lá embaixo (porque `expanded` vem false), e a tela dizia
      ...
      if (data?.outcome === 'already_fits') {
        ...
        setError(null)
        setPhase('idle')
        void trackEvent('script_already_fits', {
          speech_seconds: data?.before?.seconds ?? null,
      ...
      // O painel de aprovação SÓ pode aparecer quando existe expansão REAL e
      // SUFICIENTE. Antes, qualquer 200 virava "Read it before we render" —
      // inclusive a resposta "não mudei nada" — e a pessoa aprovava o mesmo
      // texto que o guard acabara de recusar (caso ofirshu555: duas voltas em
      // 32s, sempre 38s/12 palavras, zero vídeo).
      const cresceu = data?.expanded === true
      const aindaCurto = data?.stillShort === true
      void trackEvent('script_expanded', {
        before_seconds: data?.before?.seconds ?? null,
        after_seconds: data?.after?.seconds ?? null,
        before_coverage: data?.before?.coverage ?? null,
      ...
      if (!cresceu || aindaCurto) {
        ...
        // fluxo morria na primeira expansão. O teste passava e dava a impressão
        // de uma garantia que o produto não tinha. Ou liga, ou tira do teste.
        //
        // Ligado: a 2ª rodada parte do CANDIDATO da 1ª (senão repetiria o mesmo
        // pedido e voltaria o mesmo resultado), mas o teto de crescimento
        // continua sendo medido contra a BASE IMUTÁVEL da pessoa — é isso que
        // impede 2,5× × 2,5× = 6,25× pela porta dos fundos.
        const cobertura = Number(data?.after?.coverage) || 0
        if (
          candidato &&
          deservesSecondRound({ outcome: 'still_short', round: rodada, grew: cresceu, coverageAfter: cobertura })
        ) {
          expandCandidateRef.current = candidato
          void trackEvent('script_expand_second_round', {
            round: rodada + 1,
            after_coverage: cobertura,
            target_seconds: scriptTooShort.targetSeconds,
          })
          setExpanding(false)
  ...
  async function handleWriteFullScript(autoAlvo?: { speechSeconds: number; targetSeconds: number }) {
    ...
    try {
      ...
      const res = await fetchResilienteSemDinheiro('/api/generate-script', {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'same-origin',
        // KINEO-351 — o CTA mandava só { topic, language }, e a rota é calibrada
        // em 60s: escolher 90s voltava curto e escolher 35s voltava grande demais.
        // Agora a duração escolhida é contrato, e `forceAuthoring` proíbe o
        // early-return que devolvia um esqueleto de 3 segundos INTACTO.
        body: JSON.stringify({
          topic: ideia,
          language,
      ...
      if (res.status === 401) {
        setExpandState({ kind: 'auth_required' })
        void trackEvent('script_authoring_failed', { http: 401, reason: 'auth_required' })
        return
      }
      ...
      // KINEO-351 — o servidor diz se o roteiro escrito ATINGIU a duração.
      // Se não atingiu, ele NÃO é apresentado como pronto: mostrar é honesto,
      // deixar aceitar como se estivesse fechado seria mandá-la de volta ao
      // guard que acabou de recusar — o loop com outro nome.
      setAuthoredStillShort(data?.stillShort === true
        ? { words: Number(data?.words) || 0, minWords: Number(data?.minWords) || 0 }
        : null)
      void trackEvent('script_authoring_delivered', {
        outcome: data?.outcome ?? null,
        words: data?.words ?? null,
        min_words: data?.minWords ?? null,
        target_seconds: data?.targetSeconds ?? null,
      })
  ...
  function acceptAuthoredScript() {
    ...
    void trackEvent('script_authoring_accepted', {
      ...
      target_seconds: scriptTooShort?.targetSeconds ?? null,
      still_short_when_accepted: authoredStillShort !== null,
    })
    setAuthoredStillShort(null)
  ...
  useEffect(() => {
    ...
    // KINEO-P0A-SEM-LOOP-2026-08-26 — trava dupla contra o loop do
    // ofirshu555: (1) já esgotamos as saídas desta tentativa → não redispara;
  ...
  function acceptExpandedScript() {
    ...
    // KINEO-P0A-2026-08-26 — o texto aprovado é EXATAMENTE o que vai ao guard
    // e à geração: structuredScriptRef é a fonte que handleGenerate lê
    // (`structuredScriptRef.current ?? prompt`), e o prompt acompanha para a
    ...
    // KINEO-351 — a rodada é lida ANTES de zerar. No #350 o zeramento vinha
  ...
  async function handleAnalyze(
    ...
    // KINEO-FIRST-CLICK-AUTOFIT-2026-09-03 — o cliente não recusa mais um
    // roteiro curto antes de o servidor aplicar o degrau de duração. Desde o
    // #1 do diário, a rota cinematográfica reduz o alvo com a mesma régua e
    // registra `script_duration_autofit_down`; manter a antiga checagem aqui
      ...
      {showStep1 && (
        ...
        >
          ...
          <ModeSelector
            ...
            track={(name, meta) => { void trackEvent(name, meta as Record<string, unknown>) }}
          />
          )}
            ...
            >
              ...
              {authoredScript ? (
                ...
                <>
                  ...
                  {authoredStillShort && (
                    // KINEO-351 — honestidade em vez de "pronto". Se o texto
                    // ainda não cobre a duração, dizer isso AQUI evita ela
                    // aceitar e levar a mesma recusa dois cliques depois.
                    <div className="text-xs mb-3" style={{ color: '#e0a458', lineHeight: 1.5 }}>
                      Heads up: this is {authoredStillShort.words} spoken words and a {scriptTooShort.targetSeconds}s
                      video needs about {authoredStillShort.minWords}. Add a couple of lines, or pick a shorter length —
                      otherwise the same check will stop it again.
                    </div>
                  )}
                  <div className="flex flex-wrap gap-2">
                    <button
                ...
                <>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {expandState.kind === 'transient_failure' && (
                      ...
                      <button
                        onClick={() => {
                          const op = expandState.retryOp
                          setExpandState(null)
                          if (op === 'authoring') void handleWriteFullScript()
                          else void handleExpandScript()
                        }}
                        disabled={expanding || authoring}
                        className="rounded-xl px-5 py-2.5 text-sm font-bold text-white"
                        style={{ background: '#2997ff', border: 'none', opacity: expanding || authoring ? 0.6 : 1 }}
                      >
                        {expanding || authoring ? 'Trying…' : 'Try again'}
                      </button>
                    )}
                    {expandState.kind === 'auth_required' && (
                      // KINEO-351 — antes só havia a frase "entre de novo" e
                      // nenhum caminho. Agora há botão, e o destino traz a
                      // pessoa de volta a esta mesma tela.
                      <a
                        href={`/login?redirect=${encodeURIComponent(
                          typeof window !== 'undefined' ? window.location.pathname + window.location.search : '/studio',
                        )}`}
                        onClick={() => void trackEvent('script_expand_auth_relogin', {})}
                        className="rounded-xl px-5 py-2.5 text-sm font-bold text-white"
                        style={{ background: '#2997ff', border: 'none', textDecoration: 'none' }}
                      >
                        Sign in again
                      </a>
                    )}
                      ...
                      expandState.suggestedDuration !== null && (
                        ...
                        <button
                          onClick={() => {
                            ...
                            // KINEO-351 — NÃO PERDER O TEXTO. O #350 apagava o
                            // candidato aqui e na edição manual: a tela dizia
                            // "termine na mão" e sumia com o que havia para
                            // terminar. O que a IA acrescentou é dela agora.
                            if (expandedScript) {
                              structuredScriptRef.current = expandedScript
                              setPrompt(expandedScript)
                              expandBaseRef.current = expandedScript
                    ...
                    <button
                      onClick={() => {
                        // KINEO-351 — o candidato vai para a caixa de texto em
                        // vez de sumir: é literalmente o que ela vai editar.
                        if (expandedScript) {
                          structuredScriptRef.current = expandedScript
                          setPrompt(expandedScript)
                          expandBaseRef.current = expandedScript
                        ...
                        void trackEvent('script_short_manual_edit', {
                          after_expand_exhausted: true,
                          from_outcome: expandState.kind,
                          kept_candidate: expandedScript !== null,
                        })
                      }}
                      className="rounded-xl px-5 py-2.5 text-sm font-bold"
                      style={{ background: 'transparent', border: '1px solid var(--border)', color: 'var(--muted2)' }}
...
function ModeSelector({
  ...
  trialActive: boolean
  onUpgrade: () => void
  // sprint-v1v4 #13 — o desvio de UM CLIQUE precisa poder mexer na duração e
  // se registrar. Opcionais de propósito: se um dia o card for montado em
  // outro lugar sem eles, ele degrada para "só avisa", nunca quebra.
  ...
  // Medido em produção (14d, externos): 21 recusas de `trial_credits_stalled` /
  // `cinematic_gate_trial_stalled` em 14 pessoas, e em NENHUMA delas a pessoa
  // estava sem saldo — em todas existia uma combinação motor×duração que o
  // saldo pagava. A tela só nunca disse qual. Aqui ela passa a dizer, ANTES do
  // roteiro e ANTES do clique. Sem preço, sem plano, sem oferta: dois números.
  const CUSTO_POR_MOTOR: Record<string, Parameters<typeof creditCostForDuration>[0]> = {
  ...
  const custoDoMotor = (m: string, d: number): number =>
  ...
  const resgateDeMotor =
    mode === 'cinematic_ai'
      ? planoDeResgate({
          ...
          saldo: credits,
          duracoes: duracoesDoSeletor,
          custoDe: custoDoMotor,
          motoresDisponiveis,
        })
      : ({ tipo: 'cabe' } as const)
  ...
  return (
    <div className="mt-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <EngineCard
          ...
          badge={seedanceUnlocked ? (
            /* sprint-v1v4 #13 — este selo dizia `25 credits` SEMPRE, em toda
               duração. A 35s o servidor cobra 15 e a 90s cobra 38: a tela
               prometia um número que o biller não praticava em 2 dos 3 chips.
               Agora sai da MESMA função que cobra, e diz a verdade do saldo. */
            <span className="text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(41,151,255,.18)', color: cabeNoSaldo(custoDoMotor('seedance', duration), credits) ? '#2997ff' : '#ffb454', border: `1px solid rgba(${cabeNoSaldo(custoDoMotor('seedance', duration), credits) ? '41,151,255' : '255,180,84'},.3)` }}>
              {custoDoMotor('seedance', duration)} credits
              {!cabeNoSaldo(custoDoMotor('seedance', duration), credits) && ` · +${faltamCreditos(custoDoMotor('seedance', duration), credits)}`}
            </span>
          ) : (
                  ...
                  <span className="text-[11px] font-black px-2 py-0.5 rounded-full whitespace-nowrap" style={{ background: naoCabe ? 'rgba(255,180,84,.14)' : 'rgba(41,151,255,.18)', color: naoCabe ? '#ffb454' : '#7cc0ff', border: `1px solid rgba(${naoCabe ? '255,180,84' : '41,151,255'},.3)` }}>
                    {!cinematicUnlocked
                      ...
                      /* sprint-v1v4 #13 — `+N` é a frase mais curta possível de
                         ...
                         e ler o desvio abaixo é caminho legítimo. */
                      : naoCabe ? `${m.cr} cr · +${faltamCreditos(m.cr, credits)}` : `${m.cr} cr`}
                  </span>
                </button>
              )
            })}
      ...
      {/* ═══ sprint-v1v4 #13 — O DESVIO DE UM CLIQUE ═══════════════════════════
          A parede vira placa. Mesmo desenho do aviso de duração
          (KINEO-CUSTO-VISIVEL), agora para a escolha de MOTOR — porque a
          medição de 14 dias mostrou que em 21 recusas de saldo NENHUMA pessoa
          estava sem crédito: existia sempre uma combinação que cabia, e a tela
          nunca a nomeava. Aqui ela nomeia, com o botão que a executa.
          NENHUM PREÇO, NENHUM PLANO, NENHUMA OFERTA nesta caixa — isso é a
          pista do Codex e ele já tem o modal de upgrade para isso. */}
      {(resgateDeMotor.tipo === 'mesma_camera' || resgateDeMotor.tipo === 'outra_camera') && (
        <div
          className="rounded-xl px-3 py-2.5 mt-3"
          style={{ background: 'rgba(255,180,84,.07)', border: '1px solid rgba(255,180,84,.28)' }}
        >
          <p className="text-xs font-bold" style={{ color: '#ffc47a', lineHeight: 1.5 }}>
            {NOME_DO_MOTOR[aiEngine] ?? 'This engine'} at {duration}s costs{' '}
          ...
          <button
            type="button"
            onClick={() => {
              const alvo = resgateDeMotor.alvo
              if (alvo.motor !== aiEngine) { setMode('cinematic_ai'); setAiEngine(alvo.motor as typeof aiEngine) }
              if (alvo.duracao !== duration) setDuration?.(alvo.duracao as Duration)
              track?.('engine_downshift_clicked', {
                de_motor: aiEngine, de_duracao: duration, de_custo: custoDoMotor(aiEngine, duration),
                para_motor: alvo.motor, para_duracao: alvo.duracao, para_custo: alvo.custo,
                tipo: resgateDeMotor.tipo, credits,
              })
            }}
            className="rounded-lg px-3 py-1.5 mt-1.5 text-xs font-black"
            style={{ background: '#ffb454', border: 'none', color: '#1a1206', cursor: 'pointer' }}
          >
            {resgateDeMotor.tipo === 'mesma_camera'
              ? `Make it ${resgateDeMotor.alvo.duracao}s (${resgateDeMotor.alvo.custo} cr) →`
              : `Use ${NOME_DO_MOTOR[resgateDeMotor.alvo.motor] ?? 'it'} (${resgateDeMotor.alvo.custo} cr) →`}
          </button>
        </div>
      )}
    </div>
