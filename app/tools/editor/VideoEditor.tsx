...
const ERRORS: Record<string, [string, string]> = {
  ...
  invalid_speed: ['Choose a speed between 0.5× and 2×.', 'Elige una velocidad entre 0,5× y 2×.'],
  invalid_text: ['Use up to 100 characters and 3 lines of text.', 'Usa hasta 100 caracteres y 3 líneas de texto.'],
  invalid_framing: ['Choose a supported aspect ratio, framing and text position.', 'Elige una proporción, un encuadre y una posición de texto compatibles.'],
    ...
    <div className="ke-shell">
      ...
      <div className="ke-layout">
        <section className="ke-stage" aria-label={t('Video preview', 'Vista previa del vídeo')}>
          ...
          </> : <div className="ke-empty"><div className="ke-upload-symbol" aria-hidden="true">↑</div><h2>{t('Start with your video', 'Empieza con tu vídeo')}</h2><p>{t('Choose a local file. It stays on this device.', 'Elige un archivo local. Se queda en este dispositivo.')}</p><button className="ke-primary" type="button" disabled={Boolean(busy) || supported !== true} onClick={() => fileInput.current?.click()}>{t('Choose video file', 'Elegir archivo de vídeo')}</button><button type="button" disabled={Boolean(busy) || supported !== true} onClick={demo}>{t('Try a sample clip', 'Probar un vídeo de muestra')}</button></div>}
          <input ref={fileInput} className="ke-file" type="file" accept="video/mp4,video/webm,video/quicktime,video/*" aria-label={t('Video file', 'Archivo de vídeo')} disabled={Boolean(busy) || supported !== true} onChange={event => { const next = event.target.files?.[0]; event.target.value = ''; void choose(next) }} />
          {file ? <div className="ke-file-info"><span>{file.name}</span><button type="button" disabled={Boolean(busy)} onClick={() => fileInput.current?.click()}>{t('Change video', 'Cambiar vídeo')}</button></div> : null}
        </section>
        <section className="ke-controls" aria-label={t('Edit settings', 'Ajustes de edición')}>
          <fieldset disabled={!info || Boolean(busy)}><legend>{t('Video settings', 'Ajustes del vídeo')}</legend>
            ...
            <label>{t('Aspect ratio', 'Proporción del vídeo')}<select value={settings.aspect} onChange={event => update({ aspect: event.target.value as EditSettings['aspect'] })}><option value="original">{t('Original', 'Original')}</option><option value="9:16">9:16 · {t('Vertical', 'Vertical')}</option><option value="1:1">1:1 · {t('Square', 'Cuadrado')}</option><option value="16:9">16:9 · {t('Wide', 'Horizontal')}</option></select></label>
            <label>{t('Framing', 'Encuadre')}<select value={settings.fit} onChange={event => update({ fit: event.target.value as EditSettings['fit'] })}><option value="contain">{t('Fit — add borders', 'Encajar — añadir bordes')}</option><option value="cover">{t('Fill — crop to frame', 'Llenar — recortar al marco')}</option></select></label>
            ...
            <label className="ke-check"><input type="checkbox" checked={settings.mute} onChange={event => update({ mute: event.target.checked })} />{t('Remove audio from export', 'Quitar audio de la exportación')}</label>
            <label>{t('On-screen text (100 characters, 3 lines)', 'Texto en pantalla (100 caracteres, 3 líneas)')}<textarea maxLength={100} rows={2} value={settings.text} placeholder={t('Add a title or message', 'Añade un título o mensaje')} onChange={event => update({ text: event.target.value })} /></label>
