      ...
      <div className="ke-layout">
        ...
        <section className="ke-controls" aria-label={t('Edit settings', 'Ajustes de edición')}>
          <fieldset disabled={!info || Boolean(busy)}><legend>{t('Make it yours', 'Hazlo tuyo')}</legend>
            <div className="ke-two"><label>{t('Start (seconds)', 'Inicio (segundos)')}<input type="number" min="0" max={info?.duration ?? 0} step="0.01" value={settings.start} onChange={event => update({ start: event.target.valueAsNumber })} /></label><label>{t('End (seconds)', 'Final (segundos)')}<input type="number" min="0" max={info?.duration ?? 0} step="0.01" value={settings.end} onChange={event => update({ end: event.target.valueAsNumber })} /></label></div>
