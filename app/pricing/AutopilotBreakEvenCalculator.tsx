    ...
    >
      <div className="grid grid-cols-1 gap-5 md:grid-cols-[1fr_260px] md:items-end">
        <div>
          ...
          <h3 id="autopilot-break-even-heading" className="mt-1.5 text-[1.2rem] font-black tracking-tight text-[var(--text)]">
        ...
        <div>
          <label htmlFor="autopilot-gross-profit" className="block text-[11px] font-extrabold uppercase tracking-[.09em] text-[var(--text2)]">
            Gross profit per customer · USD
          </label>
          <div className="mt-2 grid grid-cols-[1fr_auto] gap-2">
            <div className="relative">
              <span aria-hidden="true" className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[14px] font-bold text-[var(--muted2)]">$</span>
              <input
                ...
                placeholder="150"
                className="min-h-11 w-full rounded-lg border border-[var(--border2)] bg-[var(--card)] py-2 pl-7 pr-3 text-[14px] font-bold text-[var(--text)] outline-none focus:border-[var(--accent)]"
              />
            </div>
            <button
              type="button"
              onClick={calculate}
              className="min-h-11 rounded-lg bg-[var(--indigo)] px-4 text-[13px] font-black text-[var(--on-accent)] transition hover:brightness-110"
            >
              Calculate
            </button>
          </div>
      ...
      {error ? <p role="alert" className="mt-3 text-[12px] font-bold text-[var(--pricing-error)]">{error}</p> : null}
      ...
      {result ? (
        <div aria-live="polite" className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-2">
          <article className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-4">
            <div className="text-[10px] font-black uppercase tracking-[.1em] text-[var(--accent)]">7-day pilot</div>
            <strong className="mt-1 block text-[1.35rem] font-black text-[var(--text)]">{customerLabel(result.pilotCustomers)}</strong>
            ...
            <button
              ...
              onClick={() => start('pilot')}
              className="mt-3 w-full rounded-lg border border-[var(--accent)] px-3 py-2.5 text-[12.5px] font-black text-[var(--accent)] transition hover:bg-[var(--indigo)] hover:text-[var(--on-accent)] disabled:opacity-60"
            >
          ...
          <article className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-4">
            <div className="text-[10px] font-black uppercase tracking-[.1em] text-[var(--accent)]">Monthly Autopilot</div>
            <strong className="mt-1 block text-[1.35rem] font-black text-[var(--text)]">{customerLabel(result.monthlyCustomers)}</strong>
            <p className="mt-1 text-[12px] leading-relaxed text-[var(--muted)]">
            ...
            <button
              ...
              className="mt-3 w-full rounded-lg bg-[var(--indigo)] px-3 py-2.5 text-[12.5px] font-black text-[var(--on-accent)] transition hover:brightness-110 disabled:opacity-60"
            >
              {pending !== null ? 'Opening secure checkout…' : 'Start monthly Autopilot →'}
            </button>
          </article>

          <p className="md:col-span-2 m-0 text-[11.5px] leading-relaxed text-[var(--muted2)]">
