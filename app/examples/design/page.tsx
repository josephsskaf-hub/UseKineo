...
export default async function ExamplesDesign({ searchParams }: { searchParams: { option?: string } }) {
  ...
  const selection = userSelection
    ? { name: 'Sua seleção · nove vídeos', note: 'Robôs no destaque horizontal, com Tunguska e Lituya Bay à direita. Outros seis vídeos abaixo, sem repetição ou valores de créditos.', ids: EXAMPLES_SELECTION_SEP24.map(v => v.id) }
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.content}>
      ...
      <ExamplesGallery key={selection.name} videos={videos.slice(0, userSelection ? 9 : 6)} separateFeatured={userSelection} startPaused={!userSelection} />
    </section>
  </main>
}
