...
function ExamplePreview({ video, onClose, actionLabel }: { video: WallVideo; onClose: () => void; actionLabel?: string }) {
  ...
  return <dialog ref={dialog} className={styles.dialog} aria-labelledby={title} onClose={onClose} onCancel={onClose}
    ...
    <div className={styles.dialogInner}>
      <button type="button" autoFocus onClick={onClose} className={styles.close} aria-label={t('Close preview')}>×</button>
      <div className={styles.player}>
        {failed ? <p role="status"><UiLabel>This preview is unavailable. You can still explore this style in the Studio.</UiLabel></p>
          : <video src={video.videoUrl} poster={video.posterUrl ?? showcasePoster(video)} controls autoPlay muted playsInline preload="metadata"
              aria-label={`${t('Preview')}: ${video.title}`} onError={() => setFailed(true)} />}
      </div>
      <div className={styles.previewInfo}>
        <span className={styles.eyebrow}>{video.badge} · <UiLabel>Preview</UiLabel></span>
        <h2 id={title}>{video.title}</h2>
        <p><UiLabel>A short preview from a film made with Kineo.</UiLabel></p>
        {video.href && <Link className={styles.action} href={video.href}>
          {actionLabel ? t(actionLabel) : video.engine === 'static_example' ? t('Explore this example') : `${t('Create with')} ${video.badge}`} <span aria-hidden="true">→</span>
        </Link>}
      </div>
    </div>
  </dialog>
...
export default function ExamplesGallery({ videos, startPaused = false, separateFeatured = false, heroOnly = false, featuredCount = 3, previewActionLabel, featuredPlaylists }: {
  ...
  return <div className={`${styles.gallery}${heroOnly ? ` ${styles.homeHero}` : ''}`}>
    {videos.length >= 3 && <section className={styles.featuredSection} aria-label={t('Featured examples')}>
      <div className={styles.sectionTop}><span className={styles.eyebrow}><UiLabel>The Kineo selection</UiLabel></span>
        <button type="button" className={styles.quietButton} onClick={() => setPaused(!paused)} aria-pressed={paused}>{t(paused ? 'Play preview' : 'Pause preview')}</button>
      </div>
