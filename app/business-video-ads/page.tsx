...
export default function BusinessVideoAdsPage() {
  return <main className={styles.page}>
    <nav className={styles.nav} aria-label="Business video navigation"><a href="/" className={styles.brand}>kineo<span> / empresas</span></a><a href="/studio">Make it yourself in Studio ↗</a></nav>
    ...
    <section className={styles.section} aria-labelledby="briefs-heading">
      ...
      <div className={styles.briefs}>{BRIEFS.map(brief => <article className={styles.brief} key={brief.label}><span className={styles.category}>{brief.label}</span><small>{brief.tag}</small><h3>{brief.title}</h3><p>{brief.text}</p></article>)}</div>
    </section>
