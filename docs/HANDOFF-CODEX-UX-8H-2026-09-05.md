...
**IMPLEMENTADO NA BRANCH / L2c:** três ocorrências JSX no histórico (duas apresentações mutuamente exclusivas do marco + card) e uma na biblioteca consomem o mesmo adaptador. Prefetch desligado. Mesmo tema/instrução, Fast/35s, atribuição e saída simples para título ausente no marco. Nada muda nos gatilhos, ofertas, preços, downloads, filtros, abas ou acervo. `lib/seriesContinuation.ts` segue intocado. Não chamar isto de migração dos roteiros prontos ou de todos os links do site.

**TESTADO LOCALMENTE:** `test-archive-studio-review.mjs` → **26 cenários**; extrai e executa quatro JSX reais e a expressão followUpHref, preserva ref de impressão/metadados de clique e testa ida/volta pelo Studio. Inclui duas contagens, Unicode e título fallback. Gate de diff normalizado exige que os dois arquivos mudem SOMENTE imports, helper de href e prefetch; resto idêntico à base. Não monta History/Library inteiros nem executa observer/browser.

**REGRESSÕES:** history-second-video-milestone **44/44**, library-caminho-volta **26/26**, L2 **16/16**, faixa home nova **13/13**. TypeScript exit 0 sem diagnóstico. As três falhas iniciais do teste legado da biblioteca eram âncoras do nome/import da chamada direta; atualizadas com justificativa no arquivo. O comportamento real passa a ser coberto pelo teste de JSX/reader, não pela regex. Revisão React aplicada: nenhum hook, listener, fetch ou dependência acrescentado aos componentes; Link e refs/handlers preservados.

**PREVIEW CONSOLIDADO:** `UX-BLOCO1-CONTINUACAO-STUDIO-2026-09-05.html` ganhou seção L2c com as quatro ocorrências antes/depois, desktop/mobile. JSX real, contexto visual simplificado, URLs exibidas e links desativados; sem conta, mídia ou serviços. As duas apresentações de marco são mostradas separadas, não como se convivessem no produto. Aprovação visual e validação de browser continuam abertas; nenhum merge/main/deploy.

### Checkpoint 14:44 BRT — abrangência de navegação, sem falsos consertos

**FATO CONFIRMADO / diagnóstico local em 05/09 14:47 BRT:** `diagnose-static-navigation.mjs` leu 308 arquivos TSX de app/components e as 124 páginas do inventário. Encontrou 477 ocorrências de href interno literal: 463 casam com forma de rota de página e 14 com route handler. Nenhuma ficou sem caminho candidato após incluir handlers. São ocorrências de código, não 477 botões validados nem pessoas. Há também 345 expressões href não literais fora desta checagem; handlers onClick/router.push e condições não foram executados.

**CORREÇÃO DE MÉTODO:** uma primeira consulta que considerava apenas page.tsx apontou `/free`, `/agency-production-scope.txt` e `/shorts-ideas.xml` como ausentes. Todos têm route.ts: não são 404s demonstrados. Não reescrevi esses links, não criei páginas duplicadas e não chamei o scanner de teste de UX. O script salvo inclui os handlers e declara limites: auth, query, valor de rota dinâmica, método HTTP, destino semântico e browser ainda precisam verificação.

**ESTADO:** main permanece `2ca9a06c`, sem aprovação visual nova. Nenhuma edição de runtime neste checkpoint. O diagnóstico não executa imports do produto, rede, middleware, banco ou APIs; apenas fs/AST/Git. Próximo foco continua nos contratos dinâmicos e nos previews já preparados, não em trocar links que já têm destino por parecerem inexistentes. Uso não reconsultado fora da cadência (última leitura 37%/63%, próxima 16:14 BRT). Término 18:14.

### Checkpoint 15:14 BRT — reserva L2d

**RESERVA UX / ACK DE TELAS MANTIDO:** `components/ActiveRenderPill.tsx`, somente import e destino em `handleNextEpisode`; adicionar fonte `render_pill` ao adaptador próprio. As duas apresentações do botão (Next episode / Series) partilham o mesmo handler. Estado/probe/poller, Watch, dismiss, fila guardada e Make it now permanecem exatamente iguais. Main segue `2ca9a06c`, sem nova reserva concorrente observada. Testar o handler real e cliques JSX com storage normal/indisponível, contexto e dismiss preservados; preview de percurso, sem produção.

**IMPLEMENTADO NA BRANCH / L2d:** somente import e chamada do adaptador no handler. Fonte render_pill preservada; tema abre revisão no Studio. Nenhuma edição de compose/active, probe, estados de render ou lógica da fila. Comparei o componente inteiro à base normalizando apenas esses dois nomes de import/chamada: igualdade exata. O fallback vazio continua Studio simples; o botão permanece condicionado a completed + tema conforme código anterior.

**TESTADO LOCALMENTE:** `test-ready-notice-studio-review.mjs` → **13 cenários**, handler real + duas ocorrências JSX, UTF/tema vazio, storage normal ou indisponível, eventos e descarte iguais antes/depois, ida/volta no leitor/Generate. HTML dos dois botões idêntico à base. Não monta/polla o componente nem aciona dados reais. Regressão estática da pílula **29/29**, arquivo de navegação **26/26**, Studio **16/16**; TypeScript exit 0 sem diagnóstico. Duas âncoras do teste legado eram nomes/import da chamada direta e foram atualizadas com justificativa; garantias comportamentais estão no teste novo, não nessas regexes.
