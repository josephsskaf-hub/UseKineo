...
-- servido em produção ~16:37 UTC de 24/09). Base medida antes: 13 pessoas/7 d apertaram Gerar e não receberam filme
...
select case when e.created_at < '2026-09-24 16:37+00' then 'antes' else 'depois' end as janela,
       e.metadata->>'status' as status, count(*) as pedidos, count(distinct e.user_id) as pessoas
from events e
where e.name = 'render_job_finished' and e.created_at > '2026-09-17 16:37+00'
group by 1, 2 order by 1, 2;

-- 2. A carta falsa "a aba fechou" deve cair (ela saía para quem o cron via como órfão e recebia 409).
select case when created_at < '2026-09-24 16:37+00' then 'antes' else 'depois' end as janela,
       count(*) as cartas, count(distinct user_id) as pessoas
from events where name = 'attempt_lost_rescue_sent' and created_at > '2026-09-17 16:37+00'
group by 1;
...
from events e where e.name = 'engine_fit_box_shown' and e.created_at > '2026-09-17 16:37+00'
group by 1;
...
select count(*) as apertaram,
       ...
       count(*) filter (where st.t0 >= '2026-09-24 16:37+00') as apertaram_depois_do_deploy,
       count(*) filter (where st.t0 >= '2026-09-24 16:37+00' and not exists (select 1 from videos v where v.user_id = st.user_id and v.status = 'completed' and v.created_at >= st.t0 - interval '10 minutes')) as sem_filme_depois
