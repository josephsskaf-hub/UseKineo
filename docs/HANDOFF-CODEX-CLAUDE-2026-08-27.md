...
**IMPLEMENTADO:** o commit funcional `454f3befef4176b3f067b706abf62f40ba5437ce`, refinado pelo forward-fix posterior, mostra `Card, or Apple Pay / Google Pay when available · Link sign-in is optional` nos três planos públicos e no CTA pós-vídeo. O item sempre visível do Stripe acrescenta que carteiras dependem de disponibilidade e `Link is optional — choose Pay without Link to enter your card.` A fonte única é `lib/growth/checkoutPaymentGuidance.ts`; preço, grant e métodos continuam provider-owned.
...
## 99. B2C — cadastro reconhece uma compra interrompida sem revelar se a conta já existe (01/09/2026)

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** as intervenções versionadas `limit_purchase_fit_v1`, `checkout_password_recovery_handoff_v1` e `autopilot_break_even_v1` ainda não tinham pessoas externas expostas na consulta; seus gates permanecem congelados. Em uma janela separada de 48 horas, 28 pessoas externas concluíram o primeiro vídeo, 27 chegaram ao estado de vídeo pronto, 14 baixaram, sete viram a oferta do histórico, duas viram a oferta pós-vídeo, sete viram a ponte de saldo, duas chegaram a pricing, uma chegou ao checkout e uma pagou. Seis pessoas baixaram sem nenhum evento de oferta; entre as não pagantes, 20 ainda tinham saldo positivo e sete estavam com saldo zero. É um retrato de diagnóstico, não uma atribuição causal entre os estágios.

**EVIDÊNCIA DE PRODUÇÃO / PAREDE DE AUTENTICAÇÃO (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** em sete dias, `checkout_auth_page_view` alcançou 14 atores e `checkout_auth_method_selected` alcançou seis, todos selecionando Google. `checkout_auth_required` teve 22 eventos atribuídos a seis atores, portanto os eventos não foram contados como pessoas. A amostra justifica investigar a resolução de cadastro, mas ainda não mede o novo estado.

**FATO CONFIRMADO / LACUNA NO CÓDIGO ANTERIOR:** a rota Stripe envia um comprador não autenticado para signup preservando o checkout exato. O signup identifica a resposta Supabase de usuário já existente por `identities.length === 0`, mas levava esse caso ao mesmo estado visual do cadastro novo e sempre dizia `Check your email! We sent a confirmation link`. Em webview, onde o OAuth automático já é suprimido, uma pessoa com conta existente podia esperar um e-mail de confirmação que não corresponde ao próximo passo correto. O `AuthModal` já tratava a distinção; a lacuna era específica da página completa de signup.

**HIPÓTESE CAUSAL NOVA:** uma instrução neutra que nomeia a compra interrompida e oferece login como ação imediata reduz abandono de compradores com conta existente, sem enumerar conta, alterar autenticação ou interferir na confirmação legítima de uma conta nova.

**IMPLEMENTADO:** o commit funcional `eca6224ded398182b6305e61a047133d6cff1f0a` adiciona `checkout_signup_resolution_v1`. `lib/growth/checkoutSignupResolution.ts:21` só ativa a nova mensagem em uma retomada de checkout; `app/(auth)/signup/page.tsx:273` reaproveita o `loginHref` que preserva o destino exato e `:504` mantém a mensagem anterior para signup comum. O novo estado usa `Finish your purchase`, explica de forma neutra que usuários existentes devem entrar e contas novas devem usar o link de confirmação, declara que o checkout está salvo e oferece `Continue to Sign In`. Não abre checkout automaticamente.

**PRIVACIDADE / ANTIENUMERAÇÃO:** `lib/authAnalytics.ts:129-130` grava somente `checkout_signup_resolution_viewed` ou `checkout_signup_resolution_clicked`, versão, superfície, ação e categorias allow-listed do destino. E-mail, senha, estado de identidade, existência da conta, URL crua e redirect livre não entram na telemetria. A mensagem é deliberadamente a mesma para conta existente e nova.

**MEDIÇÃO / GATE:** seguir pessoas externas autenticadas em `checkout_signup_resolution_viewed → checkout_signup_resolution_clicked → checkout_started → payment_success`, preservando a variante até cinco pessoas externas versionadas chegarem novamente a `checkout_started`. Pagamento real é a métrica final. Parar por perda de tier, billing ou campanha, loop signup/login, enumeração de conta ou regressão da confirmação de cadastro comum.

**COMPARAÇÃO VISUAL:** `docs/previews/CHECKOUT-SIGNUP-RESOLUTION-V1-2026-09-01.html` contém antes/depois desktop e mobile e foi inspecionado no Chrome conectado. O estado novo mantém a compra salva como contexto e torna o login a ação primária; a experiência comum continua separada.

**TESTADO LOCALMENTE:** contrato novo 27/27, recuperação de senha 57/57, contexto B2B de autenticação 26/26, destino de produto no signup 109/109, prova de criação no signup 44/44 e verdade orgânica do signup 36/36 — 299 verificações. TypeScript repetiu exatamente os quatro erros baseline preexistentes — `mrr.ts`, `me/subscription` e dois em `stripe/checkout` — e nenhum arquivo desta entrega introduziu erro. Whitespace ficou limpo. A revisão React não encontrou request, listener, pacote, estado global ou waterfall novo.

**VALIDADO EM PRODUÇÃO COM FRONTEIRA EXPLÍCITA (01/09/2026 UTC):** `origin/main` avançou por fast-forward para `eca6224ded398182b6305e61a047133d6cff1f0a`. O deploy Vercel `dpl_4URboEcVaGHTDjLukdQGjyYgEBEq` chegou a `READY`, target production, SHA exato e alias em `www.usekineo.com`; `/signup` teve zero erro de runtime na janela consultada. O canário no Chrome estava autenticado como conta interna do fundador, portanto pulou corretamente signup e criou uma única Session interna de Creator mensal. A leitura no Stripe confirmou `open`, `unpaid`, sem PaymentIntent e sem assinatura; nenhuma forma foi preenchida e nenhum pagamento ocorreu. A aba foi fechada. Assim, rota/deploy e continuidade autenticada estão **VALIDADOS EM PRODUÇÃO**, enquanto o estado de sucesso após um cadastro real permanece **IMPLEMENTADO / TESTADO LOCALMENTE**, aguardando uso legítimo — não foi fabricada conta externa para promovê-lo artificialmente.
...
## 101. B2C — uma falha no callback OAuth não apaga mais a compra escolhida (01/09/2026)

**FATO CONFIRMADO NO CÓDIGO ANTERIOR:** `app/(auth)/signup/page.tsx` enviava o destino exato do checkout no parâmetro `next`, e o callback o retomava quando `exchangeCodeForSession` funcionava. Se o código estivesse ausente, expirado ou a troca falhasse, `app/auth/callback/route.ts` gravava apenas `had_code` e redirecionava para `/login?error=oauth_failed`, descartando provedor de pagamento, plano, periodicidade e campanha. A página de login só reconhece uma compra preservada quando recebe `reason=checkout` e `redirect`; portanto o comprador voltava para um login genérico e precisava reconstruir a compra.

**HIPÓTESE CAUSAL NOVA:** preservar o checkout já escolhido durante uma falha técnica ou cancelamento do OAuth reduz abandono no último metro sem mudar preço, oferta, autenticação ou iniciar pagamento automaticamente.

**IMPLEMENTADO:** o commit funcional `9a280a47e7d259d3b4b88315b2a8fa19186cea19` adiciona `checkout_oauth_failure_handoff_v1`. `lib/growth/checkoutOAuthFailureHandoff.ts` reaproveita a mesma validação fechada e same-origin da recuperação de senha, aceita apenas `/api/stripe/checkout`, `/api/paypal/checkout` ou `/api/mercadopago/checkout`, e constrói o fallback `/login?error=oauth_failed&reason=checkout&redirect=<checkout exato>`. Destino ausente, externo, malformado, página comum ou caminho parecido mantém exatamente o fallback legado. `app/auth/callback/route.ts` usa o contrato antes da troca com o provedor e grava a falha antes do redirect.

**PRIVACIDADE / SEMÂNTICA:** `auth_callback_failed` recebe apenas `version`, `is_checkout_destination`, `checkout_provider`, `tier`, `billing`, `intent_campaign` e o `had_code` já existente. URL/redirect cru, promoção, código OAuth, token, erro livre, e-mail e identificador não entram na telemetria. O campo se chama `checkout_provider`; `provider` continua reservado ao Google/Apple nos callbacks concluídos, evitando misturar provedor de pagamento com provedor de autenticação.

**TESTADO LOCALMENTE:** o contrato novo passou 56/56; recuperação de senha 57/57; resolução de cadastro 27/27; contexto B2B de autenticação 26/26 — 166 verificações. Os testes exercitam os três checkouts, destino exato, open redirect, backslash, caminho semelhante, página comum e categorias inválidas. Uma asserção foi corrigida antes do commit: ela confundia o booleano `is_checkout_destination` com uma URL crua; passou a excluir somente esse campo categórico e continua rejeitando qualquer chave livre de navegação ou credencial. O typecheck repetiu somente os quatro erros baseline preexistentes em `mrr.ts`, `me/subscription` e `stripe/checkout` ×2; nenhum erro novo. Whitespace limpo.

**VALIDADO EM PRODUÇÃO COM FRONTEIRA EXPLÍCITA (01/09/2026 UTC):** `origin/main` avançou por fast-forward para o SHA funcional exato. O deploy Vercel `dpl_2cE6FW3yixkrcGPdLRwy4edTSmwZ` chegou a `READY`, target production, framework Next.js, aliasado em `www.usekineo.com`; o build terminou sem erro e `/auth/callback` apresentou zero erro de runtime na janela de 15 minutos consultada. O caminho de falha não foi fabricado em produção para não criar evento anônimo falso nem interferir em uma sessão OAuth legítima. Portanto commit, build, alias e ausência de erro estão **VALIDADOS EM PRODUÇÃO**; o redirect após uma falha real permanece **IMPLEMENTADO / TESTADO LOCALMENTE** até ocorrer organicamente.

**MÉTRICA / GATE:** contar somente pessoas externas que reaparecem autenticadas depois de `auth_callback_failed(version=checkout_oauth_failure_handoff_v1,is_checkout_destination=true)` e seguem para `checkout_started`; `payment_success` continua sendo a métrica final. Evento anônimo isolado não é pessoa. Preservar a variante até cinco pessoas externas autenticadas retomarem checkout ou até o primeiro sinal de parada.

**PARADA / RISCO:** interromper se tier, billing ou campanha divergirem do checkout original, se surgir loop OAuth ↔ login, redirect externo, dado livre no evento ou alteração em callback não-checkout. Não houve mudança visual, portanto não se aplica comparação antes/depois.
...
## 105. B2C — falha anterior à Stripe preserva a compra e oferece retry exato (01/09/2026)

**RECONCILIAÇÃO DA INTERVENÇÃO ANTERIOR / EVIDÊNCIA DE PRODUÇÃO (Supabase, SELECT somente leitura em 01/09/2026 02:57:41 UTC; contas internas excluídas):** a consulta por `home_outcome_selector_v3` ainda retornou zero pessoas externas. A variante B2B da seção 104 permanece congelada até o próprio gate; o smoke interno não foi promovido a amostra.

**EVIDÊNCIA DE PRODUÇÃO / DIAGNÓSTICO EM JANELAS SEPARADAS (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** nas 48 horas medidas, 29 pessoas externas chegaram a `video_ready_viewed`, 16 baixaram vídeo, 12 chegaram a `pricing_view`, uma apareceu em `checkout_started`, uma em `checkout_session_expired` e uma em `payment_success`. Esses totais por estágio não foram somados como um único funil nem tratados como causalidade. Entre as 29 pessoas com vídeo pronto, cinco baixaram com saldo atual zero sem `trial_balance_bridge_viewed`, oferta, repeat, pricing, checkout ou pagamento na cadeia observada.

**FATO CONFIRMADO / LACUNA NO CÓDIGO ANTERIOR:** os builders normais da rota Stripe já gravavam `checkout_failed`, mas o catch externo de `app/api/stripe/checkout/route.ts` devolvia somente `/pricing?checkout_error=...`. Ele não nomeava o estágio, descartava tier, billing, pack, campanha e contexto Plan Fit e levava a uma frase vermelha sem ação em `app/pricing/PricingClient.tsx`. Esse caminho ocorre antes de existir Checkout Session, e `pagehide` encerra o watchdog normal; portanto a retomada de sessão não o resgatava. A mesma leitura do cliente fazia `decodeURIComponent` depois de `URLSearchParams.get`, isto é, uma segunda decodificação capaz de lançar `URIError` em mensagens com `%`.

**FRONTEIRA DA EVIDÊNCIA:** a busca de runtime das últimas 24 horas não encontrou ocorrência recente da assinatura antiga `[stripe/checkout GET] Unexpected error`; a incidência real deste terminal path é **DESCONHECIDA**. A mudança corrige um beco determinístico confirmado em código, mas não é apresentada como a causa dos checkouts recentes sem pagamento.

**HIPÓTESE CAUSAL NOVA:** quando uma falha inesperada acontece antes da Stripe, preservar exatamente a escolha já feita e oferecer uma única tentativa explícita reduz uma perda técnica evitável sem desconto, pressão ou mudança de oferta.

**IMPLEMENTADO:** o commit funcional `cab55f4b5aa3f1489c4918b9fe07331bfc03b3c4` publica `checkout_setup_failure_return_v1`. `lib/growth/checkoutSetupFailureReturn.ts:36` aceita somente a rota Stripe same-origin que o handoff de autenticação já valida; `:68` encapsula a URL exata dentro do retorno versionado à pricing. O catch em `app/api/stripe/checkout/route.ts:2825-2845` grava `checkout_failed(stage=outer_get,reason=unexpected_server_error)` antes do redirect e nunca registra corpo ou mensagem do provedor. `app/pricing/PricingClient.tsx:594-606` reutiliza `useCheckoutLaunch`, preservando trava de clique e idempotência, e `:1087-1105` mostra `No payment was created` com `Try secure checkout again`. Erros antigos que não trazem o contrato versionado mantêm a apresentação anterior.

**SEGURANÇA / PRIVACIDADE:** destinos externos, backslash, página parecida, PayPal e Mercado Pago são rejeitados por este contrato Stripe. A navegação exata pode preservar promoção, retorno e campos assinados do Plan Fit, mas telemetria e chave de dedupe contêm apenas versão, `product_kind`, seleção allow-listed, billing e campanha categórica. URL, redirect, cupom, campo Plan Fit, mensagem livre, e-mail e erro do provedor não entram nos eventos. Tier desconhecido reflete o comportamento real do servidor (`basic`); pack desconhecido reflete o fallback real (`starter`).

**MEDIÇÃO / GATE:** seguir pessoas externas em `checkout_failed(stage=outer_get) → checkout_setup_failure_return_viewed → checkout_setup_failure_retry_clicked → checkout_started` em 30 minutos → `payment_success` em 24 horas. Preservar até dez pessoas externas expostas. Parar imediatamente por tier/billing/pack diferente, URL externa, mais de uma Session por clique, mensagem livre em telemetria ou regressão da pricing normal. Zero evento externo versionado na consulta pós-deploy é baseline, não sucesso nem fracasso.

**TESTADO LOCALMENTE:** contrato novo 109/109; recuperação de senha 57/57; falha OAuth 56/56; tier handoff 39/39; orientação de pagamento 29/29; janela de Session 25/25; verdade de falha Stripe 67/67; atribuição de escolha 26/26; barra mobile 29/29 — 436 verificações. O typecheck repetiu exatamente os quatro erros baseline preexistentes em `mrr.ts`, `me/subscription` e `stripe/checkout` ×2, deslocados apenas pelas novas linhas; nenhum erro novo. Whitespace ficou limpo.

**COMPARAÇÃO VISUAL:** `docs/previews/CHECKOUT-SETUP-FAILURE-RETURN-V1-2026-09-01.html` contém antes/depois responsivo. O Chrome recusou abrir o arquivo `file://` por política própria; não houve contorno. A apresentação real foi inspecionada no deploy de produção: card azul discreto sob os planos, explicação, botão habilitado e a seção normal `What one credit buys` ainda presente.

**VALIDADO EM PRODUÇÃO (01/09/2026 UTC):** `origin/main` avançou por fast-forward para o SHA funcional exato. O deploy Vercel `dpl_DUcxCmj3KRHh9Te78xAajFHq1TzJ` chegou a `READY`, target production, framework Next.js, aliasado em `www.usekineo.com`, sem erro de alias; o build terminou sem erro. Um retorno interno controlado em pricing confirmou mensagem, ação habilitada e página normal. O botão não foi acionado: nenhuma Checkout Session ou pagamento foi criado, e a aba foi fechada. A varredura de runtime mostrou somente um aviso `url.parse()` em rotas de geração e no deploy anterior `dpl_DCMe...`; é pista do Claude e não regressão desta entrega.

**ACHADO PARA A PISTA DO CLAUDE / NÃO EDITADO:** cinco pessoas externas baixaram vídeo com saldo zero sem ponte de oferta na cadeia observada. `app/(dashboard)/my-videos` pertence à pista do Claude; Codex não alterou essa superfície. Claude deve reconciliar se outputs premium limpos fora de `isWatermarkedFastAsset` terminam sem qualquer próximo passo quando `cleanExportLocked` é falso, contando pessoas e preservando os experimentos já ativos.

**NÃO TOCADO:** preço, plano, desconto, cupom, crédito, validade, trial, SKU, promessa, método de pagamento, Stripe Session/webhook/configuração, Supabase schema/dados/configuração, render, motor, duração, cena, voz, legenda, admin, e-mail, outreach, anúncio ou contato externo.

**PRÓXIMO DONO:** Claude deve executar `git fetch origin` e partir de `cab55f4b` ou da ponta posterior. Não reeditar pricing nem esta recuperação antes do gate. Tratar somente o achado cross-lane de My Videos na própria worktree, sem tocar no contrato de checkout. Codex alterna a próxima sprint para B2B/afiliados em uma superfície diferente.

## 106. REGRA DO BOARD + B2B — recrutar afiliados com audiência de empresas (01/09/2026)

**DECISÃO APROVADA PELO FUNDADOR (01/09/2026):** toda tarefa geral do board UseKineo precisa declarar como contribui para mais assinaturas no curto, médio e longo prazo. Clique, visita, cadastro, aplicação de afiliado ou checkout não são receita. `payment_success` e assinatura ativa permanecem o placar final. Uma mudança sem cadeia causal até assinatura, métrica e gate não entra na fila; iniciativas B2C, B2B, afiliados, AEO e checkout devem se complementar em vez de disputar a mesma pessoa ou reeditar a mesma superfície antes da amostra mínima.

**EVIDÊNCIA DE PRODUÇÃO / BASELINE (Supabase, SELECT somente leitura em 01/09/2026 03:07:22 UTC; contas internas excluídas):** nos 30 dias observados, `/partners` registrou 15 sessões anônimas distintas em `landing_session_started` e seis sessões anônimas distintas em `organic_cta_clicked`; sessão anônima não foi chamada de pessoa. Nove pessoas externas enviaram a aplicação, as mesmas nove concluíram a ativação, cinco pessoas copiaram ao menos um link de destino e nenhuma copiou o destino `business`. Existem 11 afiliados externos ativos, nove criados em 30 dias. `affiliate_clicks` registrou 19 linhas em 30 dias, zero com destino business; indicação externa e indicação convertida permaneceram em zero. Nenhum desses números é receita.

**AUDITORIA ANTI-DUPLICAÇÃO / FATO CONFIRMADO:** o dashboard interno do afiliado e `lib/affiliateDestinations.ts` já possuíam a campanha B2B `business`, apontando para `/business-video-content-plan`. A página pública `/partners`, porém, recrutava somente três públicos de creators. A seção 80 congelou o kit interno até cinco exposições ou cinco cliques business; esta entrega não o reeditou. Ela atua antes da aplicação, na superfície pública distinta que seleciona quem entra no programa.

**HIPÓTESE CAUSAL NOVA:** parceiros que atendem empresas, freelancers e agências podem se excluir antes de aplicar porque a página pública apresentava o programa como creator-only, apesar de a campanha B2B já estar pronta depois da ativação. Expor esse caso de uso pode recrutar distribuidores adequados sem criar nova landing page, desconto ou promessa.

**IMPLEMENTADO:** o commit funcional `14f011151518208101dc8e213d380906fe945270` adiciona à grade de `/partners` o quarto card `Businesses & freelancers`, com explicação factual e CTA `Apply and get the business campaign →`. O CTA reutiliza exatamente o fluxo existente `/affiliate?utm_source=partners&utm_medium=organic&utm_campaign=push33_partner_program`. Os três cards anteriores, comissão, preço, checkout e contrato de destinos permaneceram intactos. A grade foi ajustada para 2×2 equilibrado em desktop e uma coluna sem overflow em mobile.

**MEDIÇÃO / PRIVACIDADE:** `affiliate_business_recruitment_viewed` exige ao menos 60% de visibilidade e só entra no gate no host canônico exato `www.usekineo.com`; preview e localhost não contam. `affiliate_business_recruitment_clicked` mede o CTA e preserva o evento geral `organic_cta_clicked`. Metadados são somente categorias allow-listed: versão, surface, placement, audience e destination; sem URL livre, e-mail, preço, prompt ou identificador novo. O smoke de produção não rolou até a exposição e a consulta pós-deploy confirmou zero eventos versionados fabricados.

**CADEIA OBRIGATÓRIA ATÉ ASSINATURA:** curto prazo = visitante de `/partners` reconhece o público B2B, clica e aplica; médio prazo = afiliado ativado copia a campanha business, leva uma empresa ao plano semanal e ao pack/checkout existente; longo prazo = visitante referido cria conta, paga e renova, produzindo assinatura e comissão recorrente. Cada etapa será medida separadamente por pessoa externa; nenhuma intermediária será apresentada como venda.

**GATE / PARADA:** preservar a variante até dez exposições externas reais. Sinal inicial: duas pessoas distintas clicam o CTA business e ao menos uma conclui aplicação ou copia o link business. Pagamento referido é o resultado final. Parar ou reformular com dez exposições e zero clique, ou se a tração do CTA geral cair sem avanço B2B. Não reeditar `/partners` nem o kit de afiliado antes do respectivo gate.

**COMPARAÇÃO VISUAL:** `docs/previews/AFFILIATE-BUSINESS-RECRUITMENT-V1-2026-09-01.html` mostra antes/depois desktop e mobile; o SVG adjacente é artefato suplementar. Dois previews reais foram inspecionados no Chrome: a primeira grade deixava o quarto card órfão, então foi corrigida antes da produção; a versão final ficou em 2×2 no desktop e uma coluna em 390 px, sem overflow. A aba de produção foi fechada após a validação.

**TESTADO LOCALMENTE:** recrutamento business 48/48 e destinos de afiliado 266/266 — 314 verificações determinísticas. O typecheck repetiu exatamente os quatro erros baseline preexistentes em `mrr.ts`, `me/subscription` e `stripe/checkout` ×2; nenhum erro novo. `git -c core.whitespace=cr-at-eol diff --check` ficou limpo após rebase na ponta do Claude.

**VALIDADO EM PRODUÇÃO (01/09/2026 UTC):** `origin/main` avançou por fast-forward para `14f011151518208101dc8e213d380906fe945270`. O deploy Vercel `dpl_887iMZ6BmQxZq8SFfxEjEyWPk5GY` chegou a `READY`, target production, SHA exato e alias em `www.usekineo.com`. O Chrome conectado confirmou card único, CTA único e href exato; zero erro de console. A Vercel confirmou zero erro de runtime em `/partners` na janela de uma hora.

**NÃO TOCADO:** preço, plano, comissão, desconto, cupom, crédito, trial, SKU, promessa, checkout, Stripe, Supabase schema/dados, dashboard interno de afiliado, render, motor, cena, voz, legenda, admin, e-mail, outreach, anúncio ou contato externo.

**PRÓXIMO DONO:** Claude deve executar `git fetch origin` e partir de `14f01115` ou da ponta posterior. Não reeditar `/partners`, o kit de afiliado ou o destino business antes dos respectivos gates. Codex alterna a próxima sprint para B2C, mantendo a regra do board: toda hipótese deve explicitar curto, médio e longo prazo até assinatura real.

## 107. B2C — Free/Starter deixam de receber uma compra de créditos que o servidor recusa (01/09/2026)

**EVIDÊNCIA DE PRODUÇÃO / BASELINE (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** nos 60 dias observados, três pessoas externas com plano atual `free` clicaram o CTA `credits_topup_modal_sidebar_chip`. As três chegaram a `pricing_view` em até 30 minutos; nenhuma abriu Checkout Session de top-up e nenhuma apresentou `payment_success`. Uma dessas pessoas já tinha concluído e baixado vídeo antes do clique. Uma pessoa tentou Creator depois e outra tentou Creator e Autopilot no mesmo intervalo, mas nenhuma pagou. Isso prova o desvio; não prova sozinho que a copy anterior causou toda a não conversão.

**FATO CONFIRMADO / LACUNA ANTERIOR:** `components/Sidebar.tsx` abria `CreditsTopupModal` para qualquer pessoa logada e `components/AccountPanel.tsx` mostrava `Buy more credits` inclusive para Starter, porque usava `isPaid` como substituto de elegibilidade. O servidor em `app/api/stripe/checkout/route.ts` permitia top-up somente a `basic`, `basic_trial`, `pro` e `pro_trial`; Free e Starter eram desviados a `/pricing` depois de pedir a compra. O GET inelegível fazia redirect direto e não emitia `checkout_failed`, deixando o motivo invisível.

**HIPÓTESE CAUSAL NOVA:** dizer o próximo passo verdadeiro antes do clique reduz uma ida e volta no último metro: Free/Starter escolhem plano; Creator/Studio compram crédito avulso. A mudança não promete desconto nem força checkout.

**IMPLEMENTADO:** o commit funcional `b6814982b039bf938167a6eed57d8e0157b09202` publica `topup_eligibility_handoff_v1`. `lib/growth/topupEligibility.ts` é a fonte única de elegibilidade compartilhada pelo Sidebar, AccountPanel e rota Stripe. Free, Starter, plano desconhecido ou ainda não resolvido nunca abrem o modal; após a leitura do plano, usuários inelegíveis veem `See plans` e vão direto a `/pricing`. Creator e Studio preservam `+`, `Top up anytime` e o modal existente. No AccountPanel, o CTA primário também respeita a mesma política. O GET/POST inelegível agora registra `checkout_failed(reason=topup_requires_creator_plus)` antes do redirect/403.

**MEDIÇÃO / PRIVACIDADE:** `topup_eligibility_handoff_viewed` exige 60% de visibilidade, plano resolvido, estado inelegível e host canônico exato `www.usekineo.com`; `topup_eligibility_handoff_clicked` mede o avanço. Eventos bem-sucedidos deduplicam por sessão e falha de gravação não fecha a trava. Metadados contêm somente versão, superfície, estado categórico e destino; plano bruto, saldo, preço, e-mail, URL e UTM não são adicionados.

**CADEIA OBRIGATÓRIA ATÉ ASSINATURA:** curto prazo = pessoa inelegível entende que precisa escolher um plano e chega a pricing sem recusa intermediária; médio prazo = `pricing_view → checkout_started`; longo prazo = `payment_success → assinatura ativa/renovação`. Visualização, clique, pricing e Checkout Session não são receita.

**GATE / PARADA:** preservar até dez pessoas externas inelegíveis expostas. Sinal inicial exige duas pessoas distintas clicando e pelo menos uma chegando a `checkout_started`; pagamento real é o resultado final. Parar imediatamente se Free/Starter ainda abrir modal de top-up, Creator/Studio perder o modal, plano desconhecido puder iniciar compra, metadado livre surgir ou preço/crédito/SKU divergir. Não reeditar Sidebar/AccountPanel nesta hipótese antes da amostra mínima.

**COMPARAÇÃO VISUAL:** `docs/previews/TOPUP-ELIGIBILITY-HANDOFF-V1-2026-09-01.html` mostra antes/depois desktop e mobile e foi inspecionado no Chrome conectado. O estado Free/Starter troca promessa de compra por `See plans` e seta; o quadro adjacente demonstra que Creator/Studio mantém o `+` e o modal. A apresentação ficou legível em desktop e 390 px; a aba e o servidor temporário foram fechados.

**TESTADO LOCALMENTE:** contrato específico 105/105; verdade de falha Stripe 67/67; orientação de pagamento 29/29; janela de Session 25/25; atribuição de escolha 26/26 — 252 verificações. O typecheck repetiu exatamente os quatro erros baseline preexistentes em `mrr.ts`, `me/subscription` e `stripe/checkout` ×2; nenhum erro novo. `git -c core.whitespace=cr-at-eol diff --check` ficou limpo. A revisão React confirmou plano desconhecido bloqueado, efeito com dependências primitivas, observer desconectado e nenhum listener/request duplicado.

**VALIDADO EM PRODUÇÃO COM FRONTEIRA EXPLÍCITA (01/09/2026 UTC):** `origin/main` avançou por fast-forward para `b6814982b039bf938167a6eed57d8e0157b09202`. O deploy Vercel `dpl_7J2RPQwMQQTBoyn5sUWyYxbrZhoP` chegou a `READY`, target production, SHA exato. No Chrome conectado, a conta interna Creator/Studio começou com o chip desabilitado durante a resolução e terminou em `Add credits`, `1114 credits`, `Top up anytime` e `+`; o AccountPanel manteve `Buy more credits` visível e habilitado. Zero erro no console e zero erro de runtime em `/generate`, `/api/me/plan`, `/api/me/subscription` e `/api/stripe/checkout` na janela consultada. Nenhum modal, Checkout Session ou pagamento foi iniciado. O estado Free/Starter está **IMPLEMENTADO / TESTADO LOCALMENTE** e aguarda uso orgânico externo; não foi criada ou alterada conta para fabricar o canário.

**NÃO TOCADO:** preço, desconto, cupom, grant, validade, trial, SKU, descrição de pack, catálogo Stripe, método de pagamento, webhook, Supabase schema/dados, render, motor, cena, voz, legenda, e-mail, outreach, anúncio ou contato externo.

**PRÓXIMO DONO:** Claude deve executar `git fetch origin` e partir de `b6814982` ou da ponta posterior. Não reeditar esta elegibilidade antes de dez exposições externas ou de um sinal de parada. Codex gira a próxima sprint para B2B em superfície distinta, mantendo assinatura real como placar do board.

## 108. B2B — checkout interrompido do Autopilot Pilot passa a ser retomável (01/09/2026)

**EVIDÊNCIA DE PRODUÇÃO / BASELINE (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** em 90 dias, quatro pessoas externas distintas chegaram a `checkout_attempted`, `checkout_started` e `checkout_session_expired` com `pack=autopilot_pilot`; foram quatro linhas por estágio e nenhum pagamento Pilot observado. Bulk não apareceu nesta coorte filtrada. Pessoas, não eventos, definiram o escopo.

**FATO CONFIRMADO / LACUNA ANTERIOR:** a retomada global aceitava apenas Checkout Session `mode=subscription`. O Autopilot Pilot é `mode=payment`, `pack=autopilot_pilot`, portanto uma saída silenciosa — fechar aba ou deixar a Session expirar — não produzia caminho global de volta. O retorno explícito por `cancel_url` já existia e foi preservado. Bulk continua fora porque não atingiu o gate de cinco pessoas e uma expiração.

**HIPÓTESE CAUSAL NOVA:** lembrar uma compra Pilot já escolhida, sem criar cobrança ao visualizar e sem alterar a oferta, recupera empresas que interromperam o checkout e cria uma ponte mensurável do teste B2B avulso para o Autopilot mensal.

**IMPLEMENTADO:** o commit funcional `7164de38c00ee3977856b8382624efe92a154146` publica `one_time_pilot_resume_v1`. A criação original do Pilot habilita a recuperação documentada da Stripe para `mode=payment`, guarda o identificador da Session somente em cookie privado e expõe ao cliente apenas um sinal booleano. A rota isolada `/api/stripe/checkout/pilot-resume` confirma autenticação, dono, produto exato, preço/moeda canônicos, estado não pago e ausência de entitlement antes de oferecer uma Session aberta, a URL de recuperação da Stripe ou um retry interno do mesmo pack. Visualizar nunca chama `checkout.sessions.create`; somente o clique explícito pode retomar ou recriar a Session. Estado terminal, preço divergente e cookie inconsistente falham fechados e são limpos para não repetir request a cada navegação.

**ISOLAMENTO / VERDADE COMERCIAL:** `AutopilotPilotResumeBanner` é separado do experimento recorrente `resume_smaller_choice_v1`, cujos três arquivos permaneceram byte a byte iguais. A copy informa preço vindo do servidor, `one time`, `no auto-renewal` e que o pagamento só termina dentro da Stripe. Bulk, assinatura recorrente, preço, SKU, crédito, validade e entitlement não mudaram. A revisão React encontrou dois riscos antes do commit: squeeze/overflow mobile e fetch global para visitantes sem intenção Pilot; ambos foram corrigidos com breakpoint real e hint booleano sem identificador, preço ou destino.

**MEDIÇÃO / PRIVACIDADE:** seguir pessoas externas em `checkout_session_expired(pack=autopilot_pilot) → autopilot_pilot_resume_viewed → autopilot_pilot_resume_clicked → checkout_started(pack=autopilot_pilot)` em 30 minutos → `payment_success` em 24 horas. Exposição exige 60% de visibilidade no host canônico. Eventos carregam somente variante, produto, tipo de compra, superfície e classe de destino; Session ID, URL, identidade e preço não entram na telemetria.

**CADEIA OBRIGATÓRIA ATÉ ASSINATURA:** curto prazo = recuperar a compra avulsa Pilot interrompida; médio prazo = a empresa que pagou recebe o teste operacional e comprova o uso recorrente; longo prazo = medir quantas pessoas com Pilot pago avançam ao Autopilot mensal e mantêm assinatura ativa. View, clique, Session e até o Pilot avulso não serão chamados de assinatura; `payment_success` mensal/assinatura ativa permanece o placar final.

**GATE / PARADA:** preservar até cinco pessoas externas elegíveis. Sinal inicial exige ao menos duas pessoas distintas clicando e uma reiniciando o checkout Pilot; pagamento Pilot é a primeira métrica financeira e conversão posterior ao mensal é a métrica de assinatura. Parar por produto/preço/moeda divergente, entitlement duplicado, mais de uma Session por clique, URL/ID em telemetria ou regressão da retomada de assinatura existente.

**COMPARAÇÃO VISUAL:** `docs/previews/AUTOPILOT-PILOT-RESUME-V1-2026-09-01.html` contém antes/depois desktop e mobile. O Chrome conectado revelou que a primeira versão horizontal apertava a ação em 390 px; a versão publicada empilha a ação, ocupa a largura disponível e não apresenta overflow. A aba e o servidor temporário foram fechados.

**TESTADO LOCALMENTE:** contrato específico 67/67; retorno Autopilot 36/36; preço/retomada 47/47; janela de Session 25/25; verdade de falha Stripe 67/67; orientação de pagamento 29/29 — 271 verificações. O typecheck repetiu exatamente os quatro erros baseline preexistentes em `mrr.ts`, `me/subscription` e `stripe/checkout` ×2; nenhum erro novo. Whitespace ficou limpo.

**VALIDADO EM PRODUÇÃO COM FRONTEIRA EXPLÍCITA (01/09/2026 UTC):** `origin/main` avançou por fast-forward para `7164de38c00ee3977856b8382624efe92a154146`. O deploy Vercel `dpl_FqNJ2dNbzSxPEDnMA9fAgso5dm7F` chegou a `READY`, target production, framework Next.js, SHA exato e alias em `www.usekineo.com`. Um GET anônimo confirmou `401`, `Cache-Control: private, no-store` e `X-Vercel-Cache: MISS`; o scan de runtime encontrou zero erro nas rotas de checkout na janela validada. Nenhuma Session ou cobrança foi criada no canário. A elegibilidade de uma pessoa real com Pilot interrompido está **IMPLEMENTADA / TESTADA LOCALMENTE** e aguarda retorno orgânico; não foi fabricada compra para promovê-la artificialmente.

**NÃO TOCADO:** preço, desconto, cupom, crédito, trial, SKU, promessa, assinatura existente, Stripe webhook/métodos, Supabase schema/dados, bulk, render, motor, cena, voz, legenda, admin, e-mail, outreach, anúncio ou contato externo.

**PRÓXIMO DONO:** Claude deve executar `git fetch origin` e partir de `7164de38` ou da ponta posterior. Não reeditar Pilot resume, subscription resume ou bulk antes dos gates. A próxima rodada do Codex volta a B2C em superfície distinta e continua subordinada à regra do board: cada ação deve mostrar curto, médio e longo prazo até assinatura real.

## 109. B2C — a oferta de boas-vindas deixa de se repetir no retorno do checkout (01/09/2026)

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** nas 72 horas observadas, 43 pessoas externas tiveram `video_ready_viewed`, 22 baixaram vídeo, 12 chegaram a pricing, seis iniciaram checkout e uma apresentou `payment_success`. Em 30 dias, 372 pessoas externas tiveram a primeira entrega; 58 chegaram a pricing depois dela, 47 iniciaram checkout e quatro pagaram. Quatorze pessoas que iniciaram checkout voltaram a pricing em até 24 horas sem pagamento. As variantes imediatamente anteriores `topup_eligibility_handoff_v1` e `one_time_pilot_resume_v1` continuavam com zero pessoas externas expostas; por isso foram preservadas e esta rodada atuou em outra superfície.

**FATO CONFIRMADO / LACUNA ANTERIOR:** `components/WelcomeOfferModal.tsx` prometia uma frequência de 72 horas, mas gravava `kineo_welcome20_seen` somente em `dismiss()`. O clique em Creator ou Studio apenas marcava o estado pendente e navegava ao checkout. Assim, uma pessoa que visse a oferta, clicasse, saísse da Stripe e retornasse sem fechar o modal podia receber a mesma interrupção outra vez. A oferta estava montada na home, em pricing e no dashboard, e não possuía telemetria própria de exposição, dismiss ou clique.

**HIPÓTESE CAUSAL NOVA:** iniciar a janela quando a oferta realmente fica visível, e não apenas quando é fechada, evita pressão repetida sobre quem já demonstrou intenção e preserva o retorno ao pricing ou à retomada de checkout. A intervenção corrige a verdade da frequência; não atribui o abandono inteiro a esse fator sem amostra.

**IMPLEMENTADO:** o commit funcional `0761a9876b03590b1ce0dff7e3e497513bd5a5f8` publica `welcome_offer_frequency_truth_v1`. A primeira exposição continua usando a oferta, desconto, copy, planos, cupom, delay e destino existentes. Imediatamente antes de abrir, o caller grava a mesma chave histórica em memória, `localStorage` e `sessionStorage`; outro mount, aba ou retorno dentro de 72 horas não abre novamente. Aba em segundo plano só inicia a frequência quando volta a ficar visível. Timestamp inválido ou muito adiante falha aberto; indisponibilidade de storage nunca repete a oferta na mesma página. Plano e identidade são lidos em paralelo, removendo uma espera de rede sem acrescentar requests.

**MEDIÇÃO / PRIVACIDADE:** `welcome_offer_viewed`, `welcome_offer_dismissed` e `welcome_offer_checkout_clicked` entram somente no host canônico e carregam versão, superfície allow-listed e, no clique, tier categórico. Não entram URL, referrer, e-mail, nome, preço, cupom, timestamp da frequência, sessão Stripe ou identificador novo. A medição não chama impressão de pessoa ou clique de receita.

**CADEIA OBRIGATÓRIA ATÉ ASSINATURA:** curto prazo = uma pessoa exposta vê a oferta uma vez e, se retornar, encontra pricing/retomada sem outra interrupção; médio prazo = mais retorno limpo vira `checkout_started` e `payment_success`; longo prazo = pagamentos se tornam assinatura ativa e renovação. A regra geral do board permanece: toda ação deve declarar essa cadeia, e assinatura real — não visita, modal, clique ou Session — é o placar.

**GATE / PARADA:** preservar até dez pessoas externas expostas. Sinal inicial: duas pessoas distintas clicam a oferta e pelo menos uma chega a `checkout_started`; `payment_success` é a métrica final. Parar imediatamente se a primeira exposição desaparecer, plano/cupom/destino mudar, a oferta repetir dentro de 72 horas no mesmo navegador ou uma pessoa paga receber a oferta. Não reeditar as três superfícies antes do gate.

**COMPARAÇÃO VISUAL:** `docs/previews/WELCOME-OFFER-FREQUENCY-TRUTH-V1-2026-09-01.html` mostra antes/depois desktop e mobile. O Chrome conectado confirmou a mesma oferta aprovada na primeira exposição e o retorno sem uma segunda interrupção; a versão mobile esconde corretamente os quadros desktop e não apresenta overflow. A aba e o servidor temporário foram fechados.

**TESTADO LOCALMENTE:** frequência 34/34; verdade monetária 306/306; Studio mobile 8/8; pricing saved-checkout 47/47; janela de Session 25/25 — 420 verificações. O typecheck repetiu exatamente os quatro erros baseline preexistentes em `mrr.ts`, `me/subscription` e `stripe/checkout` ×2; nenhum erro novo. `git -c core.whitespace=cr-at-eol diff --check` ficou limpo. A revisão React levou as leituras independentes de plano e identidade para `Promise.all`, sem request, listener ou efeito adicional.

**VALIDADO EM PRODUÇÃO COM FRONTEIRA EXPLÍCITA (01/09/2026 04:53 UTC):** `origin/main` avançou por fast-forward para o SHA funcional exato. O deploy Vercel `dpl_5pCyrpsgE7QExiZFGfuhWn5Liojh` chegou a `READY`, target production, framework Next.js, aliasado em `www.usekineo.com` e sem erro de alias. A varredura de runtime encontrou zero erro na janela de dez minutos. Nenhum clique, Checkout Session ou pagamento foi fabricado no canário; a frequência real por pessoa externa permanece **IMPLEMENTADA / TESTADA LOCALMENTE** até uso orgânico.

**NÃO TOCADO:** preço, desconto, cupom, crédito, validade, trial, SKU, promessa, Stripe server/configuração, método de pagamento, Supabase schema/dados, render, motor, cena, voz, legenda, admin, e-mail, outreach, anúncio ou contato externo.

**PRÓXIMO DONO:** Claude deve executar `git fetch origin` e partir de `0761a987` ou da ponta posterior. Não reeditar WelcomeOfferModal, home, pricing ou dashboard nesta hipótese antes de dez exposições externas ou de um sinal de parada. Codex alterna a próxima sprint para B2B em superfície distinta, mantendo a regra do board como critério de entrada e de sucesso.

## 110. B2B — rodada de contenção: cadastro não é o gargalo observado (01/09/2026)

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** nos 30 dias observados, nenhum perfil externo foi atribuído às campanhas `weekly_business_video_plan`, `weekly_business_video_plan_share_v1`, `client_short_brief_v1`, `client_short_brief_share_v1`, `agency_margin_proposal_v1` ou `affiliate_business_plan`. Na janela separada de sete dias, uma única sessão anônima produziu `agency_bulk_page_viewed`, `agency_margin_calculator_viewed` e `b2b_brief_viewed`; não houve ator em seleção de pack, submissão de brief, checkout bulk ou pagamento B2B. Uma sessão repetida em três etapas continua sendo uma sessão, não três pessoas.

**FATO CONFIRMADO / HIPÓTESE DESCARTADA:** o cadastro já preserva o prompt B2B e mostra um preview limitado do trabalho em `app/(auth)/signup/page.tsx` por `lib/growth/signupCreationPreview.ts`. A campanha e o destino também atravessam a autenticação por `activationRedirectFromSearch()`. A classificação `organic_signup_truth_v1` não inclui as campanhas B2B, portanto há uma lacuna de mensuração específica; contudo, a consulta não encontrou pessoa B2B chegando ao cadastro. Trocar a copy ou adicionar outro card nessa tela agora otimizaria uma etapa sem público observado e reeditaria uma prova de continuidade já publicada.

**DECISÃO OPERACIONAL:** nenhuma linha de produto foi alterada nesta rodada. `/youtube-automation`, `/partners`, página de agências, planejador empresarial, gerador de brief, case study Autopilot, Pilot resume e as pontes B2B recentes permanecem sob seus gates próprios. A sugestão de mudar novamente `/youtube-automation` foi recusada porque a página teve uma sessão anônima nos 14 dias consultados, anterior à variante publicada em 28/08; ainda não existe amostra da versão atual.

**CADEIA OBRIGATÓRIA ATÉ ASSINATURA:** curto prazo = conseguir visita B2B qualificada nas portas já existentes e observar avanço a brief, proposta ou pricing; médio prazo = a mesma pessoa alcançar checkout e `payment_success`; longo prazo = o comprador avulso ou empresa ativada migrar para Autopilot mensal, permanecer assinante e indicar outra empresa. Sessão, formulário visto, brief gerado, clique e Checkout Session não são assinatura.
...
## 113. B2C — primeiro pagamento real do contexto de valor v3; checkout preservado (01/09/2026)

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** nos 30 dias observados, 94 pessoas externas criaram ao menos uma Checkout Session; seis apresentaram `payment_success`, 47 tiveram sessão expirada, 13 retornaram explicitamente por cancelamento, 16 criaram uma nova Session depois da primeira e 31 voltaram a pricing. Na janela de sete dias, 15 pessoas externas criaram Session e uma pagou. Uma única pessoa externa em 30 dias gerou `checkout_failed`, em 28/08, antes de qualquer `checkout_started`; não apareceu falha técnica nova depois de Session criada. Session criada continua não sendo tratada como prova de que a página hospedada foi carregada.

**EVIDÊNCIA DE PRODUÇÃO / PRIMEIRO SINAL DA VARIANTE:** `checkout_value_context_v3` apareceu em uma única pessoa externa até a leitura. Essa pessoa criou uma Session Studio mensal e emitiu `payment_success` 33 segundos depois. A reconciliação atual do perfil confirmou, sem expor identidade: plano Studio, `is_pro=true`, `has_paid=true`, Customer Stripe vinculado e Subscription Stripe vinculada. É uma assinatura real; com uma pessoa, ainda não existe base para atribuir causalidade à copy da variante.

**DECISÃO OPERACIONAL / ANTI-DUPLICAÇÃO:** nenhuma tela ou rota de checkout foi reeditada nesta rodada. O contexto v3, o Plan Fit v3, a prova de jornada em pricing, a frequência verdadeira da oferta de boas-vindas, o top-up elegível e as retomadas de checkout permanecem sob seus gates próprios. O primeiro sinal financeiro do v3 é razão para preservar a experiência e acumular amostra, não para empilhar outra mudança no mesmo último metro.

**CADEIA OBRIGATÓRIA ATÉ ASSINATURA:** curto prazo = a pessoa entende dentro da Stripe quantos resultados o plano entrega e conclui o pagamento; médio prazo = `payment_success` reconcilia com Subscription e entitlement corretos; longo prazo = a assinatura permanece ativa, renova e pode gerar indicação. Nesta rodada o placar avançou em uma assinatura externa real, mas renovação permanece desconhecida.

**GATE / PARADA:** preservar `checkout_value_context_v3` até pelo menos dez pessoas externas com `checkout_started` versionado. Medir por pessoa `checkout_started(v3) → payment_success(v3) → Subscription vinculada`; separar tier e billing sem comparar percentuais antes da amostra. Parar imediatamente por divergência entre preço, grant, descrição, pagamento e entitlement, ou por nova falha técnica terminal. Não chamar 1/1 de taxa causal nem otimizar a variante com base em uma pessoa.
...
## 115. B2C — fim do trial deixa de presumir que todo comprador quer Creator (01/09/2026)

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** nos sete dias observados, 19 pessoas externas viram o modal de fim do trial, 13 o dispensaram e nenhuma clicou o CTA direto de Creator. Na janela separada de 30 dias, 56 pessoas externas viram o modal, 10 clicaram Creator e 41 dispensaram. Entre as 46 pessoas que não clicaram o CTA do modal, duas depois visitaram pricing, iniciaram checkout e pagaram Studio; a sequência é observada, não prova causal. Entre as dez que clicaram o CTA Creator, uma pagou Creator. Evento, impressão e sessão não foram chamados de pessoa ou receita.

**HIPÓTESE CAUSAL NOVA / ANTI-DUPLICAÇÃO:** o modal oferecia somente Creator ou a permanência no free, embora a coorte real também contenha compradores de Studio. A hipótese é que parte da recusa seja necessidade de comparar capacidade e motores antes de escolher, não rejeição definitiva da assinatura. O CTA Creator continua primário, a escolha free continua disponível e nenhum experimento de checkout, Plan Fit, pricing, WelcomeOfferModal ou pós-vídeo foi reeditado.

**IMPLEMENTADO:** o commit funcional `c8827a012e0cbbbc1edd149e60d8ca976f60779e` adiciona a ação secundária `Compare all plans →` abaixo de Creator e antes da escolha free. O destino é `/pricing?intent_campaign=trial_downgrade_plan_choice_v1#plans`, sem UTM e sem criar Checkout Session. A comparação emite `trial_downgrade_compare_plans_clicked` com metadata finita e adia reversivelmente o modal; não grava `stay_free`, não consome o último adiamento e não reabre a interrupção na navegação seguinte.

**CADEIA OBRIGATÓRIA ATÉ ASSINATURA:** curto prazo = pessoa com trial encerrado entende que pode comparar Starter, Creator e Studio antes de decidir; médio prazo = comparação vira escolha de plano, `checkout_started` e `payment_success`; longo prazo = pagamento reconciliado vira assinatura ativa, renovação e possível indicação. Modal visto, comparação, pricing e Checkout Session não são receita.

**MEDIÇÃO / GATE:** preservar `trial_downgrade_plan_choice_v1` até 20 pessoas externas expostas depois do deploy. Sinal inicial: pelo menos duas pessoas distintas clicam comparar e uma chega a `checkout_started`; `payment_success` e assinatura vinculada são o resultado final. Parar imediatamente se o CTA Creator perder sua primazia, a escolha free quebrar, o modal reaparecer logo após a comparação, campanha/destino divergirem ou uma conta paga receber a experiência.

**COMPARAÇÃO VISUAL / TESTADO LOCALMENTE:** `docs/previews/TRIAL-DOWNGRADE-PLAN-CHOICE-V1-2026-09-01.html` mostra antes/depois em desktop e mobile e foi inspecionado no Chrome conectado sem overflow. Contrato específico 39/39; verdade monetária 306/306; atribuição da escolha em pricing 26/26; oferta pós-vídeo 57/57 — 428 verificações. `git -c core.whitespace=cr-at-eol diff --check` ficou limpo. O typecheck repetiu exatamente os quatro erros baseline preexistentes em `mrr.ts`, `me/subscription` e `stripe/checkout` ×2; nenhum erro novo.

**VALIDADO EM PRODUÇÃO COM FRONTEIRA EXPLÍCITA (01/09/2026):** `origin/main` avançou por fast-forward para o SHA funcional exato. O status `Vercel` do commit chegou a `success` às `2026-09-01T06:37:33Z` com a descrição `Deployment has completed`. No Chrome conectado, o destino atribuído abriu o grid `#plans`, preservou a campanha e apresentou Starter, Creator e Studio sem overflow ou erro de console. A conta autenticada do fundador é paga, portanto o modal de trial encerrado não foi forçado; impressão, clique e conversão orgânicos da variante permanecem **IMPLEMENTADOS / TESTADOS LOCALMENTE / AGUARDANDO AMOSTRA EXTERNA**.

**NÃO TOCADO:** preço, desconto, cupom, crédito, validade, trial, SKU, promessa, Stripe server/configuração, método de pagamento, Supabase schema/dados, render, motor, cena, voz, legenda, admin, e-mail, outreach, anúncio ou contato externo.

**PRÓXIMO DONO:** Claude deve executar `git fetch origin` e partir de `c8827a01` ou da ponta posterior. Não reeditar o modal de fim do trial antes de 20 exposições externas ou de um sinal de parada. A próxima rodada do Codex alterna para B2B em superfície distinta; a regra geral do board permanece curto prazo → checkout/pagamento → assinatura ativa/renovação.

## 116. B2B — comprador mensal do Autopilot chega ao setup correto (01/09/2026)

**FATO CONFIRMADO / LACUNA ANTERIOR:** a assinatura mensal do Autopilot criava sua Checkout Session com o retorno genérico `/checkout/success`. Essa página preservava os pixels e aguardava créditos, mas sempre mostrava sugestões de criação self-serve e redirecionava para `/generate` depois de 15 segundos. O Pilot avulso já retornava diretamente a `/autopilot`. A página `/autopilot` lê o entitlement uma vez; portanto, redirecionar antes de o webhook estabelecer `plan=autopilot` poderia mostrar `not_entitled` imediatamente depois de um pagamento correto.

**EVIDÊNCIA DE PRODUÇÃO / FRONTEIRA:** o histórico observado não contém `payment_success` externo do Autopilot mensal. A reconciliação anterior encontrou 11 pessoas externas com checkout Autopilot em 30 dias — quatro Pilot e sete mensais — e todos expiraram. Assim, esta entrega corrige um defeito terminal confirmado em código, mas não atribui a ele os abandonos nem inventa comprador afetado.

**HIPÓTESE CAUSAL NOVA:** depois de pagar pelo serviço gerenciado, ver tópicos self-serve e cair no gerador comum reduz a percepção de que a assinatura comprada está ativa. Levar o comprador ao setup prometido somente depois da confirmação autoritativa do plano reduz essa contradição sem desconto, nova promessa ou nova oferta.

**IMPLEMENTADO:** o commit funcional `0984907bef133d62bb5de400c06ad108755c2650` publica `autopilot_checkout_success_v1`. Somente `tier=autopilot` seleciona o fluxo visual Autopilot; Starter, Creator e Studio preservam byte a byte o retorno anterior. A página mantém `checkout_success_viewed` e os pixels existentes uma única vez, substitui os tópicos self-serve por preparação de canal, nicho, agenda e visibilidade e consulta o campo autoritativo `plan` já retornado por `/api/credits`. O contador visual termina em 15 segundos, mas o redirecionamento para `/autopilot` só acontece quando `plan==='autopilot'`; se o webhook ainda não liquidou o entitlement, a página permanece honesta, continua consultando por até 45 segundos e oferece nova verificação. Session ID aceita formato limitado e só atravessa o redirect do próprio Stripe; não entra nos novos eventos.

**MEDIÇÃO / PRIVACIDADE:** a cadeia por pessoa externa é `payment_success(tier=autopilot) → checkout_success_viewed(intended_tier=autopilot) → autopilot_checkout_handoff_ready → autopilot_page_arrived → autopilot_page_viewed(no_channel|no_schedule|schedule_active) → conexão/agendamento`. `autopilot_checkout_handoff_pending`, `ready` e `clicked` carregam apenas versão e estado categórico. URL, Session ID, preço, valor, e-mail e texto livre não entram. View, pending, ready, clique e chegada não são receita.

**CADEIA OBRIGATÓRIA ATÉ ASSINATURA:** curto prazo = pagamento liquidado leva ao setup correto, sem gerador comum nem falso paywall; médio prazo = comprador conecta o canal e cria a primeira agenda; longo prazo = serviço diário entregue sustenta assinatura ativa, renovação e indicação. Esta mesma regra passa a governar o board geral: toda tarefa deve declarar seu caminho de curto, médio e longo prazo até assinatura real; tarefa sem conexão mensurável com esse objetivo não entra na fila.

**GATE / PARADA:** no primeiro pagamento externo mensal, exigir `payment_success → checkout_success_viewed → handoff_ready → autopilot_page_arrived` sem `not_entitled`; preservar até cinco pessoas externas. Parar imediatamente se houver pixel duplicado, redirect antes de `plan=autopilot`, tópicos self-serve no fluxo mensal, Session/tier divergente ou regressão do sucesso de Starter/Creator/Studio. Não reeditar esta superfície antes do gate mínimo.

**COMPARAÇÃO VISUAL:** `docs/previews/AUTOPILOT-CHECKOUT-SUCCESS-V1-2026-09-01.html` mostra antes/depois desktop e mobile. O Chrome conectado confirmou os quatro quadros, sem overflow: antes com tópicos e destino genérico; depois com preparação do serviço e confirmação de acesso. A aba e o servidor temporários foram fechados; somente a aba Stripe já pendente permaneceu aberta.

**TESTADO LOCALMENTE:** contrato específico 84/84; verdade monetária 306/306; checkout assíncrono 31/31; verdade de falha Stripe 67/67; contexto de valor 59/59; retorno Autopilot 36/36; janela de Session 25/25; orientação de pagamento 29/29; bulk truth 63/63; break-even Autopilot 40/40; retorno de agência 75/75. O typecheck repetiu exatamente os quatro erros baseline preexistentes em `mrr.ts`, `me/subscription` e `stripe/checkout` ×2; nenhum erro novo. O teste legado `test-autopilot-pilot-resume.mjs` continua com uma asserção frágil a CRLF do Windows já presente em `origin/main`; o código coberto não foi alterado para maquiar o teste. `git -c core.whitespace=cr-at-eol diff --check` ficou limpo.

**VALIDADO EM PRODUÇÃO COM FRONTEIRA EXPLÍCITA (01/09/2026 UTC):** `origin/main` avançou por fast-forward para `0984907bef133d62bb5de400c06ad108755c2650`. O status `Vercel` do SHA chegou a `success` com `Deployment has completed` às `2026-09-01T07:11:29Z`. Uma leitura HTTP sem executar JavaScript confirmou `200 OK`, `X-Matched-Path: /checkout/success`, `Server: Vercel` e resposta canônica em `www.usekineo.com`; nenhum pixel, evento, Session, pagamento ou render foi fabricado. O comportamento de uma pessoa externa paga permanece **IMPLEMENTADO / TESTADO LOCALMENTE / AGUARDANDO AMOSTRA EXTERNA**.

**NÃO TOCADO:** webhook e fulfillment Stripe, preço, desconto, cupom, crédito, trial, SKU, promessa, método de pagamento, Supabase schema/dados, dashboard do Autopilot, render, motor, cena, voz, legenda, admin, e-mail, outreach, anúncio ou contato externo.

**PRÓXIMO DONO:** Claude deve executar `git fetch origin` e partir de `0984907b` ou da ponta posterior. Não reeditar o sucesso do checkout Autopilot antes do gate. Codex alterna a próxima rodada para B2C em superfície distinta e reconcilia primeiro a amostra da rodada 115. A regra comum do board é: curto prazo → pagamento; médio prazo → ativação; longo prazo → assinatura ativa, renovação e indicação.

## 117. B2C/AEO — o catálogo que o ChatGPT lê aponta para cada motor (01/09/2026)

**DECISÃO APROVADA / REGRA PERMANENTE DO BOARD:** toda tarefa geral precisa declarar o caminho até mais assinaturas no curto, médio e longo prazo. Curto prazo = avanço qualificado até pagamento; médio prazo = `payment_success` reconciliado e assinatura ativa; longo prazo = renovação, indicação ou expansão de receita recorrente. B2C, B2B, afiliados, AEO/SEO, ativação e checkout compartilham esse placar. Clique, visita, cadastro, vídeo ou Checkout Session continuam sendo etapas, não receita. Cada tarefa também exige métrica por pessoa externa, gate de sucesso e gate de parada.

**EVIDÊNCIA DE PRODUÇÃO (SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** em 30 dias, 374 pessoas externas chegaram a `video_ready_viewed`, 59 dessas chegaram depois a pricing, 47 a checkout e quatro a pagamento. Na coorte cujo primeiro referrer é ChatGPT, os caminhos por motor já trazem pessoas qualificadas: `/ai-video-generator/kineo-1` teve 16 pessoas identificadas, 11 com vídeo e uma com checkout; `/ai-video-generator/seedance` teve nove pessoas, quatro com vídeo, uma com checkout e uma com pagamento. A amostra de Seedance é uma pessoa paga e não prova causalidade nem lift.

**FATO CONFIRMADO / LACUNA ANTERIOR:** `/llms.txt` enumerava os motores, mas não ligava cada nome à sua página canônica; o catálogo JSON de `/api/facts` também não publicava URL por motor. Answer engines podiam conhecer o catálogo e ainda encaminhar a pessoa à home genérica.

**HIPÓTESE CAUSAL NOVA:** dar a answer engines uma URL canônica por intenção reduz a perda entre a pergunta específica (“Seedance”, “Kling”, “Veo”, “AI avatar”) e a prova pública do motor correspondente. Isso pode elevar a qualidade de chegada e o caminho página do motor → cadastro → primeiro vídeo → checkout → pagamento; o efeito sobre assinatura permanece hipótese a medir.

**IMPLEMENTADO:** o commit funcional `6bdce4d6fcea03685c2ec24ad2c814090fbba15a` cria uma fonte única para os sete destinos de vídeo, adiciona a URL canônica aos oito `EngineFact` (sete motores de vídeo + Avatar), publica o campo aditivo em `/api/facts` e transforma os oito nomes do catálogo de `/llms.txt` em links Markdown diretos. Nenhum preço, crédito, oferta, promessa ou entitlement mudou.

**CADEIA OBRIGATÓRIA ATÉ ASSINATURA:** curto prazo = a pessoa vinda do ChatGPT chega ao motor exato e entende o produto antes do cadastro; médio prazo = chegada identificada vira primeiro vídeo, checkout e `payment_success`; longo prazo = pagamento reconciliado vira assinatura ativa, renovação e possível indicação. A ação não será avaliada por quantidade de links nem por pageview isolado.

**MEDIÇÃO / GATE:** cadeia por pessoa externa: `landing_session_started(referrer_host=chatgpt, path=/ai-video-generator/...|/ai-avatar) → pessoa identificada → video_ready_viewed → checkout_started → payment_success → assinatura ativa`. Sessões anônimas e pessoas identificadas permanecem separadas. Preservar até pelo menos 20 pessoas externas identificadas vindas do ChatGPT nas páginas de motor após a fronteira do deploy, com tráfego em pelo menos dois destinos. Sinal inicial = vídeo e checkout; resultado final = pagamento e assinatura ativa. Parar se qualquer URL responder diferente de 200, canonical divergir, catálogo ficar desatualizado ou a atribuição específica for sobrescrita por campanha genérica.

**EXPERIMENTOS PRESERVADOS:** `pricing_journey_proof_v1` segue abaixo do gate (quatro pessoas `after_delivery`, duas `before_first_delivery`); `resume_smaller_choice_v1` ainda não tem exposição versionada; `trial_downgrade_plan_choice_v1` ainda não tem exposição versionada válida; `chatgpt_quickstart_v5` segue com 30 pessoas expostas, uma com checkout e pagamento posterior. Nenhuma dessas superfícies foi reeditada nesta rodada.

**TESTADO LOCALMENTE / AUDITORIA:** contrato AEO 65/65; acesso trial 59/59; intenção das páginas de motor 126/126; destino pós-cadastro 109/109. A auditoria adversarial encontrou dois falsos gates no primeiro teste, bloqueou o push, e a versão final passou a verificar os sete slugs/parâmetros reais e a canonical de `/ai-avatar`; parecer final **GO**. `diff --check` limpo. Typecheck com exatamente os quatro erros baseline preexistentes em `mrr.ts`, `me/subscription` e `stripe/checkout` ×2; nenhum erro novo. Não houve comparação visual porque a mudança é exclusivamente texto/JSON legível por máquina.

**VALIDADO EM PRODUÇÃO (01/09/2026 UTC):** `origin/main` avançou por fast-forward para `6bdce4d6fcea03685c2ec24ad2c814090fbba15a`. O status Vercel chegou a `success` (`Deployment has completed`) às `2026-09-01T07:33:43Z`. Em produção, `/llms.txt` respondeu 200 com oito links; `/api/facts` respondeu com oito motores, oito URLs e oito URLs únicas; os oito destinos canônicos responderam 200. Nenhum IndexNow, recrawl, anúncio, e-mail, outreach, render, pagamento ou evento foi forçado.

**NÃO TOCADO / COORDENAÇÃO:** preço, desconto, cupom, crédito, validade, trial, SKU, promessa, checkout Stripe, banco, render, motor, cena, voz, legenda, admin ou comunicação externa. A falta de medição dos novos CTAs em `/account` pertence à pista do Claude (`app/(dashboard)/account/**`) e foi apenas registrada, não corrigida pelo Codex. Claude deve executar `git fetch origin` e partir de `6bdce4d6` ou da ponta posterior. A próxima rodada do Codex alterna para B2B em superfície distinta.

## 118. B2B — tom Business do Publishing Kit ganha uma continuação empresarial (01/09/2026)

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO ANTI-DUPLICAÇÃO (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** em sete dias, a ponte de volume de agência teve duas pessoas externas identificadas na entrada de pricing e duas sessões anônimas na entrada do relatório; houve uma única sessão anônima em `agency_bulk_page_viewed`, `agency_margin_calculator_viewed` e `b2b_brief_viewed`, sem pack escolhido, brief enviado, bulk checkout ou pagamento. Autopilot, proposta/margem, bulk, pricing business path, break-even e Pilot resume permanecem abaixo de seus gates. Essas superfícies não foram reeditadas.

**FATO CONFIRMADO / LACUNA ANTERIOR:** o Publishing Kit público já permitia gerar títulos, descrição e hashtags com `tone=business`, mas o resultado oferecia somente o CTA creator para transformar um tópico em um único Short. A escolha Business não tinha continuação para planejamento, proposta ou volume, nem medição própria (`app/youtube-shorts-title-generator/PublishKitClient.tsx`, versão anterior a `4780e281`; `lib/growth/shortsPublishKit.ts`). O planejador empresarial já existia, era gratuito e já media `viewed → generated → activation/packs clicked`; não foi reconstruído (`app/business-video-content-plan/BusinessContentPlanClient.tsx`).

**HIPÓTESE CAUSAL NOVA:** depois de receber valor no artefato gratuito, uma pessoa que escolheu linguagem Business pode estar planejando uma sequência para uma empresa, não apenas uma peça creator. Manter o CTA de vídeo como principal e oferecer uma continuação empresarial secundária pode transformar descoberta em plano semanal e depois em escolha de produção, checkout e pagamento. `tone=business` ainda pode significar somente estilo; por isso a ponte não substitui nem enfraquece o caminho creator.

**IMPLEMENTADO:** o commit funcional `4780e28144e855ae6f69df153acdae1c276c84f2` preserva `Turn this topic into a Short` como CTA primário e acrescenta, somente depois de um kit realmente gerado com tom Business, `Planning a business content batch? Build the free weekly plan` para `/business-video-content-plan`. Curiosity, Clear explainer e Story não recebem a ponte. O tom usado fica congelado com o artefato, então mudar o seletor depois não relabela um resultado antigo (`app/youtube-shorts-title-generator/PublishKitClient.tsx`; `lib/growth/publishKitBusinessPath.ts`).

**TELEMETRIA / PRIVACIDADE:** `publish_kit_business_path_viewed` exige pelo menos 50% do card no viewport; `stored:false` libera nova tentativa e somente `stored:true` fecha o latch. `publish_kit_business_creator_clicked` registra `post_value_primary → creator_signup`; `publish_kit_business_path_clicked` registra `post_value_secondary → business_video_content_plan`. Cada evento novo acrescenta quatro campos específicos (`version`, `surface`, `placement`, `destination`) aos campos globais de atribuição já herdados por `trackEvent` (`utm_source`, `utm_medium`, `utm_campaign`, `utm_content`, `utm_term`, `gclid`, `ref`). Tópico, takeaway, título, descrição, hashtag, URL individual e e-mail não entram nos callers novos.

**CADEIA OBRIGATÓRIA ATÉ ASSINATURA:** curto prazo = uma sessão que escolheu Business recebe valor, escolhe creator ou planner e, no caminho empresarial, gera um plano; médio prazo = a mesma pessoa avança a pack/plano, checkout e `payment_success`; longo prazo = o comprador empresarial comprova uso recorrente, migra ou permanece em assinatura ativa, renova e pode indicar outra empresa. View, kit, clique, plano gerado e Checkout Session não são receita.

**MEDIÇÃO / GATE:** manter `publish_kit_business_path_v1` até 20 sessões anônimas distintas realmente expostas ou cinco pessoas externas identificadas. Sinal inicial = ao menos duas sessões distintas clicam a ponte e uma chega a `business_content_plan_generated`; resultado financeiro = `bulk_purchase_completed`/`payment_success`, reconciliado com pessoa e produto; resultado final = assinatura ativa. Parar com 20 exposições e zero clique, queda material do CTA creator no grupo Business, destino/campanha divergente, conteúdo do usuário em telemetria ou qualquer regressão dos outros três tons. Sessões anônimas e pessoas identificadas permanecem réguas separadas.

**TESTADO LOCALMENTE / AUDITORIA:** teste específico 98/98 e regressão do planejador 171/171; `git -c core.whitespace=cr-at-eol diff --check` limpo. O typecheck repetiu exatamente os quatro erros baseline em `mrr.ts`, `me/subscription` e `stripe/checkout` ×2; nenhum erro novo. No Chrome conectado, o caminho Business mostrou os dois CTAs com destinos corretos, Story manteve somente o CTA creator, o console ficou sem erro e o viewport real de 390 px teve `scrollWidth=390` com ambos os cards dentro da tela. A comparação antes/depois desktop e mobile está em `docs/previews/PUBLISH-KIT-BUSINESS-PATH-V1-2026-09-01.html`. A aba e os servidores temporários foram fechados.

**AUDITORIA ADVERSARIAL:** o primeiro parecer foi NO-GO porque o clique creator usava metadata do planner, uma falha de armazenamento fechava cedo o latch e o teste chamava quatro campos específicos de payload total. Os três contratos foram corrigidos. Uma segunda âncora falsamente verde, encontrada pelo próprio Codex depois do GO, também foi eliminada; a reauditoria final deu GO presa ao SHA `4780e281`, com 98/98.

**VALIDADO EM PRODUÇÃO (01/09/2026 UTC):** `origin/main` avançou por fast-forward para `4780e28144e855ae6f69df153acdae1c276c84f2`. O deploy Vercel `dpl_CJRKfPVf5iGC3w3joZcYc1SmcwQF` chegou a `READY`, alvo production, com aliases `www.usekineo.com` e `usekineo.com`. `/youtube-shorts-title-generator` respondeu 200 com `X-Matched-Path` correto; o chunk publicado `page-83e287de58042149.js` contém a condição Business, os dois destinos e os três eventos; `/business-video-content-plan` respondeu 200. Nenhum formulário, evento de produção, pagamento, render, IndexNow, recrawl ou contato foi forçado. O scan de runtime encontrou somente o aviso antigo de `url.parse()` em `/api/generate-broll-plan`, `/api/generate-script` e `/api/compose`, fora desta entrega; nenhuma falha da superfície publicada.

**NÃO TOCADO / COORDENAÇÃO:** preço, desconto, cupom, crédito, validade, trial, SKU, promessa, checkout Stripe, Supabase schema/dados, dashboard autenticado, render, motor, cena, voz, legenda, admin, e-mail, outreach, anúncio ou comunicação externa. Claude deve executar `git fetch origin` e partir do SHA da documentação desta seção ou da ponta posterior. Não reeditar Publishing Kit Business, planejador, proposta, bulk ou Autopilot antes de seus gates. A próxima rodada do Codex volta a B2C em superfície distinta, mantendo assinatura real como placar do board.

## 119. B2C — fallback degradado deixa de chamar checkout não confirmado de “pronto” (01/09/2026)

**EVIDÊNCIA DE PRODUÇÃO / BASELINE (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** em 60 dias, uma pessoa externa distinta recebeu duas impressões de `checkout_fallback_shown(fallback_kind=idempotent_retry)`. Não houve clique no fallback, novo `checkout_started` nem `payment_success` dessa pessoa nas 24 horas seguintes à primeira impressão. O alcance é pequeno e não prova que a copy causou o abandono; prova somente que o estado degradado chegou a uma pessoa real e terminou sem avanço observado.

**RECONCILIAÇÃO ANTI-DUPLICAÇÃO:** `pricing_journey_proof_v1`, `resume_smaller_choice_v1`, `trial_downgrade_plan_choice_v1`, `checkout_value_context_v3`, `welcome_offer_frequency_truth_v1`, `topup_eligibility_handoff_v1`, a oferta pós-vídeo e os destinos AEO continuam abaixo de seus gates ou em amostra protegida. Nenhuma dessas superfícies foi reeditada. Este trabalho atua apenas no card global imediato que nasce quando a abertura do checkout excede o watchdog.

**FATO CONFIRMADO / CONTRADIÇÃO ANTERIOR:** `lib/checkoutTelemetry.ts` já distinguia três estados: URL direta da Stripe (`stripe_direct`), endpoint de retomada de Session viva (`resume_endpoint`) e repetição idempotente da rota que acabou de travar (`idempotent_retry`). `components/CheckoutStalledCta.tsx` ignorava essa distinção e dizia `Your checkout is ready` e `Your payment page is ready` nos três casos. No estado degradado, o código possuía apenas uma rota segura para tentar de novo; não havia confirmação de página Stripe pronta.

**HIPÓTESE CAUSAL NOVA:** no instante de maior intenção, afirmar que um checkout está pronto quando ele ainda precisa ser reaberto pode parecer erro persistente ou risco de cobrança duplicada. Dizer exatamente o que aconteceu e nomear a nova tentativa pode recuperar o segundo clique. O risco inverso é a copy mais cautelosa reduzir confiança; por isso a mudança é estritamente condicionada ao tipo degradado e não será chamada de lift antes da amostra.

**IMPLEMENTADO / PUBLICADO:** o commit funcional `6370fedb99c5aed9de3a3bb48e0c3fbbe1c62238` cria `checkout_fallback_truth_v1`. Somente `idempotent_retry` passa a mostrar `Checkout took too long to open`, `This attempt did not charge you` e `Try secure checkout again`. `stripe_direct` e `resume_endpoint` preservam exatamente `Your checkout is ready`, `Your payment page is ready` e `Continue to payment`. A âncora continua apontando para `stalled.url`, sem `preventDefault`, sem alteração em launcher, Session, idempotência ou código Stripe. No mobile até 520 px, mensagem e ação ocupam linhas separadas e o botão recebe a largura disponível; desktop preserva a composição anterior.

**TELEMETRIA / PRIVACIDADE:** `checkout_fallback_shown`, `checkout_fallback_clicked` e o novo `checkout_fallback_dismissed` recebem `version=checkout_fallback_truth_v1`. Clique e dismiss preservam `fallback_kind` no instante da ação e `shown_kind` no instante da impressão, de modo que uma promoção posterior do card não falsifique CTR por tipo. Os callers novos não acrescentam URL, Session ID, preço, e-mail, texto do usuário ou metadata livre; reaproveitam apenas superfície, seleção e classes categóricas já existentes.

**CADEIA OBRIGATÓRIA ATÉ ASSINATURA:** curto prazo = a pessoa entende que a primeira abertura travou e executa uma segunda tentativa consciente; médio prazo = o mesmo ator chega a `checkout_started` e `payment_success`; longo prazo = o pagamento reconciliado vira assinatura ativa e renovação. Impressão, dismiss, clique e Checkout Session continuam sendo etapas, não receita.

**MEDIÇÃO / GATE:** preservar a variante até dez pessoas externas realmente expostas, exigindo no mínimo três pessoas em `shown_kind=idempotent_retry`. Sinal inicial = ao menos duas dessas três clicarem novamente; avanço = `checkout_started` da mesma pessoa; resultado financeiro = `payment_success` reconciliado e assinatura vinculada. Parar imediatamente se a copy degradada aparecer em `stripe_direct` ou `resume_endpoint`, o `href` mudar, uma impressão duplicar, o tipo exibido divergir do registrado, tier/preço divergir ou a navegação nativa deixar de funcionar. Com dez exposições e zero clique no degradado, reavaliar a hipótese sem maquiar o gate.

**COMPARAÇÃO VISUAL:** `docs/previews/CHECKOUT-FALLBACK-TRUTH-V1-2026-09-01.html` mostra antes/depois desktop e 390 px. A primeira auditoria deu NO-GO porque o preview inventava uma borda âmbar ausente no produto; o artefato foi corrigido. No Chrome conectado, os quatro cards passaram a usar a mesma borda azul real, o quadro mobile posterior apresentou `flex-wrap: wrap`, `scrollWidth=clientWidth=350` e nenhum overflow. A aba e o servidor temporário foram fechados.

**TESTADO LOCALMENTE:** contrato específico 35/35; checkout window 25/25; pricing saved checkout 47/47; Stripe failure truth 67/67; bulk checkout truth 63/63. O typecheck executado pelo TypeScript local repetiu exatamente os quatro erros baseline preexistentes em `mrr.ts`, `me/subscription` e `stripe/checkout` ×2; nenhum erro novo. `git -c core.whitespace=cr-at-eol diff --check` ficou limpo. Uma primeira asserção falsa procurava `preventDefault` no arquivo inteiro e tropeçava no comentário histórico; o teste final extrai e inspeciona o bloco real da âncora, sem alterar o produto para agradar à regex.

**AUDITORIA ADVERSARIAL:** o primeiro parecer foi NO-GO por preview visual desonesto e gate ausente do pacote. Depois das duas correções, a reauditoria deu GO preso ao SHA funcional `6370fedb99c5aed9de3a3bb48e0c3fbbe1c62238` e ao diff documental desta seção, repetindo todas as suítes acima e confirmando que apenas `idempotent_retry` muda a copy.

**VALIDADO EM PRODUÇÃO (01/09/2026 UTC):** `origin/main` avançou por fast-forward até o commit documental `3d2c83de4c2d601461509d9a813ea9af6e08a053`, que contém o funcional acima. O deploy Vercel `dpl_2Fwk7DFmCgBBQw3hfiKKVTg251Qd` chegou a `READY`, alvo production, sem erro de alias e com `www.usekineo.com` e `usekineo.com`. `/pricing` respondeu HTTP 200 e o próprio HTML declarou esse deployment ID. A varredura dos 21 bundles públicos encontrou `checkout_fallback_truth_v1`, `Checkout took too long to open`, `Try secure checkout again` e a copy preservada `Your checkout is ready` no chunk global publicado. Não houve log `error` ou `fatal` nesse deploy na janela de duas horas. O agrupador global mostrou somente o aviso histórico de depreciação de `url.parse()` em rotas de geração e no deploy anterior, fora desta entrega. Nenhum checkout, evento, pagamento, render ou comunicação foi forçado para produzir a prova.

**NÃO TOCADO / COORDENAÇÃO:** preço, desconto, cupom, crédito, validade, trial, SKU, oferta, promessa comercial, launcher de checkout, Stripe server/webhook/métodos, Supabase schema/dados, render, motor, cena, voz, legenda, admin, e-mail, outreach, anúncio ou comunicação externa. Preservar `checkout_fallback_truth_v1` até o gate acima; a próxima rodada do Codex alterna para B2B em superfície distinta, mantendo assinatura real como placar do board.

## 120. B2B — a prova pública dos vídeos volta a oferecer o caminho de lote para clientes (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** esta tarefa foi admitida porque declara o caminho até assinatura no curto, médio e longo prazo. Curto prazo = prova vista → ponte clicada → pack comparado; médio prazo = checkout → `payment_success`; longo prazo = comprador empresarial recorrente → assinatura ativa, renovação ou indicação. View, clique, visita ao pack e Checkout Session não são receita.

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** a variante B2B da rodada 118 não recebeu nova pessoa externa nem sessão anônima versionada depois da publicação e permanece congelada até o gate. Na janela separada de 30 dias, `example_video_play` teve 17 sessões anônimas distintas e sete pessoas externas identificadas; `example_preview_ended`, 14 sessões anônimas e uma pessoa externa; o CTA do índice de exemplos teve duas sessões anônimas e duas pessoas externas no header, além de uma pessoa externa no footer. Plays de detalhe não foram chamados de exposição do índice. A superfície não possuía evento de impressão, portanto não existe denominador histórico do novo card.

**FATO CONFIRMADO / LACUNA ANTERIOR:** a página de agências envia compradores para `/examples` por `Watch real outputs` (`app/ai-shorts-for-agencies/page.tsx:143-145`), mas o índice terminava somente no card creator `Bring your own topic` e em `Create a Fast video`/`Open Studio` (`app/examples/page.tsx:148-163`). Não havia continuidade de volta aos packs. A oferta de 10–50 Shorts, uso comercial, MP4 limpo, conta única e ausência de contrato recorrente já existe no produto (`app/ai-shorts-for-agencies/page.tsx:14-20,48-53,134-137,197-201,229`; `app/ai-shorts-for-agencies/AgencyPacksClient.tsx:73-82,172-175`).

**HIPÓTESE CAUSAL NOVA:** uma agência ou empresa que saiu da página comercial para verificar os renders pode perder o contexto de compra ao chegar a um índice exclusivamente creator. Uma continuação secundária depois da prova pode recuperar essa pessoa para os packs sem desviar o visitante B2C do CTA primário.

**IMPLEMENTADO / PUBLICADO:** o commit funcional `30621adb82ed8dc3dfbd896602fa3bbffea2e283` cria `examples_business_proof_bridge_v1`. O card `For client work · Making Shorts for clients?` aparece somente depois do CTA creator e leva ao anchor real `/ai-shorts-for-agencies#agency-pack-heading` (`app/examples/page.tsx:148-167`; `lib/growth/examplesBusinessProof.ts:1-26`; `app/examples/ExamplesBusinessProofBridge.tsx:68-102`). O destino não recebe query nem UTM e, portanto, não sobrescreve first-touch. O caminho creator, seus textos e seus destinos permanecem intactos.

**TELEMETRIA / PRIVACIDADE:** `examples_business_proof_bridge_viewed` exige 50% do card visível; `examples_business_proof_bridge_clicked` mede a ação. Os dois carregam apenas `version`, `surface=examples_index`, `placement=after_creator_cta` e `destination=agency_packs`, além da atribuição global já herdada por `trackEvent`. Não entram e-mail, tópico, prompt, slug, vídeo, preço, Session ID ou URL individual. Dedupe usa request em voo, memória e `sessionStorage`, fechando somente quando `/api/events` devolve `stored:true` (`app/examples/ExamplesBusinessProofBridge.tsx:13-65`).

**MEDIÇÃO / GATE:** preservar a variante até 20 sessões anônimas distintas realmente expostas ou cinco pessoas externas identificadas. Sinal inicial = duas sessões clicam e uma chega a `agency_bulk_page_viewed`, `agency_bulk_pack_clicked` ou etapa equivalente; avanço financeiro = `bulk_checkout_started` e `bulk_purchase_completed`/`payment_success`; resultado final = assinatura ativa ou repetição paga. Para chamar um clique de retorno da agência, exigir visita anterior à página de agência na mesma sessão; clique universal do índice não prova origem B2B. Parar com 20 exposições e zero clique, destino quebrado, metadata indevida, queda material do CTA creator, overflow ou divergência comercial.

**RISCO / DÍVIDAS P2:** a escolha extra pode distrair creators, mitigada por posição posterior e estilo secundário. Se a primeira escrita de impressão falhar enquanto o card permanece visível, o latch continua aberto mas a nova tentativa depende de outra transição do `IntersectionObserver`; isso pode subcontar o gate, não muda UX ou checkout. O teste específico executa a política pura e inspeciona o caller, mas ainda não monta React com Observer e `trackEvent(false→true)` mockados. Essas duas dívidas não são P0/P1 e não autorizam chamar telemetria incompleta de receita.

**COMPARAÇÃO VISUAL:** `docs/previews/EXAMPLES-BUSINESS-PROOF-BRIDGE-V1-2026-09-01.html` contém antes/depois desktop e mobile e preserva a hierarquia creator → business. O Chrome conectado recusou `file://` pela política do navegador; não houve contorno. Depois do deploy, a página HTTPS real foi inspecionada em desktop e em viewport de 390 × 844: card creator permaneceu acima, card empresarial empilhou corretamente, destino ficou correto e `scrollWidth=clientWidth=390`. A aba temporária foi fechada.

**TESTADO LOCALMENTE / AUDITORIA:** contrato específico 73/73; distribuição B2B 81/81; example remix 54/54; auth context B2B 26/26 — 234 verificações de regressão mais 73 específicas. O typecheck repetiu exatamente os quatro erros baseline em `mrr.ts`, `me/subscription` e `stripe/checkout` ×2; nenhum erro novo. `git -c core.whitespace=cr-at-eol diff --check` ficou limpo. A auditoria adversarial independente deu **GO**, nenhum P0/P1, e registrou as duas dívidas P2 acima.

**VALIDADO EM PRODUÇÃO (01/09/2026 UTC):** `origin/main` avançou por fast-forward para `a89c55ee67bc08678086615c8a260de00ad0cbee`, contendo o funcional acima. O deploy Vercel `dpl_FkHni3MwjYiWxnvnPjfTEv9JY5Jn` chegou a `READY`, target production, framework Next.js, SHA exato, sem erro de alias e com `www.usekineo.com`/`usekineo.com`. `/examples` respondeu no Chrome com título e região acessível corretos; o CTA empresarial apontou para o anchor canônico, o CTA creator permaneceu anterior e os dois viewports não apresentaram overflow. A varredura Vercel encontrou zero erro de runtime em `/examples` na janela de 30 minutos. Nenhum CTA, checkout, pagamento ou render foi acionado. A visita autenticada de canário pode ter emitido uma impressão interna ao trazer o card ao viewport; ela pertence à conta interna do fundador e deve ser excluída do gate, como todas as contas internas.

**NÃO TOCADO / COORDENAÇÃO:** preço, desconto, cupom, crédito, validade, trial, SKU, promessa, checkout Stripe, Supabase schema/dados, dashboard autenticado, render, motor, cena, voz, legenda, admin, e-mail, outreach, anúncio ou comunicação externa. Não reeditar exemplos, pack de agência ou a variante da rodada 118 antes dos respectivos gates. Claude deve executar `git fetch origin` e partir de `a89c55ee` ou da ponta posterior; o próximo commit desta seção acrescenta somente esta validação documental.

## 121. B2C — medir se a oferta gratuita esconde a oferta de assinatura (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** esta tarefa só entra porque declara o caminho até assinatura no curto, médio e longo prazo. Curto prazo = uma pessoa com intenção de preço consegue receber a oferta adequada e avançar ao checkout; médio prazo = `checkout_started` vira `payment_success` e assinatura ativa; longo prazo = a assinatura renova, indica outra pessoa ou expande receita recorrente. Impressão, supressão, clique e Checkout Session continuam sendo etapas, não receita.

**EVIDÊNCIA DE PRODUÇÃO / BASELINE (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** em 30 dias, `exit_intent_shown` teve 414 eventos, 409 atores, 22 pessoas identificadas e 387 sessões anônimas; `exit_intent_free_clicked`, 56 eventos e 52 sessões anônimas; `starter_checkout_clicked`, 20 eventos e 14 atores (11 pessoas identificadas e três sessões anônimas); `basic_checkout_clicked`, dois atores; `exit_intent_intro_starter_clicked`, uma sessão anônima. Os eventos históricos não carregavam variante e, portanto, esses números não provam colisão nem conversão da oferta.

**FATO CONFIRMADO / LACUNA ANTERIOR:** `components/ExitIntentOffer.tsx` usava a mesma chave de sessão `kineo_exit_offer_shown` para as variantes `free` e `deal`. A home monta `variant="free"` e pricing monta a variante padrão `deal`. Assim, uma oferta gratuita exibida antes da visita a pricing podia fazer o segundo modal retornar no guard compartilhado sem registrar qual variante foi pedida ou qual apareceu primeiro. A hipótese é plausível pelo código, mas seu impacto em pessoas reais era desconhecido.

**HIPÓTESE CAUSAL NOVA / MUDANÇA MÍNIMA:** parte das pessoas que chega à página de preços pode não ver a oferta de assinatura porque a oferta gratuita já consumiu a chave da sessão. O commit funcional `e927c10827f94b72c723d11fd6af21572aec9fa2` não separa chaves, não muda modal, gatilho, copy, CTA, preço ou checkout; ele apenas mede a colisão antes de autorizar uma intervenção.

**IMPLEMENTADO / TELEMETRIA:** `exit_intent_shown` e os cliques legados ganham metadata categórica `version=exit_intent_variant_collision_probe_v1` e `variant=free|deal`. No retorno do guard já existente, `exit_intent_suppressed` registra somente `version`, `requested_variant`, `prior_variant=free|deal|unknown` e `reason=session_key_seen`. Uma chave companheira registra a variante realmente mostrada sem alterar a chave legacy. O controller executável deduplica por variante com request em voo, memória e `sessionStorage`; somente `stored:true` fecha o latch, enquanto falha, exceção ou `stored:false` deixam nova tentativa possível. Nenhum e-mail, URL, preço, prompt, Session ID ou texto livre entra.

**MEDIÇÃO / GATE:** preservar o probe até 20 pessoas externas pós-vídeo chegarem a pricing ou até dez supressões da variante `deal`, o que ocorrer primeiro. Só considerar separar as chaves se pelo menos três pessoas externas distintas apresentarem a sequência `exit_intent_shown(variant=free) → exit_intent_suppressed(requested_variant=deal, prior_variant=free)` na mesma sessão e permanecerem sem assinatura. Resultado financeiro exige `checkout_started → payment_success → assinatura vinculada`; não inferir receita de view, suppress ou click. Parar se houver duplicação material, metadata fora da allowlist, mudança visual, perda da regra uma vez por sessão ou evento atribuído à variante errada.

**CADEIA OBRIGATÓRIA ATÉ ASSINATURA:** curto prazo = identificar e, somente com evidência posterior, remover uma supressão indevida no momento de preço; médio prazo = mais pessoas qualificadas veem a oferta correta e concluem pagamento; longo prazo = pagamento reconciliado sustenta assinatura ativa, renovação e indicação. Se a colisão não atingir o gate causal, não haverá mudança cosmética para fabricar atividade.

**TESTADO LOCALMENTE / AUDITORIA:** probe executável 110/110; pricing journey proof 43/43; welcome offer frequency 34/34; pricing saved checkout 47/47; mobile sticky billing truth 29/29 — 263 verificações. O teste real cobre sucesso e remount, `stored:false` com retry, duas chamadas concorrentes com um único POST, exceções de leitura/escrita, variante anterior inválida e rejeição do writer. A primeira auditoria deu NO-GO porque o controller era validado apenas por inspeção de fonte; depois da refatoração comportamental, a reauditoria deu **GO**, sem P0/P1. O typecheck repetiu exatamente os quatro erros baseline preexistentes em `mrr.ts`, `me/subscription` e `stripe/checkout` ×2; nenhum erro novo. `git -c core.whitespace=cr-at-eol diff --check` ficou limpo.

**COMPARAÇÃO VISUAL:** não aplicável. A rodada não muda DOM visível, texto, estilo, posicionamento, gatilho ou destino; criar um preview diferente seria fabricar uma mudança que não existe.

**RISCO / DÍVIDAS P2:** o wiring do effect React permanece coberto por sentinelas estáticas, enquanto a máquina crítica é executada de verdade. O probe acrescenta eventos somente quando a chave legacy já suprimiria o modal; qualquer leitura de resultado deve excluir contas internas e separar sessões anônimas de pessoas identificadas.

**VALIDADO EM PRODUÇÃO (01/09/2026 UTC):** `origin/main` avançou por fast-forward até `1795fbf676e48626f47a4df1acee34759b48f666`, contendo o funcional `e927c10827f94b72c723d11fd6af21572aec9fa2`. O deploy Vercel `dpl_7NsV4PiNiMvyrG6yNudo5XR2mv1K` chegou a `READY`, target production, framework Next.js, SHA exato e sem erro de alias, com `www.usekineo.com` e `usekineo.com`. `/pricing` respondeu 200 com `X-Matched-Path: /pricing` e o marcador do deployment correto; a varredura dos bundles encontrou `exit_intent_variant_collision_probe_v1` em um chunk publicado. O scan de runtime do deployment retornou zero logs `error`/`fatal` na janela de 30 minutos. Nenhum modal, evento, checkout, pagamento ou render foi forçado para fabricar prova.

**NÃO TOCADO / COORDENAÇÃO:** preço, desconto, cupom, crédito, validade, trial, SKU, promessa, conteúdo das ofertas, página de pricing, WelcomeOfferModal, checkout Stripe, banco/schema, render, motor, cena, voz, legenda, admin, e-mail, outreach, anúncio ou comunicação externa. O commit foi rebaseado sobre `ebbd0098`; não toca `components/video/NextShortsSection.tsx`. Não reeditar as duas ofertas antes do gate acima.

## 122. B2B — intenção empresarial na alternativa ao HeyGen ganha uma saída honesta (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** esta tarefa entra porque declara o caminho até assinatura. Curto prazo = uma sessão que compara Kineo com HeyGen e produz Shorts para clientes ou uma empresa consegue escolher o caminho empresarial existente; médio prazo = o mesmo ator, quando identificável, compara o pack, inicia checkout e apresenta `payment_success`; longo prazo = uso empresarial comprovado sustenta repetição paga, migração para assinatura ativa, renovação ou indicação. Impressão, clique, chegada ao pack e Checkout Session não são receita.

**EVIDÊNCIA DE PRODUÇÃO / BASELINE (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** `/alternatives/heygen` registrou quatro sessões anônimas distintas e zero pessoa identificada nos 90 dias observados; a última sessão foi em 09/08. Na consulta separada do funil de atacado no mesmo período, não apareceu pessoa externa em `bulk_checkout_started` ou `bulk_purchase_completed`; os 13 eventos gerais de `payment_success` de 11 pessoas não carregavam pack/sku bulk. Assim, esta é uma intervenção orgânica de cauda longa sobre uma contradição confirmada no código — não uma resposta a volume atual, nem resultado financeiro já demonstrado.

**FATO CONFIRMADO / LACUNA ANTERIOR:** a própria página diz para escolher HeyGen em bibliotecas de avatar e workflows corporativos, mas seus dois únicos caminhos Kineo eram `Try Kineo free` e `Start free`, ambos creator. A página é estática, indexável e listada no sitemap. Não existia bifurcação para a oferta empresarial já disponível nem evento de exposição capaz de medir essa escolha.

**HIPÓTESE CAUSAL NOVA / IMPLEMENTADO:** o commit funcional `a74d2d2d1f5f1b877fc0187e741076e4838ec481` publica `enterprise_alternative_business_path_v1`. Somente `/alternatives/heygen`, depois do bloco honesto `Which one should you pick?`, recebe o rail secundário `For clients and business content`. Ele leva ao anchor existente `/ai-shorts-for-agencies?entry=heygen_alternative#agency-pack-heading`, sem UTM, preservando first-touch. A copy reutiliza a fronteira comercial existente de 10–50 Fast Shorts one-time/self-service e explicita que não há workspace de equipe, governança de avatar ou serviço gerenciado. Os CTAs creator, seus textos e destinos permanecem intactos.

**ANTI-DUPLICAÇÃO:** a primeira implementação também incluía Synthesia. A auditoria deu **NO-GO** porque `/alternatives/synthesia` já possui experiência publicada e congelada aguardando seu gate. Synthesia foi removida de ponta a ponta — caller, allowlist, contexto, teste e preview — antes do commit. A versão final falha fechado para qualquer competitor diferente de `heygen`.

**TELEMETRIA / PRIVACIDADE:** `enterprise_alternative_business_path_viewed` exige 50% do card visível; `enterprise_alternative_business_path_clicked` mede a escolha. Metadata é limitada a `version`, `competitor=heygen`, `intent=business`, `placement=after_fit_decision` e `destination=agency_packs`. Não entram e-mail, prompt, tópico, URL, preço, Session ID ou texto livre. O recorder executável bloqueia concorrência, fecha o latch somente em `stored:true`, libera retry depois de exceção e suporta storage negado; a impressão tenta novamente uma única vez após 1,5 segundo, somente enquanto o card continua visível, com cleanup de timer e observer.

**MEDIÇÃO / GATE:** preservar até 20 sessões anônimas realmente expostas ou cinco pessoas externas identificadas. Sinal inicial = pelo menos duas sessões/pessoas distintas clicam e uma chega a `agency_bulk_page_viewed(entry=heygen_alternative)`; avanço financeiro = `agency_bulk_pack_clicked → bulk_checkout_started → bulk_purchase_completed/payment_success`; resultado final = assinatura ativa ou repetição paga. Parar com 20 exposições e zero clique, destino/entry quebrado, mudança na hierarquia creator, metadata fora da allowlist, overflow ou divergência da oferta. A amostra histórica pequena impede atribuir lift antes do gate.

**COMPARAÇÃO VISUAL:** `docs/previews/ENTERPRISE-ALTERNATIVE-BUSINESS-PATH-V1-2026-09-01.html` mostra antes/depois desktop e mobile somente de HeyGen. O Chrome conectado confirmou CTA empresarial secundário, CTA creator preservado e `scrollWidth=clientWidth=390` nos dois quadros móveis. A aba e o servidor temporários foram fechados.

**TESTADO LOCALMENTE / AUDITORIA:** contrato específico 86/86; atribuição da ponte 67/67; distribuição B2B 84/84; intenção InVideo 29/29; verdade Synthesia 33/33 — 299 verificações. O typecheck repetiu exatamente os quatro erros baseline preexistentes em `mrr.ts`, `me/subscription` e `stripe/checkout` ×2; nenhum erro novo. `git -c core.whitespace=cr-at-eol diff --check` ficou limpo. Depois do NO-GO inicial e das correções acima, a reauditoria independente deu **GO**, sem P0/P1.

**VALIDADO EM PRODUÇÃO (01/09/2026 UTC):** `origin/main` avançou por fast-forward até `43c505d5e6e4bb33ac29f79fc6ff6256617090f3`, contendo o funcional `a74d2d2d1f5f1b877fc0187e741076e4838ec481`. O deploy Vercel `dpl_kvh2xe5MJv8sddcAjG8Krvw3DwLx` chegou a `READY`, target production, framework Next.js, SHA exato e sem erro de alias, com `www.usekineo.com` e `usekineo.com`. A resposta pública de `/alternatives/heygen` contém, na ordem correta, “Which one should you pick?”, “For clients and business content”, o CTA “Compare business video packs” e o destino `/ai-shorts-for-agencies?entry=heygen_alternative#agency-pack-heading`; o CTA creator “Try Kineo free” continua presente. A varredura de runtime de `/alternatives/heygen` retornou zero erros na janela de uma hora. Nenhum evento, CTA, checkout, pagamento ou render foi forçado para fabricar prova. **ESTADO ATUAL:** **IMPLEMENTADO / TESTADO LOCALMENTE / VALIDADO EM PRODUÇÃO / AGUARDANDO AMOSTRA EXTERNA.**

**NÃO TOCADO / COORDENAÇÃO:** preço, desconto, cupom, crédito, validade, trial, SKU, promessa nova, pack, checkout Stripe, webhook, Supabase schema/dados, dashboard autenticado, página/agência de destino, Synthesia, render, motor, cena, voz, legenda, admin, e-mail, outreach, anúncio ou comunicação externa. Claude deve executar `git fetch origin` e partir do SHA documental desta seção ou da ponta posterior; não reeditar HeyGen nem Synthesia antes de seus gates. A próxima rodada do Codex volta a B2C em superfície distinta, mantendo assinatura real como placar.

## 123. B2C — a recuperação alcança o comprador, mas não retoma o Stripe escolhido (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** curto prazo = um comprador que abandonou o checkout consegue continuar a compra original; médio prazo = a retomada autenticada chega a `payment_success` e assinatura vinculada; longo prazo = assinatura ativa renova, indica ou expande receita recorrente. E-mail enviado, clique e Session continuam sendo etapas, não receita.

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** o probe B2C imediatamente anterior, `exit_intent_variant_collision_probe_v1`, ainda possui zero evento versionado e deve permanecer intacto até seu gate. Em uma janela separada de 30 dias, 40 pessoas externas tiveram `email_send_log(kind=checkout_recovery, ok=true)` entre 17/08 e 30/08; uma apresentou `payment_success` em até sete dias depois do primeiro envio. A sequência não prova atribuição ao e-mail. Na coorte pós-vídeo do mesmo período, 76 pessoas não tiveram nenhuma das superfícies comerciais observadas em 24 horas; 48 hoje constam como free/downgraded e 46 dessas tinham uma única entrega. Porém, em 31/08 e 01/09 houve zero pessoa downgraded nessa condição, portanto as correções recentes não devem ser reeditadas com base no estoque histórico.

**FATO CONFIRMADO / LACUNA NOVA:** `app/api/cron/send-recovery/route.ts:299-346` monta o e-mail automático com PayPal e resposta por e-mail, mas sem caminho para retomar a compra Stripe original. O próprio arquivo documenta `/api/stripe/checkout/resume?go=1` como a continuação correta ainda retida para QA (`:203-244`). A rota existente autentica a pessoa, valida posse, assinatura/duplicidade e resolve sessão aberta, recuperação Stripe ou retry interno (`app/api/stripe/checkout/resume/route.ts:291-493`); o caller dentro do site já usa esse endpoint (`components/CheckoutResumeBanner.tsx:173-185`).

**HIPÓTESE CAUSAL NOVA:** permitir que o destinatário continue exatamente o checkout Stripe que escolheu pode reduzir abandono pós-expiração sem alterar preço, plano, oferta ou método de pagamento. Isto não duplica o banner on-site: o e-mail atua depois e fora do navegador original.

**GATE PROPOSTO, NÃO EXECUTADO:** se o fundador autorizar explicitamente a mudança de comunicação, adicionar um CTA neutro `Resume your secure checkout`, preservar PayPal e resposta como alternativas e medir por pessoa externa `checkout_recovery enviado → retomada autenticada resolvida → payment_success → assinatura vinculada`. Preservar até dez destinatários versionados e exigir pelo menos três retomadas autenticadas antes de ler sinal. Parar imediatamente por divergência de tier, periodicidade ou valor, duplicidade, ou se mais da metade das retomadas autenticadas resolver como ausente/antiga. Scanner sem sessão deve continuar no login e nunca criar Session.

**BLOQUEADO POR AUTORIZAÇÃO / NENHUMA EDIÇÃO DE RUNTIME:** `AGENTS.md` exige autorização explícita até para rascunho ou alteração de comunicação externa, e o mandato atual proíbe envio automático novo. Por isso esta rodada não alterou o e-mail, a rota, o cron ou qualquer oferta. Implementar sem essa autorização seria cruzar um gate mais importante do que a hipótese. O diagnóstico foi revisado por agente B2C independente, sem edição.

**NÃO TOCADO / ANTI-DUPLICAÇÃO:** ExitIntentOffer, pricing, WelcomeOfferModal, Plan Fit, pós-vídeo, modal de fim do trial, checkout Stripe, retomada on-site, preço, desconto, cupom, crédito, validade, SKU, promessa, Supabase schema/dados, render, motor, cena, voz, legenda, admin, outreach, anúncio ou comunicação externa. A próxima rodada alterna para B2B em superfície distinta; esta hipótese permanece pendente de autorização específica.

## 124. B2B — o checkout está íntegro; falta prova empresarial autorizada (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** curto prazo = uma pessoa empresarial encontra prova adequada e avança da descoberta para a oferta; médio prazo = seleção de pack ou plano chega a `payment_success`; longo prazo = uso recorrente sustenta assinatura ativa, renovação, repetição paga ou indicação. Impressão, clique, proposta, pack escolhido e Checkout Session não são receita.

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** em sete dias, `agency_volume_bridge_viewed` teve duas pessoas externas identificadas e quatro sessões anônimas; `autopilot_break_even_viewed`, três pessoas; `autopilot_page_arrived` e `autopilot_page_viewed`, duas pessoas cada; `autopilot_pilot_checkout_clicked`, duas pessoas em três eventos; `pricing_business_path_viewed`, duas pessoas; `agency_bulk_page_viewed`, uma sessão anônima; `agency_margin_calculator_viewed`, uma sessão anônima; `autopilot_checkout_clicked`, uma pessoa. Não houve linha em `agency_volume_bridge_clicked`, `pricing_business_path_clicked`, `autopilot_break_even_clicked`, `agency_bulk_pack_clicked`, `bulk_checkout_started` ou `bulk_purchase_completed`. Eventos e sessões não foram apresentados como pessoas ou receita.

**FATO CONFIRMADO / CAMINHO TÉCNICO:** a página de agências preserva o pack exato no CTA, mede a escolha e direciona para `/api/stripe/checkout?pack=<id>` (`app/ai-shorts-for-agencies/AgencyPacksClient.tsx:137-205`). O servidor exige autenticação, preserva o destino completo, cria uma única sessão Stripe idempotente em USD, retorna ao mesmo pack se o Checkout for fechado e emite `bulk_checkout_started` somente depois da Session real (`app/api/stripe/checkout/route.ts:2664-2833`; `lib/growth/agencyCheckoutReturn.ts`). Login e cadastro reconhecem o contexto B2B e mostram quantidade, preço e natureza one-time antes de retomar (`app/(auth)/login/page.tsx`; `app/(auth)/signup/page.tsx`; `lib/growth/bulkCheckoutAuthContext.ts`). Não apareceu contradição nova que justificasse reeditar checkout, auth ou pack sem amostra.

**PESQUISA EXTERNA / HIPÓTESE CAUSAL NOVA (fontes oficiais consultadas em 01/09/2026):** HeyGen posiciona casos de clientes, reviews, métricas de resultado e usos por equipe/agência antes do CTA empresarial (`https://www.heygen.com/customer-stories`, `https://www.heygen.com/en-gb/business`, `https://www.heygen.com/en-gb/business/agencies`); Synthesia organiza cases com resultado e contexto empresarial (`https://www.synthesia.io/case-studies`); InVideo Enterprise combina solução por workflow, segurança e formulário empresarial (`https://invideo.io/enterprise/`). A Kineo possui exemplos de saída e uma review pública creator, mas nenhum case empresarial autorizado e verificável. A hipótese é que falta de prova adequada — e não falha do checkout — limita a confiança B2B antes do clique.

**DECISÃO DE NÃO FABRICAR ATIVIDADE:** a auditoria independente propôs um aviso honesto em `/reviews`, porém a consulta de 30 dias não encontrou evento ou metadata de visita à rota e destacar ausência de case poderia reduzir confiança sem alcançar amostra. Nenhum card, CTA, preço, promessa ou evento novo foi publicado. A intervenção correta depende de um resultado empresarial real e consentimento explícito; reutilizar uma review creator como prova empresarial seria falso.

**QUESTÃO CANÔNICA / GATE:** `docs/OPEN_QUESTIONS.md` ganhou Q13. Só admitir um case quando houver pessoa externa com uso empresarial comprovado no produto, resultado separável de estimativa e consentimento registrável para cada campo publicado. Depois, medir exposição por pessoa externa → avanço à oferta → `checkout_started`/`bulk_checkout_started` → `payment_success` → assinatura ativa ou repetição paga. Parar e remover qualquer claim que não possa ser ligado à fonte, data e autorização.

**PUBLICADO / VALIDADO:** `origin/main` avançou por fast-forward para `7ca43eec4059366a45da2efb6522462a8ed3d893`, commit exclusivamente documental. O deploy automático Vercel `dpl_7aowbtF3iFZoAakpC2zszaE1xLuS` chegou a `READY`, target production, framework Next.js, SHA exato, aliases `www.usekineo.com`/`usekineo.com` ativos e `aliasError=null`. A varredura do deployment encontrou zero log `error` ou `fatal` em 30 minutos. Nenhum código de runtime, CTA, checkout, pagamento, banco ou render foi alterado ou acionado.

**ANTI-DUPLICAÇÃO / PRÓXIMA RODADA:** preservar home bridge, pricing business path, examples bridge, HeyGen rail, Autopilot break-even, proposta/margem e bulk até seus gates. Não criar outra landing, outra ponte para packs ou outra promessa empresarial enquanto essas variantes não tiverem amostra. A próxima rodada volta a B2C em superfície distinta; o déficit de prova B2B permanece no board sem bloquear aquisição orgânica já publicada.

## 125. B2C — o salto OAuth passa a ser reconciliável até pagamento (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** curto prazo = uma sessão que tentou comprar deixa de desaparecer entre autenticação e retorno ao checkout; médio prazo = a mesma sessão autenticada chega a `checkout_started` e `payment_success`; longo prazo = o pagamento reconciliado vira assinatura ativa, renovação e possível indicação. Página de login, método de autenticação, callback e Checkout Session são etapas — não receita.

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** em sete dias, sete sessões anônimas emitiram `checkout_auth_method_selected`, todas no signup e com método Google; o código prova que esse evento era disparado pelo autostart, não necessariamente por clique humano. No mesmo período, houve um único `auth_callback_completed` identificado com `is_checkout_destination=true`. Os callbacks tinham `session_id` nulo, portanto esses dois lados não podiam ser ligados individualmente. Não se afirmou que seis pessoas abandonaram: sete sessões anônimas e um callback identificado são réguas diferentes.

**FATO CONFIRMADO / LACUNA ANTERIOR:** `lib/analytics.ts` já criava `kineo_event_session_id` em `sessionStorage`, sincronizava o mesmo valor para cookie first-party `SameSite=Lax` e o gravava nos eventos do navegador. `app/auth/callback/route.ts` escrevia `auth_callback_completed` e `auth_callback_failed` sem passar esse identificador ao `writeServerEvent`, embora o helper já aceite `sessionId`. Assim, o salto pelo Google apagava a ligação da sessão exatamente no último metro. Além disso, o autostart Google e o fallback de webview eram ambos registrados como `method_selected`, misturando navegação automática, alternativa apresentada e escolha humana.

**HIPÓTESE CAUSAL NOVA / MUDANÇA MÍNIMA:** antes de trocar a experiência de autenticação, tornar a passagem mensurável revela se o abandono ocorre antes do Google, no callback, na retomada do checkout ou dentro da Stripe. O contrato `checkout_auth_session_bridge_v1` copia o mesmo ID opaco já criado por aba para um cookie companheiro de 30 minutos, exclusivo do salto de checkout. Eventos comuns de outra aba podem continuar atualizando o cookie geral, mas não reescrevem esse pin. O callback valida 8–64 caracteres, usa o valor somente em `events.session_id` quando o destino é checkout e apaga o cookie depois de sucesso ou falha (`lib/growth/checkoutAuthSessionBridge.ts`; `lib/analytics.ts`; `app/auth/callback/route.ts`). Não nasce um segundo identificador e o valor cru não é duplicado em metadata.

**SEMÂNTICA DA MEDIÇÃO:** todos os passos client-side do checkout auth recebem `auth_session_bridge_version`. `checkout_auth_method_selected` recebe `selection_kind=explicit` por padrão e o Google iniciado automaticamente declara `automatic`. O fallback de e-mail apenas apresentado em webview deixa de inflar seleção e passa ao evento próprio `checkout_auth_fallback_presented` (`lib/authAnalytics.ts`; `app/(auth)/signup/page.tsx`). A experiência visual, o autostart, o provedor, o destino e o comportamento do login não mudaram.

**MEDIÇÃO / GATE:** depois da fronteira de produção, seguir por `session_id` externo `checkout_auth_page_view → checkout_auth_method_selected(selection_kind) → auth_callback_completed|failed → checkout_started → payment_success`, promovendo a pessoa somente quando o callback autenticar `user_id`. Preservar até dez sessões externas versionadas que iniciem autenticação ou cinco callbacks de checkout versionados, o que ocorrer primeiro. Só propor desligar ou atrasar autostart se pelo menos três sessões distintas `automatic` terminarem sem callback e o ramo `explicit` apresentar comportamento melhor; parar imediatamente por sessão divergente, token livre, destino alterado, loop auth ou perda de evento. Callback comum não consome nem apaga o pin de checkout; duas autenticações de checkout simultâneas no mesmo browser ainda compartilham o companion e permanecem uma limitação rara a excluir da leitura causal, porque last writer wins.

**TESTADO LOCALMENTE:** contrato novo 61/61 executa o módulo real de analytics com cookie/fetch mocks, simula uma segunda aba, executa a classificação `automatic|explicit|fallback_presented`, invoca o GET real do callback com fronteiras mockadas para sucesso/falha/cookie inválido/destino comum, reproduz `checkout A → auth comum B → callback A` e prova que o token nunca entra em metadata; OAuth failure handoff 56/56; password recovery 57/57; signup resolution 27/27; contexto B2B 26/26; verdade orgânica 36/36; destino de produto 109/109; criação preservada 44/44 — 416 verificações. `git -c core.whitespace=cr-at-eol diff --check` ficou limpo. O typecheck repetiu exatamente os quatro erros baseline preexistentes em `app/api/admin/_shared/mrr.ts`, `app/api/me/subscription/route.ts` e `app/api/stripe/checkout/route.ts` ×2; nenhum erro novo. Comparação visual não se aplica porque DOM, copy, estilo, ordem e navegação permanecem idênticos. O evento `checkout_auth_fallback_presented` ficará visível em SQL bruto; o painel existente não o agrega, mas também deixa de inflar `checkout_auth_method_selected`.

**AUDITORIA ESPECIALIZADA / COORDENAÇÃO:** a auditoria B2C independente encontrou outra hipótese, sem edição: em mobile, `InstallAppBanner` e `EnablePushBanner` podem coexistir acima da `MobileNav` sem telemetria, competindo com o próximo passo comercial. O caller global está em `app/(dashboard)/layout.tsx`, pista do Claude; Codex não tocou nessa superfície. O handoff recomendado ao dono é instrumentar primeiro e só coordenar prioridade depois de 20 pessoas externas expostas ou dez colisões reais.

**AUDITORIA ADVERSARIAL DA ENTREGA:** a primeira revisão deu NO-GO porque o teste era parcialmente estático e o cookie geral podia ser sobrescrito por outra aba. A segunda bloqueou porque um callback comum ainda apagaria o pin de checkout da aba vizinha. O desenho final usa companion de curta duração, não o apaga em autenticação comum e executa a sequência concorrente no teste. A reauditoria final deu **GO**, com P0=0 e P1=0, presa ao diff atual; a corrida rara entre dois checkouts OAuth simultâneos permanece como P2 explícito no gate.

**VALIDADO EM PRODUÇÃO (01/09/2026 UTC):** `origin/main` avançou por fast-forward para `fd152eddd511d16fb9d883d25dc85910f2ddd24b`, já rebaseado sobre o commit `759884e968f4a41cb68ab5834ccf29a3ca0649ad` do Claude e sem sobreposição de arquivos. O deploy Vercel `dpl_FtcCKmfD1RePKGPoJokyBU2Ld521` chegou a `READY`, target production, framework Next.js, SHA exato, `aliasError=null` e aliases `www.usekineo.com`/`usekineo.com` ativos. `/signup?destination=checkout&plan=creator` respondeu 200 pelo deployment correto; o build terminou sem erro e a varredura `error|fatal` específica desse deployment retornou vazia. Nenhum OAuth, checkout, pagamento, evento ou render foi forçado para fabricar prova. **ESTADO ATUAL:** **IMPLEMENTADO / TESTADO LOCALMENTE / VALIDADO EM PRODUÇÃO / AGUARDANDO AMOSTRA EXTERNA.**

**NÃO TOCADO:** preço, desconto, cupom, crédito, validade, trial, SKU, promessa, checkout Stripe/server/webhook, Supabase schema/dados, render, motor, cena, voz, legenda, admin, e-mail, outreach, anúncio, ExitIntent, WelcomeOffer, Plan Fit, oferta pós-vídeo ou qualquer experimento abaixo do gate.

## 126. B2B — cinco compradores chegaram ao Stripe; a próxima leitura precisa respeitar a fronteira das correções (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** curto prazo = uma pessoa empresarial atravessa descoberta, escolha e Checkout sem perder o produto escolhido; médio prazo = `payment_success` reconciliado e assinatura ou compra B2B ativa; longo prazo = renovação do Autopilot, repetição de pack/Pilot ou indicação de outra empresa. Visita, clique, formulário e Checkout Session são etapas, não receita.

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO (Supabase + Stripe, SELECT/leitura somente em 01/09/2026 UTC; contas internas excluídas):** cinco pessoas externas distintas clicaram no checkout do Autopilot e criaram Checkout Sessions: três escolheram o Pilot avulso e duas o mensal. As cinco sessões estão expiradas e não pagas, sem PaymentIntent concluído, assinatura ou `payment_success`. Assim, existe intenção empresarial real no último metro, mas ainda não existe venda Autopilot.

**FATO CONFIRMADO / FRONTEIRA CAUSAL:** as cinco sessões foram criadas antes das correções recentes de retorno, recuperação e sucesso Autopilot. As duas mensais antigas possuíam recuperação Stripe; as três Pilot antigas não. O código atual já habilita recuperação do Pilot e preserva contexto específico no retorno. Portanto, usar essas sessões históricas para reeditar agora a copy, o checkout ou o método de pagamento confundiria variante antiga com produto atual. O próximo diagnóstico válido começa na primeira Session externa criada depois da fronteira atual de produção e segue a mesma pessoa até expiração, retomada ou pagamento.

**FATO CONFIRMADO / MÉTODOS DE PAGAMENTO:** as cinco sessões usaram configuração dinâmica do Stripe; naquele contexto, somente cartão foi apresentado como elegível. Isto não prova que a Kineo restringiu manualmente outros métodos, nem autoriza habilitar um método, moeda ou país sem evidência de elegibilidade e reconciliação no webhook.

**EVIDÊNCIA DE PRODUÇÃO / SAASHUB (Supabase, 90 dias até 01/09/2026 UTC; contas internas excluídas):** não existe evento cuja metadata contenha `saashub`, nem `directory_bridge_viewed` atribuído à rota dedicada. O código possui `/from-saashub`, telemetria e contrato documental desde julho, mas não há amostra atribuída. A página pública atual de alternativas ao InVideo foi consultada e não permitiu confirmar o outbound da listagem Kineo; `/kineo-studio` devolveu 403 ao crawler. Assim, não se afirmou que a listagem está ausente nem que aponta para a home.

**DECISÃO DE NÃO DUPLICAR / NENHUMA EDIÇÃO DE RUNTIME:** não nasceu outra landing, outro rail B2B, outra copy de checkout nem redirect especulativo. As variantes recentes continuam abaixo dos próprios gates; o déficit B2B observado agora é de amostra pós-fronteira e prova empresarial autorizada, não de mais uma superfície.

**GATE DO PRÓXIMO EXPERIMENTO:** preservar até cinco pessoas externas que criem Checkout Session Autopilot depois da fronteira atual ou até o primeiro `payment_success`, o que ocorrer antes. Por pessoa, reconciliar `autopilot_*_checkout_clicked → checkout_started → expiração/retomada → payment_success → entitlement`. Só propor mudança causal se pelo menos três pessoas pós-fronteira pararem na mesma etapa ou se aparecer contradição de tier, valor, retorno, recuperação ou entitlement. Parar imediatamente por Session duplicada, produto divergente, pagamento sem acesso ou acesso sem pagamento.

**SAASHUB / PRÓXIMA VERIFICAÇÃO:** antes de qualquer alteração externa, confirmar no painel autenticado do diretório qual URL foi aprovada. Se a URL real não for `/from-saashub`, corrigir a configuração somente com autorização específica de comunicação externa; depois preservar por dez sessões externas atribuídas e medir `directory_bridge_viewed → organic_topic_submitted|signup → primeiro vídeo → checkout → payment_success`. Sem tráfego, publicação em diretório não será chamada de aquisição.

**ESTADO ATUAL:** **DIAGNÓSTICO CONCLUÍDO / NENHUMA MUDANÇA DE RUNTIME / AGUARDANDO AMOSTRA PÓS-FRONTEIRA.** Não foram acionados checkout, pagamento, e-mail, outreach, render ou escrita no banco para fabricar prova.

## 127. B2C — a missão que já produziu uma assinatura ganha denominador confiável (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** curto prazo = uma pessoa elegível recebe a missão de primeira entrega premium e conclui o vídeo; médio prazo = percebe valor, chega ao checkout e produz `payment_success`; longo prazo = a assinatura permanece ativa, renova ou indica outra pessoa. Banner autorizado, card elegível, clique, vídeo e Checkout Session são etapas; somente pagamento reconciliado e assinatura contam no placar final.

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** nas 72 horas examinadas havia seis pessoas externas com `checkout_started` e uma com `payment_success`. A pessoa paga tinha uma entrega concluída antes do checkout e havia passado por `video_ready_viewed`, download, pricing, Plan Fit e prova da jornada. Entre as cinco não pagas, quatro chegaram ao checkout sem entrega concluída e uma com uma entrega; nenhuma das cinco tinha a cadeia completa de valor observada. Isto descreve caminhos observados, não prova causalidade de uma superfície isolada.

**EVIDÊNCIA DE PRODUÇÃO / EXPERIMENTO ATUAL:** `trial_first_seedance_60s_v1` teve quatro pessoas externas que clicaram, duas que concluíram vídeo e zero checkout/pagamento observado. `trial_first_seedance_35s_v2` teve três pessoas externas que clicaram, uma que concluiu, uma que chegou ao checkout e a mesma uma com `payment_success`. Somadas, as duas versões tiveram sete clickers, seis chegadas ao destino, cinco progressões, três conclusões em 24 horas, duas pessoas com download, uma com checkout e uma com pagamento. A amostra é sinal pequeno, não prova de lift.

**FATO CONFIRMADO / LACUNA DE MEDIÇÃO:** o clique `trial_first_delivery_clicked` já carregava a versão da missão, mas `trial_active_banner_shown` misturava três experiências mutuamente exclusivas — primeira entrega, escada de retorno e CTA de assinatura — sem dizer se o card de primeira entrega estava elegível. Portanto havia numerador versionado e nenhum denominador válido. A impressão existente representa banner autorizado/montado com card elegível; não comprova que o card entrou no viewport.

**HIPÓTESE CAUSAL PRESERVADA / MUDANÇA MÍNIMA:** a missão de primeira entrega premium pode criar valor suficiente para levar parte dos novos trials ao pagamento, mas qualquer decisão de UI agora seria prematura. O commit funcional `5c5cd15d71ed24c31d565f694022741cde1d74a3` mantém a experiência intacta e acrescenta ao evento existente apenas `first_delivery_eligible`, `first_delivery_version` e `first_delivery_reason`. A decisão é calculada diretamente do payload de créditos antes de qualquer `setState`, preserva uso desconhecido como `null` e não promove desconhecido a zero.

**PRIVACIDADE / CONTRATO:** o builder puro aceita a decisão já executada e retorna exatamente três campos categóricos. Não inclui saldo, e-mail, conteúdo, roteiro, vídeo, preço, Session ID, URL ou metadata livre. A metadata nova entra somente em `trial_active_banner_shown`; clique, checkout, crédito, card, copy, destino e gate de produto não mudaram.

**MEDIÇÃO / GATE:** preservar `trial_first_seedance_35s_v2` sem reeditar até pelo menos dez pessoas externas distintas pós-deploy com banner autorizado e `first_delivery_eligible=true`/versão presente. Ler por pessoa `banner elegível → trial_first_delivery_clicked → chegada/progresso → vídeo concluído/download → checkout_started → payment_success → assinatura ativa`. Parar imediatamente se metadata e card divergirem, se `null` virar elegível, se o evento duplicar materialmente ou se qualquer saldo/texto livre entrar. Esta rodada não chama a métrica de impressão de viewport.

**TESTADO LOCALMENTE / AUDITORIA:** contrato trial balance bridge 208/208; trial best activation 27/27; trial repeat before checkout 47/47; money truth 306/306. O typecheck local repetiu exatamente os quatro erros baseline em `app/api/admin/_shared/mrr.ts`, `app/api/me/subscription/route.ts` e `app/api/stripe/checkout/route.ts` ×2; nenhum erro novo nos três arquivos editados. `git diff --check` ficou limpo, com apenas os avisos de conversão LF/CRLF já esperados no Windows. A reauditoria B2C independente deu **GO preso ao diff**, P0=0 e P1=0; repetiu também checkout cancel 39/39, pricing saved checkout 47/47 e money truth 306/306. Comparação visual não se aplica porque DOM, copy, estilo, ordem e navegação não foram alterados.

**VALIDADO EM PRODUÇÃO (01/09/2026 UTC):** `origin/main` avançou por fast-forward para `5c5cd15d71ed24c31d565f694022741cde1d74a3`. O deploy Vercel `dpl_58JhACECMFEn5eKpJPrreCJA51wr` chegou a `READY`, target production, framework Next.js, SHA exato, `aliasError=null` e aliases `www.usekineo.com`/`usekineo.com` ativos. A busca de logs `error|fatal` específica desse deployment em 30 minutos retornou vazia. Nenhum checkout, pagamento, render, crédito, evento artificial ou comunicação foi acionado para fabricar prova.

**NÃO TOCADO / COORDENAÇÃO:** preço, desconto, cupom, crédito, validade, trial como oferta, SKU, promessa, Stripe, Supabase schema/dados, render, motor, cena, voz, legenda, admin, e-mail, outreach ou anúncio. Preservar a missão e todas as superfícies recentes abaixo do próprio gate. A próxima rodada alterna para B2B em superfície distinta e continua subordinada ao objetivo permanente de assinatura real.

## 128. B2B — o rodapé global ganha denominador por porta empresarial, sem criar outra porta (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** curto prazo = descobrir qual caminho empresarial existente é realmente exposto e escolhido; médio prazo = a mesma sessão avança à proposta, pack ou Autopilot e chega ao checkout/pagamento; longo prazo = pagamento empresarial vira repetição, assinatura ativa, renovação ou indicação. View, clique, visita e Checkout Session são etapas — não receita.

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** nas 24 horas observadas, `agency_volume_bridge_viewed` teve sete eventos — duas pessoas identificadas e cinco sessões anônimas —, `autopilot_break_even_viewed` teve três pessoas e `pricing_business_path_viewed` teve duas. Não apareceu clique ou checkout no conjunto reconciliado. Em 30 dias, `autopilot_page_viewed` alcançou 42 pessoas, nove pessoas clicaram upgrade, três clicaram o Pilot e duas clicaram o checkout mensal; as cinco Checkout Sessions históricas permanecem anteriores às correções e não pagas. Esses números pertencem a superfícies e janelas diferentes e não foram somados como um funil único.

**FATO CONFIRMADO / LACUNA ANTERIOR:** o `Footer` global já continha quatro portas empresariais honestas — case do Autopilot, packs de agência, planejador de conteúdo empresarial e briefing de cliente — mas todas eram `Link` cru, sem denominador de visibilidade nem clique. O rodapé é importado por 46 arquivos públicos. Criar outra landing ou outra ponte duplicaria oferta abaixo do gate; medir o que já existe responde qual porta merece distribuição posterior.

**HIPÓTESE CAUSAL NOVA / IMPLEMENTADO:** o commit funcional `e2a0f52f2e7ddf51e1d4a357a2a0e1c6643432a2` publica `footer_business_discovery_v1`. Somente os quatro hrefs já existentes e allow-listed passam pelo `FooterBusinessLink`; copy, href, ordem, estilo e DOM final continuam iguais. `/partners`, `/trust`, `/pricing`, real estate e demais links ficam explicitamente fora para não misturar afiliado, confiança, creator ou intenção local com compra B2B.

**TELEMETRIA / PRIVACIDADE:** `footer_business_path_viewed` exige 60% do link visível e `footer_business_path_clicked` mede a escolha sem `preventDefault`. O denominador declarado é `(session_id, destination)`: até um view e um click por destino e sessão, não quatro pessoas quando quatro links aparecem. O latch fecha somente com `stored:true`; corrida em voo, remount, storage negado e falha/throw de analytics são tratados sem bloquear navegação. A metadata nova contém somente `version`, `surface=global_footer`, `destination` enum e `measurement_unit=event_session_destination`; `trackEvent` continua anexando UTMs globais preexistentes, mas nenhum PII, conteúdo de cliente ou URL bruta foi adicionado por esta rodada.

**MEDIÇÃO / GATE:** preservar o rodapé e as quatro páginas de destino até 20 sessões externas distintas expostas ou cinco sessões externas distintas com clique, o que ocorrer primeiro. Ler por destino `footer_business_path_viewed → footer_business_path_clicked → evento de chegada da superfície → checkout_started|bulk_checkout_started → payment_success|bulk_purchase_completed → assinatura ativa ou repetição paga`. Alcance global usa `COUNT(DISTINCT session_id)`; comparação por porta usa pares distintos `(session_id, destination)`. Parar por destino quebrado, inflação acima de um evento por sessão/destino, metadata indevida, mudança visual ou queda observável da navegação.

**TESTADO LOCALMENTE / AUDITORIA:** contrato executável 48/48; distribuição B2B 84/84; home bridge 27/27; money truth 306/306. O teste antigo `test-done-footer-2026-08-31.mjs` repetiu quatro falhas preexistentes em `GenerateClient`, fora dos arquivos e do workstream desta rodada. O typecheck antes do build repetiu exatamente os quatro erros baseline em `mrr.ts`, `me/subscription` e `stripe/checkout` ×2, sem erro novo. `git -c core.whitespace=cr-at-eol diff --check` ficou limpo. O build local compilou o código com sucesso e parou depois, na coleta de `/api/generate-avatar`, porque a worktree isolada não recebe `OPENAI_API_KEY`; nenhum segredo foi lido ou exposto. A auditoria B2B deu **GO**, P0=0 e P1=0.

**RISCO / DÍVIDAS P2:** se o primeiro POST de `viewed` falhar enquanto o link permanecer continuamente acima de 60%, a nova tentativa depende de outra transição do observer; isso pode subcontar o gate, nunca inflar nem afetar UX. A camada global de analytics anexa UTMs além dos quatro campos do builder, comportamento preexistente que precisa ser lembrado ao consultar metadata.

**COMPARAÇÃO VISUAL:** não aplicável. A entrega não altera copy, href, ordem, estilo, wrapper ou layout; cada ilha cliente devolve um único `Link` idêntico ao anterior. Fabricar preview diferente seria inventar mudança visual inexistente.

**VALIDADO EM PRODUÇÃO (01/09/2026 UTC):** `origin/main` avançou por fast-forward para `e2a0f52f2e7ddf51e1d4a357a2a0e1c6643432a2`. O deploy Vercel `dpl_3ZXH5pvPkDUVRL8uDkT1fBgUxDFh` chegou a `READY`, target production, framework Next.js, SHA exato, `aliasError=null` e aliases `www.usekineo.com`/`usekineo.com` ativos. `/ai-shorts-for-agencies` respondeu HTTP 200 com `X-Matched-Path` correto e assets presos ao deployment; o bundle publicado contém `footer_business_discovery_v1`. A varredura de 30 minutos mostrou somente dois avisos preexistentes de geração (`url.parse()` e limiar de b-roll), sem erro de rodapé ou página comercial. Nenhum link, evento, checkout, pagamento, render ou comunicação foi acionado para fabricar prova.

**NÃO TOCADO / COORDENAÇÃO:** preço, desconto, cupom, crédito, validade, trial, SKU, promessa, copy comercial, página de destino, checkout Stripe, Supabase schema/dados, render, motor, cena, voz, legenda, admin, e-mail, outreach, anúncio ou tráfego ativo. Preservar as superfícies B2B recentes e este denominador até seus gates; a próxima rodada alterna para B2C em superfície distinta e permanece subordinada ao placar permanente de assinatura real.

## 129. B2C — Plan Fit passa a medir o botão realmente visto, sem reeditar a oferta (01/09/2026)

**DECISÃO APROVADA / REGRA PERMANENTE DO BOARD:** toda iniciativa continua obrigada a declarar o caminho até assinatura no curto, médio e longo prazo. Nesta rodada, curto prazo = a pessoa realmente vê o CTA do plano recomendado e decide avançar; médio prazo = a mesma pessoa cria Checkout Session, conclui `payment_success` e recebe a assinatura correta; longo prazo = a assinatura permanece ativa, renova ou gera indicação. Card montado, CTA visto, clique e Checkout Session são etapas — não receita.

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** depois da fronteira do deploy da telemetria de primeira entrega (`2026-09-01T11:40:31.028Z`), somente uma pessoa externa chegou a `video_generation_completed` e `video_ready_viewed`; não houve pessoa elegível em `trial_active_banner_shown`, clique da missão, checkout ou pagamento. O gate da primeira entrega permanece sem amostra válida e não foi reeditado. Na janela distinta de sete dias entre `2026-08-25T12:18:36Z` e `2026-09-01T12:18:36Z`, 76 pessoas externas concluíram o primeiro vídeo; 70 viram o resultado, 32 baixaram, oito chegaram a pricing, seis ao checkout e uma ao pagamento. Dessas 76, 65 tinham exatamente um vídeo concluído e nenhuma havia chegado a quatro ou mais naquela janela. Esses números descrevem pessoas e etapas observadas, não atribuem causalidade ao Plan Fit.

**EVIDÊNCIA DE PRODUÇÃO / VARIANTE ATUAL:** `plan_fit_direct_win_v3` registrou impressão do card para sete pessoas externas distintas e nenhum clique até a leitura de 01/09/2026. A impressão anterior era disparada quando 35% do card inteiro entrava no viewport; portanto, não provava que o botão `Start` — localizado abaixo no card — havia sido visto. Sete cards montados com zero clique não permitiam distinguir rejeição da oferta de simples ausência de exposição ao CTA.

**HIPÓTESE CAUSAL NOVA / IMPLEMENTADO:** o commit funcional `905a9e530650ab7707eb12e361fb6bd985933daf` adiciona exclusivamente o evento `plan_fit_checkout_cta_viewed`, emitido quando pelo menos 60% do botão real está visível e ele continua acionável. Copy, preço, plano recomendado, layout, ordem, checkout e comportamento do CTA permanecem idênticos. A elegibilidade do card e do CTA compartilha uma única verificação em voo; isso impede que os dois observers cancelem a requisição um do outro e produzam subcontagem ou estado divergente.

**TELEMETRIA / PRIVACIDADE:** o denominador novo fecha somente quando analytics devolve `stored:true`, tem dedupe em memória e `sessionStorage`, e não pode ser emitido depois do início do clique ou com botão desabilitado. A metadata é fechada e categórica; reutiliza apenas o identificador pseudônimo de vídeo já existente e não inclui e-mail, roteiro, conteúdo, saldo, preço, URL, Session Stripe ou campo livre.

**MEDIÇÃO / GATE:** preservar `plan_fit_direct_win_v3` e todas as superfícies recentes até pelo menos dez pessoas externas distintas em `plan_fit_checkout_cta_viewed` ou cinco pessoas distintas em `plan_fit_checkout_clicked`, o que ocorrer primeiro. Ler por pessoa `CTA realmente visto → plan_fit_checkout_clicked → checkout_started → payment_success → assinatura ativa → renovação`. Sinal inicial = clique depois de visão válida; resultado financeiro = pagamento reconciliado; resultado final = assinatura ativa/renovada. Parar por evento após clique, botão desabilitado, duplicação, metadata indevida, regressão visual, destino incorreto ou comportamento de checkout alterado.

**TESTADO LOCALMENTE / AUDITORIA:** Plan Fit 374/374; oferta pós-vídeo 57/57; trial balance bridge 208/208; orientação de pagamento 29/29; verdade monetária 306/306. O typecheck repetiu exatamente os quatro erros baseline preexistentes em `app/api/admin/_shared/mrr.ts`, `app/api/me/subscription/route.ts` e `app/api/stripe/checkout/route.ts` ×2; nenhum erro novo. `git -c core.whitespace=cr-at-eol diff --check` ficou limpo. A primeira auditoria B2C deu NO-GO ao encontrar a corrida entre observers; a implementação passou a usar single-flight compartilhado, ganhou testes concorrentes executáveis e a reauditoria final deu **GO**, P0=0 e P1=0. Comparação visual não se aplica porque DOM, copy, estilo, ordem e navegação não mudaram.

**VALIDADO EM PRODUÇÃO (01/09/2026 UTC):** `origin/main` avançou por fast-forward para `905a9e530650ab7707eb12e361fb6bd985933daf`, rebaseado sobre `bc77b635183ad1990e6b016a84204f78dace8221` do Claude sem sobreposição de arquivos. O deploy Vercel `dpl_2NFHzzyzTCEBWJWcvj6CB3AL1xUe` chegou a `READY`, target production, framework Next.js, SHA exato, `aliasError=null` e aliases `www.usekineo.com`/`usekineo.com` ativos. A página pública `/pricing` respondeu normalmente e as consultas de erros/runtime do deployment em 30 minutos retornaram vazias. Nenhum evento, checkout, pagamento, render ou comunicação foi forçado para fabricar amostra. **ESTADO ATUAL:** **IMPLEMENTADO / TESTADO LOCALMENTE / VALIDADO EM PRODUÇÃO / AGUARDANDO AMOSTRA EXTERNA.**

**PRÓXIMA HIPÓTESE REGISTRADA, NÃO IMPLEMENTADA:** a página `/checkout/cancelled` mede todas as visitas com `checkout_cancelled`, mas não possui denominador para a caixa de objeção que só aparece em uma condição específica; hoje existe apenas o evento de clique no motivo. Uma rodada futura pode medir `checkout_cancel_objection_viewed` a 50% sem alterar a UI, preservando-a por dez pessoas externas. Não misturar essa hipótese com o gate atual.

**NÃO TOCADO / COORDENAÇÃO:** preço, desconto, cupom, crédito, validade, trial, SKU, promessa, copy, layout, checkout Stripe/server/webhook, Supabase schema/dados, render, motor, cena, voz, legenda, admin, e-mail, outreach, anúncio ou tráfego ativo. Claude deve executar `git fetch origin` e partir do SHA desta documentação ou da ponta posterior. Não reeditar Plan Fit, primeira entrega, pricing, WelcomeOffer, retorno do checkout ou outra superfície abaixo do próprio gate. A próxima rodada alterna para B2B em uma superfície distinta, sempre subordinada ao placar comum de assinatura real.

## 130. B2B — NO-GO para outra superfície; o gargalo observável é exposição qualificada (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** curto prazo = uma sessão empresarial externa vê uma porta já publicada e escolhe um destino; médio prazo = a mesma sessão ou pessoa produz brief/proposta/plano, chega ao checkout e conclui pagamento; longo prazo = pagamento empresarial vira assinatura ativa, renovação, repetição de pack/Pilot ou indicação. Página, card, impressão, clique, artefato e Checkout Session não são receita.

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** em sete dias, `agency_volume_bridge_viewed` acumulou oito eventos — duas pessoas externas identificadas e seis sessões anônimas — sem linha em `agency_volume_bridge_clicked`. `autopilot_break_even_viewed` permaneceu em três pessoas, `pricing_business_path_viewed` em duas e as páginas de Autopilot em duas; não surgiu compra B2B. O novo rodapé empresarial emitiu quatro views, uma para cada destino, dentro de uma única sessão anônima e sem clique. Essa unidade é `(session_id, destination)`: quatro portas vistas por uma sessão continuam sendo uma sessão alcançada, não quatro pessoas.

**EVIDÊNCIA DE PRODUÇÃO / FUNIL DE PROPOSTA E PACK (Supabase, janela distinta de 30 dias até 01/09/2026 UTC; contas internas excluídas):** somente uma sessão anônima chegou a `agency_bulk_page_viewed` e `agency_margin_calculator_viewed`; não houve pessoa ou sessão em `agency_margin_proposal_copied`, `agency_margin_pack_selected`, `agency_bulk_pack_clicked`, `bulk_checkout_started`, `bulk_purchase_completed` ou qualquer evento `client_short_brief_*`. A ausência de ator depois da calculadora não prova objeção de preço: a amostra observada é uma sessão.

**FATO CONFIRMADO / ANTI-DUPLICAÇÃO:** rodapé empresarial, Agency Volume Bridge, pricing business path, break-even, Autopilot, Pilot resume, proposta/margem, packs, planner, client brief, Publishing Kit, examples, HeyGen, real estate, Trust, parceiros e home router permanecem abaixo de gates documentados. `/tools` já destaca o client brief e reúne planner/local-business/cost tool; criar ali outro seletor creator/business duplicaria o roteador da home e outra ponte duplicaria rodapé/Agency Volume Bridge. `AgencyPacksClient` não repete `entry` no clique do pack, mas a cadeia continua unível por `session_id`; reabrir bulk por essa lacuna antes da amostra violaria o gate sem mudar uma decisão agora.

**AUDITORIA ESPECIALIZADA / VEREDITO:** auditor B2B independente deu **NO-GO para nova intervenção de runtime**. Nenhuma hipótese encontrada cumpriu ao mesmo tempo: superfície distinta, evidência suficiente e ausência de experimento ativo abaixo do gate. A conclusão honesta é que o gargalo B2B imediato observável é exposição qualificada insuficiente, não outra página, CTA ou promessa.

**GATE PRESERVADO:** acumular até 20 sessões externas distintas realmente expostas ao rodapé empresarial ou cinco sessões distintas com clique, o que ocorrer primeiro. Ler por destino `footer_business_path_viewed → footer_business_path_clicked → chegada canônica → artefato/proposta → checkout_started|bulk_checkout_started → payment_success|bulk_purchase_completed → assinatura ativa ou repetição`. Parar por destino quebrado, mais de um evento por sessão/destino, metadata indevida ou navegação afetada. Não somar pessoas identificadas com sessões anônimas como se fossem a mesma régua.

**ESTADO ATUAL:** **DIAGNÓSTICO CONCLUÍDO / NO-GO PARA CÓDIGO / EXPERIMENTOS PRESERVADOS / AGUARDANDO AMOSTRA EXTERNA.** Nenhum código, copy, preço, oferta, banco, checkout, pagamento, tráfego ativo ou comunicação externa foi alterado. A próxima rodada alterna para B2C, onde existe volume observável e uma hipótese distinta já registrada sem edição.

## 131. B2C — primeira entrega deixa de ser sobrescrita pelo Quickstart do ChatGPT (01/09/2026)

**EVIDÊNCIA DE PRODUÇÃO / CAUSA DETERMINÍSTICA (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** depois da fronteira `2026-09-01T11:40:31.028Z`, cinco pessoas externas elegíveis viram a missão `trial_first_seedance_35s_v2`; duas clicaram. As duas emitiram `chatgpt_quickstart_selected`, nenhuma emitiu `trial_first_delivery_generate_committed`, nenhuma concluiu vídeo atribuído à missão e nenhuma chegou a checkout ou pagamento. Uma das duas concluiu por `chatgpt_quickstart_v5`. A amostra não prova preferência: prova que o próprio produto trocou a campanha antes do Generate.

**FATO CONFIRMADO:** o layout persistente montava `TrialActiveBanner` e logo depois `ChatGptWelcomeBanner`. O CTA da missão levava corretamente `intent_campaign=trial_first_seedance_35s_v2`, mas o Quickstart ignorava essa intenção e podia navegar novamente com `intent_campaign=chatgpt_quickstart_v5`. A atribuição era sobrescrita antes de o Studio emitir o commit da missão (`app/(dashboard)/layout.tsx`; `components/ChatGptWelcomeBanner.tsx`; `lib/growth/trialBalanceBridge.ts`).

**HIPÓTESE CAUSAL / IMPLEMENTADO:** o commit funcional `6d3a1f141fe037d652d163da3a51d1805c8d9f54` dá à campanha canônica de primeira entrega propriedade exclusiva somente da rota atual. Nessa URL o Quickstart fica oculto sem gravar dismissal ou impressão; em uma rota comum ele volta a ser elegível e mantém `chatgpt_quickstart_v5`. `useSearchParams` torna a decisão reativa a navegação query-only; uma guarda síncrona evita flash; `Suspense fallback={null}` envolve somente o banner cliente. A política executável importa `TRIAL_FIRST_DELIVERY_VERSION` e não duplica o literal.

**MEDIÇÃO / GATE REINICIADO:** as cinco pessoas contaminadas ficam fora da nova coorte. A fronteira limpa é o deployment pronto em `2026-09-01T17:16:45.551Z`. Preservar até dez pessoas externas elegíveis pós-fronteira e ler por pessoa `trial_active_banner_shown(first_delivery_eligible=true) → trial_first_delivery_clicked → trial_first_delivery_generate_committed → conclusão com intent_campaign da missão → checkout_started → payment_success → assinatura ativa`. Parar se Quickstart reaparecer na URL reservada, a campanha for sobrescrita, o banner não voltar em rota comum, impressão duplicar ou qualquer comportamento de render/crédito/preço/checkout mudar.

**TESTADO LOCALMENTE / AUDITORIA:** Quickstart 103/103; trial balance bridge 208/208; `diff --check` limpo. O typecheck repetiu exatamente os quatro erros baseline em `mrr.ts`, `me/subscription` e `stripe/checkout` ×2; nenhum erro novo. O build Next 14.2.5 compilou com sucesso e parou depois na coleta de `/api/regenerate-scene` porque a worktree isolada não recebe `OPENAI_API_KEY`; nenhum segredo foi lido. A auditoria adversarial primeiro deu NO-GO porque o teste mockava a própria constante; o teste passou a carregar `engineCost` e `trialBalanceBridge` reais e a reauditoria final deu **GO**, P0=0 e P1=0. A comparação desktop/mobile está em `docs/previews/CHATGPT-TRIAL-ARBITRATION-2026-09-01.html`.

**VALIDADO EM PRODUÇÃO (01/09/2026 UTC):** `origin/main` avançou por fast-forward para `6d3a1f141fe037d652d163da3a51d1805c8d9f54`, rebaseado sobre `7f60ab2a` do Claude sem conflito. O deploy Vercel `dpl_DJrW6UVeHe21sEu115nWPJ4fbDKv` chegou a `READY`, target production, framework Next.js, SHA exato, `aliasError=null` e aliases `www.usekineo.com`/`usekineo.com` ativos. Home, Studio reservado e Studio comum responderam HTTP 200 com `X-Matched-Path` correto. O scan error/fatal do deployment retornou vazio. Nenhum evento, render, checkout ou pagamento foi forçado.

**NÃO TOCADO / COORDENAÇÃO:** pipeline de render, cenas, voz, legendas, qualidade, crédito, preço, trial como regra financeira, SKU, promessa, checkout, banco, admin, e-mail, outreach ou anúncio. O gate de primeira entrega começa novamente após a fronteira limpa; não misturar a coorte contaminada.

## 132. B2B — rodapé continua em 5/20 sessões; próximo candidato fica atrás de gate (01/09/2026)

**EVIDÊNCIA DE PRODUÇÃO (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** `footer_business_path_viewed` tem 20 eventos, mas eles representam cinco sessões anônimas distintas vendo quatro destinos cada. `footer_business_path_clicked` permanece sem linha e não há pessoa identificada. A régua correta é **5/20 sessões expostas e 0/5 sessões com clique**, não 20 pessoas.

**FATO CONFIRMADO / DECISÃO:** a unidade do código é `(session_id, destination)` e cada destino é deduplicado separadamente. O rodapé, seus quatro destinos e sua copy permanecem intocados até 20 sessões externas distintas ou cinco sessões distintas com clique. Nova landing, novo CTA ou nova promessa agora duplicaria superfícies abaixo do gate.

**PRÓXIMA HIPÓTESE REGISTRADA, NÃO IMPLEMENTADA:** depois de enviar o formulário mensal da agência, o estado de sucesso diz que o pedido será revisado, mas não repete o caminho imediato para comparar packs já existente acima. Um futuro handoff secundário para `#agency-pack-heading` só fica autorizado por evidência após cinco submissões externas ou três sessões submetidas sem seleção/checkout. Métrica proposta: `b2b_brief_submitted → b2b_brief_self_serve_handoff_clicked → pack_selected|agency_bulk_pack_clicked → bulk_checkout_started → bulk_purchase_completed`. Risco: pack avulso não deve parecer resposta automática para necessidade recorrente.

**ESTADO ATUAL:** **DIAGNÓSTICO CONCLUÍDO / NO-GO PARA CÓDIGO / GATE PRESERVADO / AGUARDANDO AMOSTRA EXTERNA.** Nenhum lead, formulário, checkout, preço, banco, comunicação ou runtime B2B foi alterado.

## 133. B2B — nenhuma porta madura; intenção econômica de clipping fica somente instrumentável (01/09/2026)

**EVIDÊNCIA DE PRODUÇÃO (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** nas últimas 72 horas, `agency_volume_bridge_viewed` alcançou 11 atores externos e zero clique; `autopilot_break_even_viewed`, seis atores; `pricing_business_path_viewed`, três; `agency_bulk_page_viewed`, `agency_margin_calculator_viewed` e `b2b_brief_viewed`, dois atores cada; `business_content_plan_viewed`, um. O rodapé permanece em cinco sessões externas distintas, cada uma vendo quatro destinos, e zero sessão com clique. Não apareceu `agency_margin_proposal_copied`, `agency_margin_pack_selected`, `agency_bulk_pack_clicked`, `bulk_checkout_started`, `bulk_purchase_completed` ou `b2b_brief_submitted`. Os cinco `checkout_started` e um `payment_success` da mesma janela carregaram atribuição `standard`; não foram classificados como resultado B2B.

**EVIDÊNCIA DE PRODUÇÃO / ANTI-DUPLICAÇÃO:** todas as superfícies com evidência real continuam abaixo dos gates documentados nas rodadas anteriores. Outra landing, CTA, promessa ou ligação entre superfícies agora confundiria experimentos sem amostra. A auditoria especializada devolveu **NO-GO para intervenção de runtime**.

**FATO CONFIRMADO / CANDIDATA DISTINTA:** `/make-money-clipping-with-ai` é estática e canônica (`app/make-money-clipping-with-ai/page.tsx:20,28`), está no sitemap (`app/sitemap.ts:133-135`) e no rodapé (`components/Footer.tsx:161-163`). O CTA atual leva ao gerador gratuito com a campanha `earn-angle` (`app/make-money-clipping-with-ai/page.tsx:316-333`), mas não há evento próprio de exposição ou clique. A página também distingue honestamente campanha que exige footage de terceiro de canal próprio gerado por IA (`app/make-money-clipping-with-ai/page.tsx:90`).

**HIPÓTESE NÃO ADMITIDA:** o CTA genérico pode perder o contexto econômico do freelancer antes do signup. O código comprova o caminho; não comprova abandono nem perda de contexto. Por isso não foi criada ponte comercial nem alterada a página.

**SUGESTÃO / PRÓXIMO GATE REVERSÍVEL, NÃO IMPLEMENTADO:** limitar uma futura mudança à instrumentação invisível do CTA existente: `clipping_earn_angle_cta_viewed` quando pelo menos 60% estiver visível e `clipping_earn_angle_cta_clicked`, com dedupe por sessão e metadata categórica fechada. Preservar UI, copy, href e UTMs. Aguardar 20 sessões externas com CTA visto ou cinco sessões distintas com clique. Só admitir uma ponte se houver pelo menos dois clickers distintos e uma progressão na mesma sessão ao gerador/signup. Curto prazo = view → click → generator; médio = primeiro vídeo → pricing/checkout; longo = pagamento → assinatura ativa/renovação. Parar por duplicação, atribuição alterada, falsa promessa para clipping de source footage ou canibalização do CTA B2C.

**ESTADO ATUAL:** **DIAGNÓSTICO CONCLUÍDO / NO-GO PARA CÓDIGO / GATES PRESERVADOS / PRÓXIMA RODADA ROTACIONADA PARA B2C.** Nenhum runtime, checkout, preço, crédito, trial, banco, render, comunicação externa ou tráfego ativo foi alterado.

## 134. B2C — valor percebido passa a significar cinco segundos realmente assistidos (01/09/2026)

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** 16 pessoas externas tiveram a primeira entrega concluída nas 24 horas examinadas; 14 chegaram a `video_ready_viewed`, sete clicaram em download, seis concluíram o download, 12 viram ao menos uma família comercial, duas chegaram a pricing, uma ao checkout e nenhuma a `payment_success`. Quatro não viram família comercial; duas dessas quatro passaram pelo caminho intencional de próximo episódio. Isto não autoriza outra oferta: falta saber se a pessoa realmente consumiu o resultado antes de reagir à oferta existente.

**FATO CONFIRMADO / LACUNA ANTERIOR:** `video_ready_viewed` prova somente `phase=done` com aba visível; o player real não media reprodução. O novo caller está ligado ao `<video>` entregue e usa `playing`, `timeupdate`, pausa, buffering, `visibilitychange`, `seeking`, `seeked` e `ended` (`app/(dashboard)/generate/GenerateClient.tsx:4568-4618`). O acumulador soma progresso decodificado do MP4, limitado pelo tempo visível; atraso da thread sem avanço da mídia, background, pausa e salto de seek não contam (`lib/growth/resultVideoValueSample.ts:128-239`).

**HIPÓTESE CAUSAL NOVA / IMPLEMENTADO:** assistir cinco segundos cumulativos do primeiro vídeo pode ser o mediador entre entrega e ação comercial. O commit funcional `5b06238557e6a44ba32c1a5f9c4fde6e461a309c` adiciona somente o evento invisível `result_video_value_sampled`, versão `result_video_value_sampled_v1`. DOM, copy, controles, autoplay, layout, oferta e fluxo permanecem iguais.

**INTEGRIDADE / PRIVACIDADE:** metadata fechada contém somente versão, `attempt_id` saneado, limiar, faixa de motor, faixa de duração e estado tri-state da primeira entrega. `trackClosedEvent` não herda UTMs livres (`lib/analytics.ts:432-490`). Dedupe usa claim `pending` antes do POST sob Web Lock e latch em `localStorage`; navegador sem coordenação atômica falha fechado. `stored`, `not_stored` e `ambiguous` são distintos: somente falha confirmada libera retry; resposta perdida mantém a claim e impede duplicação (`lib/growth/resultVideoValueSample.ts:59-109`).

**MEDIÇÃO / GATE:** preservar todas as ofertas atuais e aguardar 20 pessoas externas com primeira entrega pós-deploy, com pelo menos cinco `sampled` e cinco `not_sampled`. Contar pessoas distintas, nunca eventos; reconciliar `video_generation_completed → video_ready_viewed → result_video_value_sampled → oferta vista/interação → pricing → checkout_started → payment_success → assinatura ativa`. Associação não será chamada de lift causal. Parar por evento antes de cinco segundos, tempo oculto/pausado/seek contado, duplicação por `attempt_id`, metadata livre ou efeito no player.

**TESTADO LOCALMENTE / AUDITORIA:** sampler e transporte 53/53; Plan Fit 374/374; eventos server-only 33/33; whitespace limpo. O typecheck repetiu exatamente os quatro erros baseline preexistentes em `app/api/admin/_shared/mrr.ts`, `app/api/me/subscription/route.ts` e `app/api/stripe/checkout/route.ts` ×2; nenhum erro novo. Cinco rodadas adversariais bloquearam autoplay sem frame, corrida entre abas, seek, UTM livre, crash entre POST/latch e resposta ambígua; depois das correções, a auditoria final deu **GO**, P0=0 e P1=0. Comparação visual não se aplica porque nenhum DOM, copy, estilo ou comportamento do player mudou.

**VALIDADO EM PRODUÇÃO (01/09/2026 UTC):** `origin/main` avançou por fast-forward até `042df22bb5e1d35dbb547412e424f7b80376c990`, contendo o commit funcional `5b06238557e6a44ba32c1a5f9c4fde6e461a309c`. O deploy Vercel `dpl_6vXqnFA1WsCnFQRt7ymAsHkS4f9F` chegou a `READY`, target `production`, framework Next.js, SHA exato e `aliasError=null`, com `www.usekineo.com` e `usekineo.com`. A home pública respondeu HTTP 200 com assets identificados pelo deployment correto; o scan de runtime desse deployment retornou zero logs `error`/`fatal` na janela de uma hora. Nenhum evento, render, oferta, checkout ou pagamento foi forçado para fabricar prova. **ESTADO ATUAL:** **IMPLEMENTADO / TESTADO LOCALMENTE / AUDITADO COM GO / VALIDADO EM PRODUÇÃO / AGUARDANDO AMOSTRA EXTERNA.** A próxima decisão comercial só ocorre depois do gate externo.

**NÃO TOCADO:** preço, desconto, cupom, crédito, validade, trial, SKU, promessa, checkout Stripe/server/webhook, Supabase schema/dados, render, motor, cena, voz, legenda, admin, e-mail, outreach, anúncio ou tráfego ativo.

## 135. B2B — o CTA econômico de clipping ganha denominador real (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** curto prazo = uma sessão externa realmente vê a ação econômica e escolhe o gerador; médio prazo = a mesma sessão chega a cadastro, primeiro vídeo, pricing e checkout; longo prazo = `payment_success` reconciliado vira assinatura ativa, renovação ou indicação. Impressão, clique, cadastro, vídeo e Checkout Session não são receita.

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO:** a seção 133 mediu em 72 horas 11 atores externos em `agency_volume_bridge_viewed`, seis em `autopilot_break_even_viewed`, três em `pricing_business_path_viewed`, dois em `agency_bulk_page_viewed`, dois em `agency_margin_calculator_viewed`, dois em `b2b_brief_viewed` e um em `business_content_plan_viewed`, sem clique, proposta, pack, checkout ou compra B2B. Todas essas superfícies permanecem abaixo dos próprios gates. A mesma auditoria registrou `/make-money-clipping-with-ai` como candidata distinta porque a página e o CTA já estavam vivos, mas não existia denominador de exposição ou escolha. Nenhum volume da página de clipping foi inventado.

**FATO CONFIRMADO / LACUNA ANTERIOR:** `/make-money-clipping-with-ai` é canônica, estática, está no sitemap e no rodapé. O CTA existente leva a `/free-ai-shorts-generator` com `utm_source=clipping-page`, `utm_medium=seo` e `utm_campaign=earn-angle`, e a própria página separa campanha que exige footage de terceiro de canal próprio gerado por IA. Antes do commit não existia evento próprio de view ou clique, portanto não era possível distinguir “CTA nunca visto” de “CTA visto e recusado” (`app/make-money-clipping-with-ai/page.tsx`; `app/sitemap.ts`; `components/Footer.tsx`).

**HIPÓTESE CAUSAL NOVA / IMPLEMENTADO:** o commit funcional `ba3aa9b93ae6dfa60d49e615c8f28ae670c0618c` publica `clipping_earn_angle_cta_v1`. `clipping_earn_angle_cta_viewed` nasce somente quando ao menos 60% do card existente está visível em aba visível; `clipping_earn_angle_cta_clicked` nasce somente no link canônico já publicado. Copy, estilo, ordem, href efetivo e navegação permanecem iguais; o componente cliente retorna `null` (`app/make-money-clipping-with-ai/ClippingEarnAngleTelemetry.tsx`; `lib/growth/clippingEarnAngleCta.ts`).

**INTEGRIDADE / PRIVACIDADE:** o payload usa `trackClosedEvent` e contém apenas cinco valores categóricos: versão, superfície, placement, destino e intenção. Não herda UTM livre e não envia tópico, roteiro, URL, e-mail ou texto do visitante. Uma claim `pending` é escrita e relida em `sessionStorage` antes do POST; `stored` finaliza, `ambiguous` permanece fechado e somente `not_stored` libera uma segunda tentativa limitada. Sem storage confiável o experimento falha fechado com zero POST, evitando linha pública sem unidade e duplicação por remount.

**MEDIÇÃO / GATE:** preservar UI e destino até 20 sessões externas distintas com CTA visto ou cinco sessões distintas com clique, o que ocorrer primeiro. Sinal inicial = pelo menos dois clickers distintos e uma progressão na mesma sessão a gerador/signup. Ler `clipping_earn_angle_cta_viewed → clipping_earn_angle_cta_clicked → free generator/signup → primeiro vídeo → pricing → checkout_started → payment_success → assinatura ativa`. Sessão anônima nunca será chamada de pessoa. Parar por duplicação, evento abaixo de 60%, aba oculta, metadata livre, atribuição/href alterado, conteúdo incompatível com campaign clipping ou regressão do CTA B2C.

**TESTADO LOCALMENTE / AUDITORIA:** contrato executável 83/83; verdade orgânica 36/36; premium-first 24/24; eventos server-only 33/33 — 176 verificações. O typecheck repetiu exatamente os quatro erros baseline preexistentes em `mrr.ts`, `me/subscription` e `stripe/checkout` ×2; nenhum erro novo. O primeiro parecer independente deu NO-GO porque storage bloqueado ainda permitia POST sem unidade confiável e o teste não cobria remount. O recorder passou a falhar fechado, ganhou claim com readback, duas instâncias concorrentes e política de viewport executada; a reauditoria final deu **GO**, P0=0 e P1=0. Whitespace limpo.
...
**NÃO TOCADO / COORDENAÇÃO:** preço, desconto, cupom, crédito, validade, trial, SKU, promessa nova, checkout Stripe, webhook, pricing, home, StructuredData, Supabase schema/dados, dashboard autenticado, render, motor, cena, voz, legenda, admin, e-mail, outreach, anúncio ou tráfego ativo. Não implementar `product_script_generated` nesta rodada; o auditor classificou-o como candidata posterior sem volume atual comprovado.

## 136. Assinatura — verdade de moeda antes do cartão (01/09/2026)

**FATO CONFIRMADO / CONTRADIÇÃO:** a fonte canônica admite somente `CheckoutCurrency = 'usd'`, o resolver devolve USD para qualquer país e `CURRENCY_DISPLAY` contém apenas USD (`lib/checkoutPricing.ts:49,607-616`). Apesar disso, a home e o FAQ JSON-LD diziam que o preço seria exibido na moeda local; o pricing repetia a promessa no FAQ e na nota acima dos planos; a calculadora SEO, o pricing embutido e dois fallbacks pós-vídeo também falavam em preço local. A promessa e o caixa discordavam exatamente no último metro.

**HIPÓTESE CAUSAL, NÃO PROVADA:** para uma pessoa internacional, especialmente vinda de recomendação do ChatGPT, encontrar USD depois de ler “local currency” pode produzir surpresa, medo de conversão/taxa e quebra de confiança. O padrão é compatível com abandono no checkout, mas este commit não chama associação de causa nem inventa contagem.

**IMPLEMENTADO / MUDANÇA MÍNIMA REVERSÍVEL:** `CHECKOUT_CURRENCY_DISCLOSURE` deriva o rótulo USD do resolver canônico e declara uma única verdade: “Kineo lists and charges plan prices in USD worldwide. Your bank may convert the charge to your local currency and may add conversion or cross-border fees.” A formulação fala da moeda, sem alegar que cupom ou promoção não pode mudar o valor. Home, FAQ JSON-LD, pricing público, FAQ do pricing, pricing embutido e fallbacks pós-vídeo importam a mesma constante (`lib/marketingPrice.ts:276-285`; `app/KineoLanding.tsx:1545`; `components/StructuredData.tsx:242`; `app/pricing/PricingClient.tsx:105,806`; `components/PricingCards.tsx:276`; `app/(dashboard)/generate/GenerateClient.tsx:12941,13189`). A página `/cheapest-ai-shorts-maker` e seu loading state agora nomeiam USD. Nenhum número foi redigitado.

**MEDIÇÃO / FRONTEIRA NOVA:** `pricing_currency_resolved` já registra `currency=usd`, região e país. Preservar ofertas e medir pessoas externas pós-deploy no caminho `primeiro vídeo → pricing_currency_resolved(usd) → checkout_started → payment_success → assinatura ativa`, separando não-EUA. Ler no primeiro de dois gates: 20 pessoas externas não-EUA expostas ao pricing ou 10 Checkout Sessions pós-deploy. Não chamar sessão, clique ou checkout de receita; não atribuir causalidade a before/after sem controle.

**TESTADO LOCALMENTE / AUDITORIA:** `test-checkout-currency-truth.mjs` 81/81; `test-money-truth-contract.mjs` 306/306; invariantes canônicos de pricing verdes; varredura runtime deixa apenas a menção legítima de que o banco pode converter; whitespace limpo. O typecheck repetiu exatamente os quatro erros baseline preexistentes em `mrr.ts`, `me/subscription` e `stripe/checkout` ×2; nenhum erro novo. A revisão React não encontrou hook, fetch, estado, DOM ou custo de bundle novo além da constante já importada por essas superfícies. O primeiro parecer independente deu NO-GO por wording absoluto incompatível com cupons e por tocar uma rota admin da pista Claude. A frase passou a afirmar somente moeda; a rota admin saiu semanticamente e do índice. Reauditoria limitada: **GO, P0=0, P1=0**.

**COMPARAÇÃO VISUAL:** `docs/previews/CHECKOUT-CURRENCY-TRUTH-2026-09-01.html` contém antes/depois de home FAQ, pricing header/FAQ, pricing embutido, pós-vídeo e calculadora em desktop/mobile. O controlador do Chrome encerrou por falha local de acesso, portanto nenhuma inspeção no Chrome será falsamente declarada; o artefato foi aberto no painel Codex para revisão visual do fundador.

**VALIDADO EM PRODUÇÃO (01/09/2026 UTC):** `origin/main` avançou por fast-forward até o commit funcional `a837fa141fcb9e0628e46aa636751e314f281745`. O deploy Vercel `dpl_2ao1JzH6qc4pnpNXqagr1LnfSMCW` chegou a `READY`, target `production`, SHA exato, `aliasError=null` e aliases `www.usekineo.com` e `usekineo.com`. Home, `/pricing` e `/cheapest-ai-shorts-maker` responderam HTTP 200 com o marcador desse deployment. A home publicou a nova frase tanto no FAQ visível quanto no JSON-LD; o HTML contém quatro ocorrências compiladas da constante e nenhuma da promessa antiga. Pricing e calculadora publicaram USD e eliminaram as frases locais. O scan do deployment retornou zero logs `error`/`fatal` em 30 minutos. Nenhum checkout, pagamento ou evento foi forçado para fabricar prova. **ESTADO ATUAL:** **IMPLEMENTADO / TESTADO LOCALMENTE / AUDITADO COM GO / VALIDADO EM PRODUÇÃO / AGUARDANDO GATE EXTERNO.**

**FATO CONFIRMADO / COMPLEMENTO PÓS-DEPLOY:** a validação do HTML publicado encontrou uma segunda formulação imprecisa no metadata de pricing: “the same price worldwide”. A frase não prometia moeda local, mas confundia igualdade de preço com moeda de cobrança e não explicava conversão bancária. A varredura completa encontrou a mesma família em metadata, FAQs orgânicos, comparações, mensagens de limite gratuito, facts feed e explicação de comissão de afiliados. O complemento substitui somente essa família por “charged in USD worldwide” ou “plan prices are charged in USD worldwide”; valores, cupons, oferta e comportamento permanecem idênticos.

**TESTADO LOCALMENTE / AUDITORIA DO COMPLEMENTO:** o contrato de moeda agora varre todos os 740 arquivos `.ts`/`.tsx` em `app`, `components` e `lib`, além das 81 invariantes anteriores: **821/821**. A busca adversarial retornou zero “same price worldwide” e zero promessa ativa de moeda local; verdade monetária permaneceu 306/306; whitespace limpo. O typecheck repetiu somente os quatro erros baseline já documentados. O preview passou a cobrir em pares FAQ/long-tail, comparação, limite gratuito, parceiros e metadata invisível. A primeira auditoria deu NO-GO apenas pela cobertura visual agregada; depois da separação por classe, a reauditoria deu **GO, P0=0, P1=0**.

**VALIDADO EM PRODUÇÃO / COMPLEMENTO (01/09/2026 UTC):** o commit funcional `1cc7f1e4fd6c394774f3cb22d79c787f25fe0a08` entrou em `origin/main` por fast-forward depois de rebase limpo sobre cinco commits do Claude sem arquivo sobreposto. O deployment `dpl_BZ4bqKa4soxFHeb1NBBxMsaKruqt` chegou a `READY`, target production, SHA exato, `aliasError=null` e aliases canônicos ativos. `/pricing` e `/partners` responderam HTTP 200 com o marcador do deployment; ambas publicaram “charged in USD worldwide”, nenhuma publicou “same price worldwide” nem promessa de troca automática para moeda local. O scan de runtime do deployment retornou zero `error`/`fatal` em 30 minutos. Nenhum checkout, pagamento, cupom ou evento foi forçado. **ESTADO DO COMPLEMENTO:** **IMPLEMENTADO / TESTADO LOCALMENTE / AUDITADO COM GO / VALIDADO EM PRODUÇÃO / AGUARDANDO GATE EXTERNO.**

**NÃO TOCADO:** valores, desconto, cupom, crédito, validade, trial, grant, SKU, moeda cobrada, `lib/checkoutPricing.ts`, rota Stripe, rota admin aposentada da campanha Índia, webhook, assinatura existente, banco, Auth, render, motor, cena, voz, legenda, e-mail, outreach, anúncio ou tráfego ativo. O texto residual da rota admin foi registrado para o Claude, dono dessa pista.

## 137. B2B — NO-GO para outra superfície; preservar os experimentos até exposição qualificada (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** curto prazo = uma sessão empresarial externa vê uma porta já publicada e escolhe um destino; médio prazo = a mesma sessão ou pessoa produz artefato, proposta ou plano e chega ao checkout; longo prazo = pagamento reconciliado vira assinatura ativa, renovação, repetição paga ou indicação. Evento, sessão, cópia de snippet, clique e Checkout Session não são cliente nem receita.

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas excluídas):** em 72 horas, `agency_volume_bridge_viewed` alcançou 13 atores externos — duas pessoas identificadas e 11 sessões anônimas —; `autopilot_break_even_viewed` alcançou sete atores — cinco pessoas identificadas e duas sessões anônimas. O rodapé empresarial gerou 24 eventos em seis sessões anônimas porque cada sessão pode ver quatro destinos: a unidade correta permanece seis sessões, não 24 pessoas. `agency_bulk_page_viewed`, `agency_margin_calculator_viewed` e `b2b_brief_viewed` alcançaram duas sessões anônimas cada; `affiliate_business_recruitment_viewed`, uma; `autopilot_page_arrived` e `autopilot_page_viewed`, uma pessoa identificada; `business_content_plan_viewed`, uma sessão anônima. Não apareceu clique, cálculo, submissão, pack, checkout ou compra B2B no conjunto reconciliado. Um `payment_success` externo existe na janela, mas não possui atribuição B2B nesta leitura e não é reivindicado como resultado desta pista.

**EVIDÊNCIA DE PRODUÇÃO / EXPOSIÇÃO DE ENTRADA (janela distinta de sete dias até 01/09/2026 UTC; contas internas conhecidas excluídas):** `/partners` recebeu sete sessões de entrada; `/product-to-video-script`, três; `/business-video-content-plan`, `/real-estate-video-maker`, `/trust`, `/tools` e `/ai-shorts-for-agencies`, duas cada. Essas sessões não são somadas às pessoas identificadas e não provam intenção comercial. O volume confirma que as páginas empresariais ainda não têm exposição suficiente para justificar outra oferta, landing ou CTA concorrente.

**FATO CONFIRMADO / ANTI-DUPLICAÇÃO:** afiliados, recrutamento empresarial em `/partners`, Agency Volume Bridge, rodapé empresarial, packs, margem/proposta, brief, planner, Autopilot, Trust, real estate, examples, Publishing Kit, text-to-video e clipping já possuem superfícies ou gates ativos documentados. Reeditar qualquer uma antes do denominador mínimo apagaria a fronteira do experimento e confundiria falta de exposição com rejeição de oferta.

**HIPÓTESE NOVA AUDITADA / NO-GO:** o `/widget` público poderia ser uma porta B2B para donos de sites e agências. Ele já é alcançável por rodapé, sitemap, facts e `llms.txt`, oferece iframe/link/badge e atribui tráfego por `utm_source=widget`. Entretanto, os botões apenas copiam o snippet; não existe identificador de instalação que una o copiador ao tráfego posterior, e copiar não prova instalação. O widget afiliado já tem adoção desconhecida abaixo do próprio gate. Instrumentá-lo agora criaria outro numerador raso sem decisão confiável rumo à assinatura. Auditoria B2B independente: **NO-GO para runtime**, P0=0 e P1=0 porque nenhum diff foi admitido.

**SUGESTÃO / GATE FUTURO, NÃO IMPLEMENTADO:** somente se surgir volume real em `/widget`, limitar a mudança a `app/widget/**` e `lib/growth/**`, sem tocar no `CopyButton` compartilhado, copy, href ou snippets. Medir `public_widget_install_option_viewed` com 60% visível e `public_widget_snippet_copied` somente após cópia confirmada. Gate: 20 sessões externas com bloco visto ou cinco sessões distintas com cópia; sinal mínimo: dois copiadores distintos e ao menos uma chegada externa por `utm_source=widget|badge`. Mesmo nesse cenário, cópia continua não sendo instalação, cliente ou receita.

**ESTADO ATUAL:** **DIAGNÓSTICO CONCLUÍDO / NO-GO PARA CÓDIGO / EXPERIMENTOS PRESERVADOS / AGUARDANDO AMOSTRA EXTERNA.** Nenhum runtime, UI, copy, preço, oferta, crédito, checkout, banco, render, tráfego ativo ou comunicação externa foi alterado. A próxima rodada alterna para B2C, onde há volume observável e o objetivo continua sendo pagamento e assinatura reais.

## 138. B2C — a caixa de objeção do checkout ganha denominador real (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** curto prazo = uma pessoa que voltou do Stripe realmente vê a pergunta de objeção e escolhe ou ignora uma resposta; médio prazo = a mesma pessoa usa a resposta, retoma checkout e paga; longo prazo = pagamento reconciliado vira assinatura ativa, renovação ou indicação. Visita, visão, motivo, clique e Checkout Session não são receita.

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO (Supabase, SELECT somente leitura, sete dias até 01/09/2026 UTC; contas internas excluídas):** 14 pessoas externas iniciaram checkout. Cinco pessoas externas emitiram dez `checkout_cancelled`; uma clicou no retry, nenhuma registrou `checkout_cancel_reason`, nenhuma emitiu `checkout_downshift_offer_viewed` e nenhuma pagou depois do cancelamento dentro da sequência reconciliada. O único `payment_success` externo da janela pertence a outra sequência e não é reivindicado como recuperação. Depois da fronteira do downshift de 30/08, houve somente uma pessoa cancelada e ela voltava do Starter, que não possui degrau recorrente menor; portanto o gate do downshift continua sem pessoa elegível observada.

**FATO CONFIRMADO / LACUNA:** `/checkout/cancelled` registrava toda visita, mas a caixa `What stopped you?` aparece somente depois da verdade do trial, abaixo das ações principais. Existia evento apenas quando um motivo era escolhido; zero resposta não distinguia “a caixa nunca entrou no viewport” de “a pessoa viu e recusou responder”. Retorno correto, trial-first, downshift, comparação, dúvidas e retomada já existiam e permaneceram intactos (`app/checkout/cancelled/page.tsx`).

**HIPÓTESE CAUSAL / IMPLEMENTADO:** o commit funcional `9159c6c58fbe1b0ca188330a1893d46fa0352c11` publica `checkout_cancel_objection_visibility_v1`. `checkout_cancel_objection_viewed` nasce somente quando pelo menos 50% da caixa real está visível, a aba está visível, a verdade do trial já resolveu para `checkout` e nenhum motivo foi escolhido. A escolha é interceptada em capture phase apenas para encerrar observer/retry; não bloqueia nem altera o handler React. DOM recebeu somente identificadores técnicos sem efeito visual; copy, ordem, CTA, preço, destino e checkout permanecem iguais.

**INTEGRIDADE / PRIVACIDADE:** metadata fechada contém exatamente versão, superfície, tier, billing, produto categórico e disponibilidade booleana de downshift. Não herda UTM livre nem inclui e-mail, URL, preço, valor, texto, motivo escolhido, Session Stripe ou conteúdo do cliente. A claim por sessão e contexto é gravada e relida antes do POST; `stored` fecha, `not_stored` permite uma única tentativa posterior e `ambiguous` permanece fechado. Storage indisponível falha fechado. Unmount ou escolha de motivo encerra o lifecycle antes de timer, observer e listener; resposta tardia não pode registrar visão na rota seguinte.

**MEDIÇÃO / GATE:** preservar a tela inteira até dez pessoas externas distintas com visão válida. Ler por pessoa `checkout_cancel_objection_viewed → checkout_cancel_reason → CTA específico da resposta|retry → checkout_started → payment_success → assinatura ativa`. Separar Starter, Creator, Studio e Autopilot; não comparar eventos brutos. Parar por evento antes de 50%, aba oculta, após escolha, duplicação, metadata livre, alteração de UI ou qualquer checkout criado sem ação da pessoa.

**TESTADO LOCALMENTE / AUDITORIA:** contrato novo 77/77; checkout cancelado 39/39; checkout salvo 47/47; eventos server-only 33/33; verdade monetária 306/306; whitespace limpo. O typecheck repetiu exatamente os quatro erros baseline preexistentes em `app/api/admin/_shared/mrr.ts`, `app/api/me/subscription/route.ts` e `app/api/stripe/checkout/route.ts` ×2; nenhum erro novo. A primeira auditoria independente deu NO-GO por uma corrida de resposta `not_stored` depois do unmount; lifecycle, geometria e retry passaram a falhar fechado e ganharam casos adversariais executáveis. Reauditoria final: **GO, P0=0 e P1=0**.

**COMPARAÇÃO VISUAL:** não aplicável. A entrega não muda copy, layout, cor, tamanho, ordem, CTA ou navegação; fabricar um before/after diferente inventaria uma mudança visual inexistente.

**VALIDADO EM PRODUÇÃO (01/09/2026 UTC):** o deployment Vercel `dpl_95RTyop7vRGaVd5SSi9S7vuYnBbo` chegou a `READY`, target production, framework Next.js, SHA funcional exato, `aliasError=null` e aliases `www.usekineo.com`/`usekineo.com` ativos. GET sem JavaScript em `/checkout/cancelled?tier=starter&billing=monthly` respondeu HTTP 200 com `X-Matched-Path=/checkout/cancelled`; o chunk publicado dessa rota está preso ao deployment e contém `checkout_cancel_objection_visibility_v1`. O scan de runtime `error|fatal` do deployment em 30 minutos retornou vazio. Nenhum navegador executou a tela, nenhum evento foi fabricado e nenhum checkout ou pagamento foi aberto para forçar amostra. **ESTADO ATUAL:** **IMPLEMENTADO / TESTADO LOCALMENTE / AUDITADO COM GO / VALIDADO EM PRODUÇÃO / AGUARDANDO DEZ PESSOAS EXTERNAS.**

**NÃO TOCADO:** preço, desconto, cupom, crédito, validade, trial, SKU, promessa, resposta visual da objeção, checkout server, Stripe, webhook, banco, Auth, render, motor, cena, voz, legenda, admin, e-mail, outreach, anúncio ou tráfego ativo. A próxima rodada alterna para B2B e preserva todos os gates ativos.

## 139. B2B — a ponte já existe; a amostra por entrada ainda não autoriza outra intervenção (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** curto prazo = uma sessão empresarial externa vê uma ponte existente e escolhe o destino; médio prazo = chega à oferta, pack ou checkout; longo prazo = pagamento reconciliado vira assinatura ativa, repetição paga ou indicação. Uma pessoa identificada e uma sessão anônima permanecem unidades separadas; evento, impressão, visita e checkout não são receita.

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO POR ENTRADA (Supabase, SELECT somente leitura, sete dias até 01/09/2026 UTC; contas internas conhecidas excluídas):** a versão `agency_volume_bridge_visibility_v1` registrou 16 exposições ator×entrada, distribuídas em `home` 4, `pricing` 4, `state_report` 4, `cost_page` 3 e `kineo1_engine` 1. Em nenhuma entrada apareceu `agency_volume_bridge_clicked`, chegada posterior à página de agência pelo mesmo ator, intenção de checkout ou pagamento. Os 16 pares não são chamados de 16 pessoas: o agregado anterior permanece 13 atores externos porque uma mesma pessoa ou sessão pode ver mais de uma entrada.

**EVIDÊNCIA DE PRODUÇÃO / DESTINO B2B:** na mesma janela, `agency_bulk_page_viewed` teve três sessões anônimas — entradas `direct`, `home` e `pricing`, uma cada — e `business_content_plan_viewed` teve uma sessão anônima. Não apareceu pack clicado, checkout bulk ou compra bulk. O único `payment_success` externo da janela não possui cadeia B2B e não é reivindicado como resultado. A ausência de clique na ponte e a presença isolada no destino não serão fundidas por inferência.

**FATO CONFIRMADO / ANTI-DUPLICAÇÃO:** `/product-to-video-script` já gera o artefato por `/api/demo-script`, preserva o roteiro até signup/generate e oferece a saída empresarial canônica com `agencyPacksHref('product_tool')` (`app/product-to-video-script/ProductToVideoClient.tsx`; `lib/growth/productToVideo.ts`; `lib/agencyDistribution.ts`). `/comment-to-video`, o Publishing Kit, o planner empresarial, o brief de cliente e as pontes em home, pricing, relatório, custo e página de motor também já possuem destinos ou gates próprios. A hipótese “há uma ferramenta empresarial sem caminho B2B” está contradita pelo código atual.

**HIPÓTESE CAUSAL / NÃO PROVADA:** zero clique pode significar oferta irrelevante, posição fraca ou simplesmente amostra insuficiente. Com no máximo quatro atores por entrada, o dado não separa essas causas. Reeditar copy, CTA, destino ou criar outra landing agora contaminaria a fronteira e transformaria falta de denominador em falsa otimização.

**GATE / PRÓXIMA DECISÃO:** preservar cada entrada até 20 atores externos com visão válida, conforme `AGENCY_BRIDGE_GATE_ACTORS_PER_ENTRY`. Reconciliar por entrada `agency_volume_bridge_viewed → agency_volume_bridge_clicked → agency_bulk_page_viewed → agency_bulk_pack_clicked|bulk_checkout_started → bulk_purchase_completed|payment_success`. Só reabrir a entrada que atingir o gate; se houver 20 visões e zero clique, investigar mensagem/posição; se houver clique sem chegada, investigar navegação/telemetria; se houver chegada sem checkout, investigar oferta; se houver checkout sem pagamento, investigar o último metro sem atribuir automaticamente a preço.

**ESTADO ATUAL:** **DIAGNÓSTICO CONCLUÍDO / NO-GO PARA NOVO RUNTIME / GATES PRESERVADOS / PRÓXIMA RODADA ROTACIONADA PARA B2C.** Nenhum código, UI, copy, preço, oferta, crédito, trial, checkout, banco, render, comunicação externa ou tráfego ativo foi alterado.

## 140. B2C — separar “baixou e gostou” de “baixou e não gostou” antes de criar outra oferta (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** curto prazo = uma pessoa que baixou o resultado realmente vê a caixa de avaliação e responde ou ignora; médio prazo = avaliação declarada separa problema de qualidade de problema comercial; longo prazo = pessoas satisfeitas avançam a pricing, checkout e pagamento, enquanto avaliações baixas voltam para a pista de produto. Download, visão, nota e checkout não são receita.

**EVIDÊNCIA DE PRODUÇÃO / JANELAS SEPARADAS:** a seção 134 registrou 16 pessoas externas com primeira entrega nas 24 horas auditadas, sete cliques de download, seis downloads concluídos, duas chegadas a pricing, uma ao checkout e zero pagamento. Em uma leitura distinta de 72 horas feita em 01/09/2026 UTC, contas internas excluídas, 40 pessoas externas emitiram `video_generation_completed` e `video_ready_viewed`, 13 chegaram a `pricing_view`, três a `checkout_started` e uma a `payment_success`. A segunda janela não é chamada de coorte de primeira entrega nem somada à primeira.

**FATO CONFIRMADO / LACUNA:** `VideoRatingAsk` já aparece somente depois do download e já grava `video_rated`, porém `video_rating_shown` nascia quando o estado React tornava o componente renderizável. A caixa real fica abaixo das ações pós-vídeo e o evento não provava viewport; portanto zero resposta misturava “viu e ignorou” com “nunca rolou até a pergunta” (`components/VideoRatingAsk.tsx`; `app/(dashboard)/generate/GenerateClient.tsx`).

**HIPÓTESE CAUSAL NOVA / IMPLEMENTADO:** o commit funcional `554d85c1898a848f5980b766f0b84de10bdd7458` publica `video_rating_prompt_visibility_v1`. O evento invisível `video_rating_prompt_viewed` nasce somente depois de download confirmado, sem resposta anterior, quando pelo menos 50% da própria caixa permanece visível por um segundo contínuo em aba visível. Cair para 49%, ocultar a aba, responder ou desmontar reinicia/cancela o dwell. `video_rating_shown`, `video_rated`, copy, estrelas, motivos, posição, estilo e CTA externo permanecem intactos.

**INTEGRIDADE / PRIVACIDADE:** metadata fechada contém exatamente versão, `surface=post_download_rating` e `render_count_bucket=first|repeat`; não inclui vídeo, título, nota, motivo, e-mail, URL, texto, preço, sessão ou UTM. Uma claim `pending` em `sessionStorage` é gravada e relida antes do POST. `stored` finaliza, `ambiguous` permanece fechado e somente `not_stored` confirmado admite uma tentativa posterior, sempre após novo dwell completo e somente com lifecycle vivo. Storage indisponível ou claim não verificável falha fechado com zero POST.

**MEDIÇÃO / GATE:** preservar UI e ofertas até dez pessoas externas distintas com `video_rating_prompt_viewed` e exigir pelo menos cinco `video_rated` depois da visão válida. Reconciliar por pessoa `video_downloaded → video_rating_prompt_viewed → video_rated → pricing_view → checkout_started → payment_success → assinatura ativa`. Se menos de cinco responderem, o prompt/posição não produz sinal. Se predominarem notas 1–3, a próxima ação pertence à qualidade do produto, não a outra oferta. Só se predominarem notas 4–5 e essas pessoas não avançarem a pricing/checkout nasce uma hipótese comercial separada.

**TESTADO LOCALMENTE / AUDITORIA:** contrato executável 84/84; eventos server-only 33/33; whitespace limpo. O typecheck retornou somente os quatro erros baseline em `app/api/admin/_shared/mrr.ts`, `app/api/me/subscription/route.ts` e `app/api/stripe/checkout/route.ts` ×2. A auditoria adversarial final confirmou dwell contínuo, captura da resposta antes do handler React, resposta tardia, concorrência, remount e estados `stored|not_stored|ambiguous`: **GO, P0=0 e P1=0**.

**COMPARAÇÃO VISUAL:** não aplicável. O DOM ganhou apenas `ref` e atributos técnicos sem efeito; copy, layout, cor, tamanho, ordem, CTA e navegação não mudaram.

**VALIDADO EM PRODUÇÃO (01/09/2026 UTC):** o deployment Vercel `dpl_8FA9XTPoW4Kj7XbfVWSXooDesZV1` chegou a `READY`, target production, framework Next.js, SHA exato `554d85c1`, `aliasError=null` e aliases canônicos `www.usekineo.com`/`usekineo.com`. O scan de runtime `error|fatal` do deployment em 30 minutos retornou vazio. Nenhum download, rating, checkout, pagamento ou evento foi forçado para fabricar amostra. **ESTADO ATUAL:** **IMPLEMENTADO / TESTADO LOCALMENTE / AUDITADO COM GO / VALIDADO EM PRODUÇÃO / AGUARDANDO DEZ PESSOAS EXTERNAS E CINCO RESPOSTAS.**

**NÃO TOCADO / COORDENAÇÃO:** render, motor, cena, voz, legenda, conteúdo do vídeo, estado de geração, preço, desconto, cupom, crédito, validade, trial, SKU, oferta, checkout Stripe, webhook, banco, Auth, e-mail, outreach, anúncio ou tráfego ativo. Esta entrega mede uma variável de conversão; qualquer achado de qualidade deve ser passado ao Claude sem o Codex editar o pipeline.

## 141. B2B — páginas públicas de vídeo têm volume, mas não autorizam uma segunda oferta (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** uma página compartilhada pode distribuir prova e levar a uma próxima ação, mas sessão, viewport, clique e roteiro gratuito não são lead empresarial, cliente ou receita. Uma oferta B2B só entra nessa superfície com evidência de intenção empresarial; não se infere agência a partir de quem assistiu a um vídeo.

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO CORRIGIDA (Supabase, SELECT somente leitura em 01/09/2026 UTC, janela de sete dias; contas internas conhecidas excluídas):** 96 sessões externas distintas entraram por caminhos `/v/*`. As 54 sessões com `public_video_cta_viewed` ocorreram integralmente entre 25 e 27/08 e carregavam `destination=/signup`: pertencem ao CTA antigo, não à variante `public_video_remix`. Depois da mudança de 27/08, houve 12 sessões de entrada — cinco em 28/08, uma em 31/08 e seis em 01/09 —, mas zero viewport válido do CTA, zero clique e zero chegada ao remix. Essas 12 sessões podem incluir crawler, navegador sem JavaScript ou abandono antes de uma impressão válida; não são chamadas de 12 pessoas e não provam rejeição da oferta nova.

**FATO CONFIRMADO / ANTI-DUPLICAÇÃO:** `/v/[id]` já possui quatro posições instrumentadas com `PublicVideoCtaLink`, denominador por viewport e destino único para `/free-script-generator`; a campanha `public_video_remix` preserva origem e `source_video_id` sanitizado. A etapa gratuita já mede chegada, geração de roteiro e intenção de cadastro (`components/PublicVideoCtaLink.tsx`; `app/v/[id]/page.tsx`; `lib/publicVideoRemix.ts`; `app/free-script-generator/FreeScriptClient.tsx`). O funil foi transformado de signup imediato em remix sem cadastro nos commits já registrados na seção 2.9. Portanto, “adicionar uma porta de valor antes do cadastro” não é hipótese nova.

**FATO CONFIRMADO / CONTINUIDADE B2B AUDITADA:** `/product-to-video-script` executa `/api/demo-script`, entrega um roteiro de cinco blocos, preserva o artefato no caminho creator até signup/generate e oferece também o destino empresarial canônico por `agencyPacksHref('product_tool')`. O destino lê e registra `entry=product_tool`. `/comment-to-video` e Publishing Kit também já preservam contexto e têm caminhos B2B. Não foi encontrada quebra técnica entre artefato e oferta que justificasse runtime novo.

**CONTRADIÇÃO CORRIGIDA / HIPÓTESE NÃO PROVADA:** a primeira leitura desta seção juntou o denominador antigo ao destino novo e, por isso, sugeria falsamente 54 visões da variante atual. A separação por dia e `metadata.destination` contradiz essa atribuição. A variante nova ainda não possui exposição humana válida; acrescentar um CTA “for agencies” dividiria a atenção, contaminaria o experimento B2C existente e chamaria audiência de intenção empresarial sem evidência.

**GATE / PRÓXIMA DECISÃO:** **NO-GO para CTA, copy, evento ou landing B2B nova nessas páginas.** Preservar o experimento público atual até 20 sessões externas com `public_video_cta_viewed(destination=/free-script-generator)` ou cinco cliques, o que ocorrer primeiro. Só então formular uma hipótese B2C sobre o CTA único; nunca usar o denominador antigo nem inferir intenção B2B. Para `product_tool`, preservar o gate da seção 139 e só reabrir com volume externo suficiente para localizar abandono em `entrada → artefato → CTA creator|B2B → agency_bulk_page_viewed(entry=product_tool) → pack|checkout → pagamento`.

**ESTADO ATUAL:** **DIAGNÓSTICO CONCLUÍDO / NO-GO PARA CÓDIGO / AMOSTRAS E EXPERIMENTOS PRESERVADOS / PRÓXIMA RODADA ROTACIONADA PARA B2C.** Nenhum runtime, UI, copy, preço, oferta, crédito, trial, checkout, banco, render, comunicação externa ou tráfego ativo foi alterado.

## 142. B2C — a prova própria ainda não atingiu o gate por estado; nenhuma nova caixa entra na pricing (01/09/2026)

**EVIDÊNCIA DE PRODUÇÃO / JANELAS SEPARADAS (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas conhecidas excluídas):** na janela de sete dias, 34 pessoas identificadas chegaram a `pricing_view`. Antes da primeira chegada, 22 tinham zero vídeo concluído, nove tinham um, duas tinham dois ou três e uma tinha quatro ou mais. Os grupos produziram respectivamente quatro, quatro, zero e zero pessoas em `checkout_started`; o único `payment_success` pertenceu ao grupo de um vídeo. Esta janela inclui dias anteriores à prova própria e não mede lift da variante.

**EVIDÊNCIA DE PRODUÇÃO / FRONTEIRA RECENTE:** desde 31/08 UTC, dez pessoas identificadas chegaram a pricing: quatro com zero vídeo, três com um, duas com dois ou três e uma com quatro ou mais. `pricing_journey_proof_viewed` alcançou duas das quatro pessoas sem entrega, as três com um vídeo e uma das duas com dois ou três; somente uma pessoa do grupo de um vídeo chegou ao checkout e pagou. Por estado do componente, `before_first_delivery` tem duas pessoas e `after_delivery` tem quatro, sendo três no bucket `1` e uma no `2_4`. Nenhum estado atingiu o gate independente de cinco pessoas da seção 81; o pagamento observado não é atribuído causalmente ao card.

**FATO CONFIRMADO / ANTI-DUPLICAÇÃO:** sampler de valor, avaliação pós-download, oferta pós-vídeo, prova própria, Plan Fit, intenção de tier, checkout salvo/retomado, orientação Stripe, verdade USD e objeção de cancelamento possuem gates ativos. `/my-videos` contém checkout instrumentado, mas está órfã da navegação atual, que aponta para `/history`; `/account` possui dois links comerciais para pricing, porém não tem denominador confiável por pessoa. Abrir runtime numa superfície sem volume apenas criaria mais um numerador.

**HIPÓTESE NOVA AUDITADA / NÃO PROVADA:** pessoas que abrem Billing/Usage depois da entrega podem constituir um segmento de consideração diferente e hoje perdem a origem ao chegar a pricing. Sem um denominador de uso de `/account`, essa hipótese não autoriza probe, copy ou CTA.

**GATE / PRÓXIMA DECISÃO:** **NO-GO para nova UI ou telemetria B2C nesta rodada.** Preservar `pricing_journey_proof_v1` até cinco pessoas externas em cada estado. Em leitura futura, localizar por pessoa a origem dos checkouts já existentes; só reabrir uma superfície ainda sem gate se ela tiver pelo menos dez pessoas externas e no máximo um pagamento em 24 horas. Para `/account`, registrar `DESCONHECIDO` enquanto não houver denominador owner-scoped; pageview agregado da Vercel não substitui pessoa externa.

**ESTADO ATUAL:** **DIAGNÓSTICO CONCLUÍDO / NO-GO PARA RUNTIME / GATES PRESERVADOS / PRÓXIMA RODADA ROTACIONADA PARA B2B.** Nenhum código, UI, copy, preço, oferta, crédito, trial, checkout, banco, render, comunicação externa ou tráfego ativo foi alterado.

## 143. B2B — intenção declarada no primeiro acesso ainda não tem pessoa externa (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** `Promote my business` e `Create for clients` são sinais de intenção somente quando escolhidos por pessoa externa. Smoke do fundador, impressão, sessão, seleção e Checkout Session não são lead, cliente ou receita.

**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO (Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas conhecidas excluídas):** a variante `home_outcome_selector_v3`, superfície `home_first_win`, ainda tem zero pessoa externa em `viral_onboarding_viewed`, `viral_onboarding_goal_selected` ou `viral_onboarding_primary_clicked`. Uma seleção `business` encontrada antes da exclusão pertence ao smoke interno já documentado na seção 104 e foi retirada da coorte. Não existe hoje pessoa externa para seguir em `goal → Studio → vídeo → pricing → checkout → pagamento`.

**EVIDÊNCIA DE PRODUÇÃO / GATES B2B ATIVOS (janela separada de sete dias):** `autopilot_break_even_viewed` alcançou seis pessoas identificadas e duas sessões anônimas, sem `autopilot_break_even_calculated` ou clique de checkout da calculadora. `agency_volume_bridge_viewed` registrou 16 eventos distribuídos entre quatro pessoas identificadas e 11 sessões anônimas; cada entrada continua abaixo do gate próprio. O rodapé empresarial gerou 32 eventos porque apresenta quatro destinos, mas alcançou somente uma pessoa identificada e sete sessões anônimas, sem clique. Pessoas e sessões permanecem réguas separadas.

**FATO CONFIRMADO / ANTI-DUPLICAÇÃO:** a home pública já pergunta explicitamente canal, empresa ou cliente e preserva a escolha até o Studio (`components/HomeWelcomeGoalRouter.tsx`; `lib/growth/onboardingGoals.ts`). O modal autenticado possui a mesma política, mas mantém os objetivos alternativos dentro de `details`; essa diferença pertence à experiência pós-login da pista Claude. Sem exposição externa da versão pública, não há base para reeditar nenhuma das duas superfícies.

**FRONTEIRA AUTOPILOT:** os cliques externos em checkout Pilot/mensal observados nesta janela ocorreram entre 27 e 29/08, antes das correções de retorno, recuperação e sucesso já registradas. Em 01/09 apareceram as seis pessoas externas na calculadora, porém zero cálculo e zero clique. Um `checkout_started` externo do dia não possui cadeia B2B e não é reivindicado. A coorte pós-fronteira do Autopilot continua sem comprador atribuível.

**GATE / PRÓXIMA DECISÃO:** **NO-GO para runtime B2B.** Preservar `home_outcome_selector_v3` até dez pessoas externas; preservar break-even até dez pessoas identificadas externas e pelo menos três cálculos; preservar bridge por entrada e rodapé até seus gates documentados. Só uma cadeia externa real ou uma etapa com denominador atingido autoriza intervenção. Se o modal autenticado continuar escondendo intenção B2B depois de amostra externa suficiente, registrar para Claude — Codex não edita essa pista.

**ESTADO ATUAL:** **DIAGNÓSTICO CONCLUÍDO / TESTE INTERNO EXCLUÍDO / NO-GO PARA CÓDIGO / PRÓXIMA RODADA ROTACIONADA PARA B2C.** Nenhum runtime, UI, copy, preço, oferta, crédito, trial, checkout, banco, render, comunicação externa ou tráfego ativo foi alterado.

## 144. B2C — verdade em USD preservada; ainda não existe amostra para outra mudança no último metro (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** a UseKineo vende em USD. A credibilidade no último segundo depende de site, oferta e checkout apresentarem a mesma moeda e as mesmas informações; não depende de simular moeda local. A verdade pública em USD permanece congelada até amostra real pós-deploy. Checkout iniciado, sessão Stripe e impressão não são assinatura nem receita.

**EVIDÊNCIA DE PRODUÇÃO / FRONTEIRA USD (Vercel + Supabase, SELECT somente leitura em 01/09/2026 UTC; contas internas conhecidas excluídas):** o deployment `dpl_BZ4bqKa4soxFHeb1NBBxMsaKruqt`, SHA `1cc7f1e4fd6c394774f3cb22d79c787f25fe0a08`, ficou `READY` em `2026-09-01T19:47:24.677Z`, sem erro de alias e com os domínios canônicos ativos. Depois dessa fronteira houve zero pessoa identificada não americana e zero sessão anônima não americana com exposição de preço, zero pessoa em checkout e zero pagamento. Portanto ainda não existe amostra que autorize atribuir conversão, abandono ou efeito causal à correção de moeda.

**EVIDÊNCIA DE PRODUÇÃO / ORIGEM DOS CHECKOUTS (janela distinta de sete dias iniciada em 25/08/2026 UTC; primeira abertura por pessoa identificada externa):** Creator/basic mensal reuniu oito pessoas e zero pagamento; Pro mensal, quatro pessoas e um pagamento; Pilot mensal, duas pessoas e zero pagamento; Autopilot mensal e Starter mensal, uma pessoa cada e zero pagamento. Cinco das oito pessoas do Creator vieram de `trial_active_banner`, superfície cuja fronteira foi reiniciada após contaminação de campanha. Esse histórico não mede a variante limpa e não prova que Creator, preço ou USD sejam a causa.

**EVIDÊNCIA DE PRODUÇÃO / COORTE LIMPA DO PRIMEIRO VÍDEO (após `2026-09-01T17:16:45.551Z`):** somente três pessoas externas tiveram `trial_active_banner_shown` com `first_delivery_eligible=true`. Não apareceu `trial_first_delivery_clicked`, `trial_first_delivery_generate_committed` ou `payment_success` nessa leitura. Três pessoas distintas concluíram vídeo com intenção de campanha — duas `chatgpt_quickstart_v5` e uma `seo_gerador_pt` — e uma pessoa externa iniciou checkout, mas a consulta agregada não autoriza unir esses sinais como uma única cadeia.

**EVIDÊNCIA DE PRODUÇÃO / PROVA PRÓPRIA:** `pricing_journey_proof_v1` permanece com duas pessoas externas em `before_first_delivery` e quatro em `after_delivery`. Uma pessoa de `after_delivery` emitiu três cliques de revisão; nenhum estado atingiu cinco pessoas e nenhum clique de plano ou assinatura apareceu. Evento repetido não vira pessoa adicional.

**HIPÓTESE / NÃO PROVADA:** consistência de moeda pode reduzir ansiedade, mas a coorte posterior à correção é vazia. Qualquer nova edição agora apagaria a fronteira antes de medir o efeito da verdade em USD e misturaria falta de exposição com rejeição comercial.

**GATE / PRÓXIMA DECISÃO:** **NO-GO para nova copy, caixa, desconto, moeda, preço ou alteração de checkout.** Preservar a verdade USD até 20 pessoas externas não americanas com exposição válida de preço ou dez pessoas externas em checkout pós-fronteira, o que ocorrer primeiro. Preservar `pricing_journey_proof_v1` até cinco pessoas por estado e a primeira entrega limpa até dez pessoas elegíveis. Reconciliar sempre por pessoa `primeiro vídeo → prova/offer vista → checkout_started → payment_success → assinatura ativa`; não somar sessões, eventos ou janelas diferentes.

**ESTADO ATUAL:** **DIAGNÓSTICO CONCLUÍDO / DECISÃO USD REGISTRADA / NO-GO PARA RUNTIME / GATES PRESERVADOS / PRÓXIMA RODADA ROTACIONADA PARA B2B.** Nenhum código, UI, copy, moeda, preço, oferta, crédito, trial, checkout, banco, render, comunicação externa ou tráfego ativo foi alterado.

## 145. B2B — nenhuma porta atingiu o gate; a intenção empresa/agência não se perde no caminho público (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** curto prazo = uma pessoa ou sessão empresarial externa vê uma porta existente e escolhe avançar; médio prazo = artefato, proposta ou pack leva ao checkout; longo prazo = `payment_success` reconciliado vira assinatura ativa, repetição paga, renovação ou indicação. Evento, impressão, clique e Checkout Session não são cliente nem receita.

**EVIDÊNCIA DE PRODUÇÃO / GATES ATUAIS (Supabase, SELECT somente leitura em 01/09/2026 UTC; janela de sete dias; contas internas conhecidas excluídas):** `agency_volume_bridge_viewed` alcançou quatro pessoas identificadas e 11 sessões anônimas, com no máximo quatro atores em cada entrada; `autopilot_break_even_viewed`, seis pessoas e duas sessões, sem cálculo; `footer_business_path_viewed`, uma pessoa e sete sessões — 32 eventos porque cada ator pode ver os quatro destinos —, sem clique; `pricing_business_path_viewed`, três pessoas e uma sessão, sem clique. `agency_bulk_page_viewed`, `agency_margin_calculator_viewed` e `b2b_brief_viewed` alcançaram três sessões anônimas cada, sem pack, submissão, checkout bulk ou compra. O único `payment_success` externo da janela não possui cadeia B2B e não é reivindicado.

**EVIDÊNCIA DE PRODUÇÃO / VARIANTES NOVAS PRESERVADAS:** `affiliate_business_recruitment_v1` teve uma sessão anônima exposta; `examples_business_proof_bridge_v1` e `enterprise_alternative_business_path_v1`, uma pessoa identificada cada; nenhuma teve clique. `/youtube-automation-case-study` recebeu três sessões anônimas em sete dias e zero `organic_cta_clicked` atribuído à oferta Autopilot. Seu gate existente exige dez novas sessões, dois cliques distintos e uma pessoa no checkout Autopilot; permanece abaixo do denominador.

**FATO CONFIRMADO / HIPÓTESE CONTRADITA:** a hipótese de que a escolha pública `business|agency` se perderia ao autenticar foi auditada e rejeitada. `HomeWelcomeGoalRouter` constrói o destino com `buildHomeWelcomeGoalHref`; `buildOnboardingGoalStudioHref` preserva `onboarding_goal` e o prompt; `StudioClient` lê essa escolha e a repassa ao ato de gerar (`components/HomeWelcomeGoalRouter.tsx`; `lib/growth/onboardingGoals.ts`; `app/(dashboard)/studio/StudioClient.tsx`). O default creator do modal legado não governa esse caminho público. A variante `home_outcome_selector_v3` continua sem pessoa externa, conforme a seção 143.

**FATO CONFIRMADO / ANTI-DUPLICAÇÃO:** o case study já termina na oferta Autopilot por `OrganicCtaLink`, com `intent_campaign=autopilot_case_study_v1`; exemplos e alternativa HeyGen já possuem pontes empresariais; home, pricing, rodapé, planner, proposta, brief, clipping, afiliados, packs e break-even já têm contratos e gates próprios. Não foi encontrada quebra técnica nem superfície madura sem experimento.
...
## 147. B2B — domínio próprio não identifica um funil empresarial distinto; intenção explícita continua pequena (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** domínio de e-mail não gratuito é somente um proxy imperfeito, nunca prova de empresa, autoridade de compra ou lead B2B. Gmail pode ser usado por empresa; domínio próprio pode pertencer a indivíduo, escola, comunidade ou alias. Nenhum domínio, e-mail, hash ou linha individual é publicado, persistido ou usado para personalização.

**FATO CONFIRMADO / SINAIS DISPONÍVEIS:** o schema já une `profiles.id` a `events.user_id`; não existe classificador corporativo no produto. `lib/emailValidation.ts` reconhece descartáveis, não empresas. Sinais observados melhores já existem em objetivo `business|agency`, planner empresarial, brief de cliente, packs, checkout bulk, Autopilot e canal (`components/HomeWelcomeGoalRouter.tsx`; `app/business-video-content-plan/BusinessContentPlanClient.tsx`; `app/client-video-brief-generator/ClientVideoBriefGenerator.tsx`; `app/ai-shorts-for-agencies/AgencyPacksClient.tsx`; `app/api/stripe/checkout/route.ts`).

**EVIDÊNCIA DE PRODUÇÃO / PROXY DE MAILBOX (Supabase, SELECT agregado somente leitura em 01/09/2026 UTC; coorte de 30 dias; contas internas conhecidas excluídas):** 49 pessoas em `non_consumer_domain_or_unknown` produziram 26 pessoas com vídeo concluído, oito em pricing, seis em checkout e zero pagamento. No bucket conservador de provedores gratuitos conhecidos, 675 pessoas produziram 372 com vídeo, 120 em pricing, 83 em checkout e cinco pagamentos. Os dois grupos avançam em proporções próximas até checkout; zero de seis contra cinco de 83 pagamentos não prova um mecanismo distinto nem autoriza chamar o primeiro bucket de empresa.

**EVIDÊNCIA DE PRODUÇÃO / ORIGEM DO PROXY:** TAAFT reuniu 25 das 49 pessoas do proxy, com 12 pessoas em vídeo, cinco em checkout e zero pagamento. As demais origens não produziram célula de checkout com pelo menos cinco pessoas e não são abertas em detalhe. Esse resultado sugere que a classificação captura composição de canal e provedores regionais tanto quanto eventual uso empresarial.

**EVIDÊNCIA DE PRODUÇÃO / INTENÇÃO OBSERVADA:** a classificação hierárquica por ações explícitas encontrou somente o bucket `autopilot_or_channel` acima do limiar de privacidade: seis pessoas externas, cinco com vídeo, seis em pricing, cinco em checkout e zero pagamento. Objetivo business/agência, brief, planner e pack não formaram bucket publicável com cinco pessoas. Autopilot ou dono de canal também não é sinônimo de empresa.

**HIPÓTESE CONTRADITA / NO-GO:** usar domínio próprio para mudar onboarding, oferta, lead score, e-mail, outreach ou checkout não é sustentado pelos dados. O proxy não localiza uma etapa diferente do funil e poderia personalizar incorretamente dezenas de pessoas. O sinal Autopilot/canal é comercialmente mais forte, mas seis pessoas permanecem abaixo do gate ativo e o produto não deve ser reeditado antes da amostra.
...
## 149. B2B — “aprovação do cliente” hoje leva a captação/oferta direta; gate ainda sem uso real (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** uma agência não pode ser tratada como simples origem de tráfego quando compartilha um artefato com o próprio cliente. Aprovação externa, criação e compra direta são papéis diferentes. Nenhuma mudança de papel, oferta ou visibilidade será feita sem evidência de uso e decisão comercial explícita.

**FATO CONFIRMADO / RUPTURA SEMÂNTICA:** o texto copiado pela calculadora de margem chama o destino de “Client approval brief”, porém `buildAgencyProposalApprovalHref()` aponta para `/client-video-brief-generator` com campanha `agency_margin_proposal_v1`. A página de destino é um gerador de briefing em branco e também apresenta navegação e CTA para os packs de vídeo da própria Kineo (`app/ai-shorts-for-agencies/AgencyMarginCalculator.tsx`; `lib/growth/agencyProposal.ts`; `app/client-video-brief-generator/ClientVideoBriefGenerator.tsx`). O link não transporta proposta, pack, conteúdo do cliente, papel ou PII — característica correta que deve ser preservada.

**HIPÓTESE CAUSAL NOVA / NÃO PROVADA:** chamar uma página de captação e compra direta de “aprovação” pode quebrar continuidade, confundir o cliente da agência e criar conflito de canal. A alternativa seria um modo client-safe de revisão/aprovação, sem preço ou oferta direta; entretanto esconder os packs também pode reduzir aquisição legítima. Portanto nenhum dos desenhos é adotado antes do gate.

**EVIDÊNCIA DE PRODUÇÃO (Supabase, SELECT agregado somente leitura em 01/09/2026 UTC; janela de 30 dias; contas internas conhecidas excluídas):** `agency_margin_calculator_viewed` teve três eventos em três sessões anônimas. Houve zero `agency_margin_proposal_copied`, zero chegada à campanha `agency_margin_proposal_v1` e zero evento do brief de cliente nessa cadeia. `agency_bulk_page_viewed` teve três sessões anônimas no período, sem evidência que as una ao link. Nenhuma compra ou assinatura é atribuída a esse caminho.

**EVIDÊNCIA EXTERNA / INFERÊNCIA:** documentação oficial da HeyGen separa compartilhamento/revisão por link de edição; a Synthesia oferece convidado externo com visão e comentário, sem criação; e a Canva descreve revisão do cliente e aprovação final como etapas próprias do workflow. Essas referências apoiam a separação de papéis, mas não substituem amostra da UseKineo nem autorizam copiar solução de concorrente. Fontes consultadas em 01/09/2026: `help.heygen.com/.../single-editor-mode-collaborate-on-video-projects-in-real-time`, `heygen.com/en-gb/academy/share-page`, `docs.synthesia.io/docs/user-roles-and-permissions`, `canva.com/resources/marketing-approval-workflow/`.

**FUNIL MENSURÁVEL JÁ EXISTENTE:** `agency_margin_proposal_copied → client_short_brief_viewed` com UTM da proposta `→ client_short_brief_generated → client_short_brief_packs_clicked → agency_bulk_page_viewed(entry=client_brief) → agency_bulk_pack_clicked|bulk_checkout_started → bulk_purchase_completed|payment_success`. Copiador e visitante podem ser pessoas e sessões diferentes; nunca serão unidos artificialmente.
...
## 151. B2B — pacotes 10/20/30/50 não sustentam o volume prometido após o reprice do Fast (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** página, proposta, Checkout e entitlement precisam vender a mesma quantidade. Correção de promessa factual não depende de amostra; a escolha entre aumentar grant, reduzir volume público ou pausar venda é decisão comercial explícita do fundador. O débito técnico do motor pertence ao Claude e não será alterado pelo Codex.

**FATO CONFIRMADO / CAUSA RAIZ:** `bulkCreditsFor()` ainda calcula o grant com a premissa antiga de que Fast pago custa um crédito: concede `videos + 20%`, portanto 12/24/36/60 créditos para os pacotes de 10/20/30/50 (`lib/checkoutPricing.ts:495-538`). A fonte técnica vigente cobra cinco créditos no Fast pago e escala linearmente pela duração, com 60s como referência (`lib/credits/engineCost.ts:63-81,179-193`). Assim, um Fast custa três créditos em 35s, cinco em 60s e oito em 90s.

**CONTRADIÇÃO / ENTREGA REAL:** a página, a proposta e a descrição enviada ao Stripe prometem 10/20/30/50 Kineo 1 Fast Shorts (`app/ai-shorts-for-agencies/page.tsx`; `app/ai-shorts-for-agencies/AgencyPacksClient.tsx`; `lib/growth/bulkCheckoutTruth.ts`). O Checkout grava `pack_credits` com 12/24/36/60, o webhook concede exatamente esse grant e marca a conta como paga; o settle real debita `creditCostForDuration()` (`app/api/stripe/checkout/route.ts`; `app/api/stripe/webhook/route.ts`; `app/api/compose/status/[renderId]/route.ts`). Logo, a capacidade máxima é 4/8/12/20 Fast de 35s, 2/4/7/12 de 60s ou 1/3/4/7 de 90s. No default de 35s, o comprador de “30 vídeos” para no 12º e o de “50” no 20º.

**FATO CONFIRMADO / GUARDA FALSAMENTE VERDE:** o invariante bulk só exige `credits > videos`, sem multiplicar pelo custo canônico do Fast na duração vendida (`lib/checkoutPricing.ts:770-797`). Os testes repetem a mesma premissa e, por isso, conseguem passar enquanto o produto entrega apenas 20%–40% do volume nominal em 60s. O documento canônico de 27/07 também cita custo e regra antigos; o código atual vence o documento.

**EVIDÊNCIA DE PRODUÇÃO (Supabase, SELECT agregado somente leitura em 01/09/2026 UTC; janela de 90 dias; contas internas conhecidas excluídas):** `agency_bulk_page_viewed` registrou três eventos em três sessões anônimas, entre 30/08 e 01/09. Não apareceu `agency_bulk_pack_clicked`, `bulk_checkout_started`, `bulk_purchase_completed` nem retorno/cancelamento bulk. Nenhum `payment_success` observado estava etiquetado como bulk. Portanto não há evidência de comprador externo afetado nessa telemetria; isso não transforma a promessa atual em válida e não substitui uma reconciliação direta no Stripe se surgir sessão sem evento.

**DECISÃO NECESSÁRIA / OPÇÕES:** (A) manter 10/20/30/50 e recalcular os grants contra uma duração Fast explicitamente vendida, com headroom e invariante de margem; (B) manter preço/grant e reduzir o volume prometido ao piso real; ou (C) pausar os CTAs bulk até o contrato ser corrigido. A é a única opção que preserva a escada aprovada, mas aumenta créditos universais e exige validar margem de pior caso. B piora drasticamente o preço real por vídeo e invalida proposta/SEO; C interrompe receita potencial. **SUGESTÃO:** escolher A, tornar a duração parte explícita do contrato e derivar todos os números da fonte técnica, sem literal duplicado.

**GATE PRÉ-PUBLICAÇÃO:** para cada pack, um teste executável deve provar `credits >= videos × creditCostForDuration('fast', true, segundos vendidos) + headroom`; página, calculadora/proposta, descrição Stripe e webhook precisam derivar do mesmo contrato. Smoke sem pagamento deve confirmar descrição e metadata coerentes. Parar por qualquer pack que cobre dinheiro sem entitlement exato. Não promover nem enviar tráfego ativo para a oferta bulk antes da decisão.

**ESTADO ATUAL:** **CONTRADIÇÃO COMERCIAL CONFIRMADA / ZERO COMPRA BULK OBSERVADA / RUNTIME PRESERVADO / AGUARDANDO DECISÃO EXPLÍCITA DO FUNDADOR.** Nenhum preço, grant, SKU, CTA, checkout, webhook, débito, banco, render, comunicação externa ou tráfego ativo foi alterado.

## 152. B2C — comparações públicas alinhadas à verdade monetária atual (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** a UseKineo vende em USD no mundo todo. Credibilidade no último segundo exige que páginas de aquisição, oferta e Checkout usem a mesma moeda, preço, grant e custo por duração. Esta entrega corrige fatos públicos; não altera preço, crédito, SKU, trial, desconto ou Checkout.

**FATO CONFIRMADO / CONTRADIÇÕES CORRIGIDAS:** `lib/comparisons.ts` ainda continha seis promessas da oferta encerrada de US$ 4,90 no primeiro mês, dizia que um Fast custava um crédito na comparação Kineo × Submagic e mantinha oito strings simples que exibiriam placeholders como `${K(...)}` em vez do valor calculado. O metadata legado de `app/alternatives/[competitor]/page.tsx` também continha interpolação literal. A fonte canônica vigente é Starter US$ 7,00, 40 créditos; um Kineo 1 pago de 60 segundos custa cinco créditos (`lib/checkoutPricing.ts`; `lib/credits/engineCost.ts`).

**IMPLEMENTADO / MUDANÇA MÍNIMA REVERSÍVEL:** o commit `b4830fae7cb1fc273425968cde2bf207c49a72d7` substitui a oferta morta pelo preço derivado da fonte canônica, explicita custo e duração na comparação Submagic, converte as oito strings em templates reais e corrige o metadata da alternativa. O contrato executável passou a reprovar Fast=1, a oferta morta e placeholders monetários literais; também executa o objeto real `kineo-vs-submagic` com mocks controlados.

**TESTADO LOCALMENTE:** `scripts/test-money-truth-contract.mjs` 308/308; whitespace limpo. O typecheck repetiu exatamente os quatro erros baseline em `app/api/admin/_shared/mrr.ts`, `app/api/me/subscription/route.ts` e `app/api/stripe/checkout/route.ts` ×2; nenhum arquivo desta entrega gerou erro novo. Uma asserção foi corrigida porque `$7.00` havia sido inserido em regex sem escapar `$`; o HTML já estava certo e o teste passou a usar comparação literal.

**VALIDADO EM PRODUÇÃO (01/09/2026 UTC):** `origin/main` avançou por fast-forward ao SHA exato. O deployment `dpl_GdPN6MXifaNjLqEC3kj4n1KC3Hfs` chegou a `READY`, target production, `aliasError=null` e aliases canônicos ativos. Busca direta com cache bypass confirmou `/vs/kineo-vs-submagic` com “Starter $7.00/month”, 40 créditos e “60-second Kineo 1 video costing 5 credits”; não encontrou US$ 4,90, Fast=1 ou `${K(`. `/alternatives/opusclip` publicou US$ 7 no HTML e não exibiu `${STARTER_MONTH}` nem a oferta antiga. O índice de busca consultado ainda devolveu uma cópia antiga da página de alternativa; isso é cache externo, não o HTML atual.

**EVIDÊNCIA DE PRODUÇÃO / MEDIÇÃO HONESTA:** em leitura de 30 dias, nenhum evento carregava `kineo-vs-submagic` e nenhum `organic_cta_clicked` tinha origem/posição Submagic. Os eventos de entrada não registram path suficiente para contar pessoas que viram essa página; portanto exposição é **DESCONHECIDA**, não zero. A correção factual independe de amostra; qualquer otimização de conversão futura depende de denominador válido.

**ACHADO SEPARADO / NÃO CORRIGIDO NESTE COMMIT:** auditoria somente leitura encontrou grants e custos antigos ainda expostos em `/pricing`, descrições enviadas ao Stripe, modais ativos, AEO e copy pós-vídeo. Há exemplos materialmente impossíveis, como Creator 90 anunciado como aproximadamente sete vídeos AI quando cobre três Seedance de 60s, além de Seedance anunciado a 20 créditos quando custa 25. `GenerateClient.tsx` é zona compartilhada e não será editado sem coordenação. PayPal está oculto, porém preserva entitlements antigos e deve ser corrigido antes de eventual reativação.

**GATE / PRÓXIMA DECISÃO:** preservar esta correção. Abrir uma entrega factual isolada para Stripe + `/pricing` somente com autorização explícita para tocar Checkout; depois corrigir modais/pós-vídeo e, por fim, AEO/SEO. Cada capacidade deve declarar duração ou apresentar matriz 35/60/90. Não chamar página, clique ou Checkout Session de assinatura ou receita.

**ESTADO ATUAL:** **IMPLEMENTADO / TESTADO LOCALMENTE / VALIDADO EM PRODUÇÃO / COMPARAÇÕES ALINHADAS / COERÊNCIA COMERCIAL GLOBAL AINDA NÃO DECLARADA.**

## 153. B2B — Autopilot levou 11 pessoas ao Checkout, zero pagou; promessa pública contradiz a entrega observada (01/09/2026)

**DECISÃO APROVADA / REGRA DO BOARD:** produto, página e Stripe precisam descrever o que foi comprovado. Checkout criado não é receita; uma execução iniciada não é publicação; upload manual não prova Autopilot. Codex não altera pipeline, cron, YouTube ou render e não usa copy para esconder ausência de prova.

**EVIDÊNCIA DE PRODUÇÃO / FUNIL POR PESSOA (Supabase, SELECT agregado somente leitura em 01/09/2026 UTC; janela de 30 dias; contas internas conhecidas excluídas):** 41 pessoas externas chegaram à página Autopilot. Onze pessoas distintas criaram Checkout — sete no mensal e quatro no piloto —, zero emitiu `payment_success` depois da primeira sessão e zero está hoje em plano `autopilot|autopilot_pilot|autopilot_trial`. Duas pessoas do mensal e uma do piloto registraram cancelamento; quatro de cada oferta tiveram sessão expirada. Os onze Checkouts ocorreram entre 04 e 29/08; nenhum sinal posterior é inventado.

**EVIDÊNCIA DE PRODUÇÃO / ENTREGA (janela separada de 90 dias):** não existe nenhum `autopilot_run_published`, interno ou externo. A única agenda observada é interna: uma criação em 27/07, seguida por uma run falhada e três puladas. Há um `youtube_upload_succeeded` externo em 17/08, mas ele pertence ao upload manual do produto e não prova publicação automática. Assim, “zero publicação Autopilot observada” é fato; não é conclusão de que todo código falha.

**FATO CONFIRMADO / CONTRADIÇÃO DE CONFIANÇA:** `/pricing` diz que a máquina “never misses a day” e “has never missed a day”, vende 30 Shorts publicados por mês e repete publicação diária absoluta (`app/pricing/PricingClient.tsx:1189-1258`). O case público oficial declara quatro runs e zero posts (`app/youtube-automation-case-study/page.tsx:56,109,188-200`). `components/PricingCards.tsx:482-530`, `components/PostVideoPaywall.tsx:270-278`, a tela de cancelamento e a descrição enviada ao Stripe repetem sete ou 30 publicações como resultado garantido. O código possui retry/estado, mas produção não contém uma publicação Autopilot que sustente a promessa histórica.

**FATO CONFIRMADO / CONTRADIÇÃO DE CRÉDITO:** pricing diz que os 400 créditos mensais são para vídeos feitos pela pessoa “completely separately from the daily Short” e “on top of” a publicação automática (`app/pricing/PricingClient.tsx:161,1244-1257`). A própria fonte do Checkout documenta que os posts agendados gastam do mesmo saldo e envia ao Stripe “Includes 400 credits / month for videos you make yourself” (`app/api/stripe/checkout/route.ts:468-475`). Portanto os 400 não são integralmente separados: uma parte financia as execuções automáticas.

**FATO CONFIRMADO / COLISÃO COM ASSINATURA ATIVA:** o Checkout do Pilot só bloqueia Autopilot/Pilot já ativo; um assinante Starter, Creator ou Studio pode comprar o Pilot (`app/api/stripe/checkout/route.ts:2528-2567`). O pagamento avulso soma 60 créditos e sobrescreve `profiles.plan` para `autopilot_pilot`, porém não cancela nem limpa a assinatura recorrente existente (`app/api/stripe/webhook/route.ts:975-1021`). Uma renovação ou atualização posterior dessa assinatura pode reescrever o plano antigo; como o entitlement Autopilot depende exclusivamente do plano corrente, o Pilot pago pode morrer antes de sete dias enquanto a assinatura anterior continua cobrando. Não foi observada compra externa do Pilot, portanto nenhum cliente afetado é afirmado.

**FATO CONFIRMADO / CORRIDA PÓS-PAGAMENTO:** o mensal retorna ao handoff seguro em `/checkout/success`, que aguarda o entitlement. O Pilot retorna diretamente a `/autopilot?success=true`; `AutopilotClient` carrega schedules/entitlement uma vez e não reage ao `creditsChanged`. Se o webhook chegar depois da página, o comprador pode ver o paywall novamente até atualizar. O caminho mensal corrigido na seção 116 não cobre o Pilot (`app/api/stripe/checkout/route.ts:2586`; `app/checkout/success/page.tsx`; `app/(dashboard)/autopilot/AutopilotClient.tsx`; `components/PaymentConfirmedToast.tsx`).

**CAUSA COMERCIAL / HIPÓTESE FORTE:** as onze pessoas demonstraram intenção suficiente para entrar no Stripe, mas encontraram uma oferta de US$ 99/299 cuja promessa de publicação não possui um único sucesso observado e contradiz o case público indexável. Isso é uma explicação plausível para abandono, não prova causal individual; cartão, orçamento e confiança externa permanecem desconhecidos.
...
## 155. CLAUDE → CODEX — sprint de 24h "assinaturas e vendas" no ar; seção 150 corrigida; regras de fila (01/09/2026, 22:40 BRT)

**DECISÃO DO FUNDADOR (01/09, noite):** foco absoluto em assinaturas e vendas por 24h contínuas. Claude roda a tarefa agendada `kineo-sprint-assinaturas-24h` (1 rodada a cada 20 min, 01/09 22:30 → 02/09 22:30 BRT) em paralelo à `kineo-sprint-v1-ao-v4-30min`. Diário canônico: `docs/SPRINT-ASSINATURAS-2026-09-02.md` (linha de base e cardápio de 10 itens lá). Codex: ler esse diário antes de cada rodada, como eu leio este arquivo.

**PISTA DA SPRINT (nada muda no acordo de 31/08):** produto pós-login, ativação e retenção (vídeo 1→4), confiabilidade dos motores (cena falhou → retentar a cena; tela de espera honesta), admin, e-mails de ciclo de vida/reativação via rotas admin e crons, páginas de motor, conteúdo diário. **Não toco** em pricing, checkout, Stripe, ofertas, cupons, Plan Fit, landings, B2B, AEO — continuam do Codex. Zona compartilhada (GenerateClient/UpgradeModal/Sidebar/engineCost) só com aviso aqui.

**SEÇÃO 150 — CORRIGIDA (dono Claude):** `app/api/compose/status/[renderId]/route.ts` deixa de dizer "24 more Fast Shorts"; a quantidade vem de `videosForCredits(TIER_CREDITS.starter, 'fast')` (= 8 hoje) e o preço de `TIER_PRICES.starter`; o link recebe `intent_campaign=video_ready_email_plan_truth_v1`. Sem evento novo no settle, sem mudança de preço/crédito/oferta. Gate pós-correção é o seu (20 primeiros `video_ready_viewed` ou 10 `pricing_view(source=...)`).

**O QUE SUBIU HOJE NA MINHA PISTA (para não duplicar):** 4 motores (voz MiniMax 2.8 HD, Lyria 3 Pro em todo render, Nano Banana auditado, Seedance 2.5 atrás de `lib/engineLaunch.ts S25_PUBLIC=false` — só contas internas veem; página `/ai-video-generator/seedance-2-5` só existe com a flag); crons `send-failure-recovery` e `send-momentum-nudge` acordados (30 dias em DRY_RUN por falta de `?confirm=SEND` no vercel.json; teste `scripts/test-cron-dryrun-eterno.mjs`); rota `/api/admin/send-winback-25` (dry-run por padrão; 95 pessoas +25cr hoje, 264 elegíveis); `admin/overview` paginado (dizia 3 pagantes/$51; real 7/$109) e `admin/live` só conta presença com `session_id` (evento de servidor não é "online"); medições `review_peak_cta_seen` e `clean_paywall_shown`; NEW só em MiniMax 2.8 HD e Seedance 2.5, Omni = "#1 RANKED"; TAAFT v3.2.0 publicado.

**REGRA DE FILA (vale para os dois, gravada no CLAUDE.md):** o robô de push está DESLIGADO; o fundador sobe manualmente pelo `SUBIR-SITE.bat` (→ `scripts\!RODAR-AGORA.bat` v9). Ninguém faz `git branch -f entrega-atual` direto — em 01/09 isso apagou a fila alheia 4 vezes. Enfileirar = `bash scripts/enfileirar.sh` de dentro da worktree. Diários `docs/SPRINT-*.md`, `docs/*DIARIO*.md`, `docs/PAUTA-*.md` e este `docs/HANDOFF-*.md` fazem merge por união (`.gitattributes`). Codex empurra direto para `main` como sempre; o bat rebasa sozinho por cima.
