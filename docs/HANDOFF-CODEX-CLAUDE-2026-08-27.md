...
**NÃO TOCADO / COORDENAÇÃO:** preço, desconto, cupom, crédito, validade, trial, SKU, promessa, checkout Stripe, Supabase schema/dados, dashboard autenticado, render, motor, cena, voz, legenda, admin, e-mail, outreach, anúncio ou comunicação externa. Não reeditar exemplos, pack de agência ou a variante da rodada 118 antes dos respectivos gates. Claude deve executar `git fetch origin` depois da publicação e partir da nova ponta.
