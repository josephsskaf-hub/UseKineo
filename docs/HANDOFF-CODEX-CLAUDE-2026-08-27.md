...
**EVIDÊNCIA DE PRODUÇÃO / RECONCILIAÇÃO (Supabase, SELECT somente leitura em 01/09/2026 UTC, janela de sete dias; contas internas conhecidas excluídas):** 96 sessões externas distintas entraram por caminhos `/v/*`. Destas, 54 sessões realmente expuseram pelo menos metade de um CTA medido por `public_video_cta_viewed`. Nenhuma das 96 sessões emitiu `public_video_cta_clicked`, `public_video_remix_arrived`, `public_video_remix_script_generated`, `public_video_remix_signup_clicked`, `video_generation_completed`, `pricing_page_viewed`, `checkout_started` ou `payment_success` depois da entrada dentro da janela. Os números são sessões, não pessoas identificadas, e não são somados a outras coortes.
...
**HIPÓTESE CAUSAL / NÃO PROVADA:** zero clique depois de 54 visões válidas mostra rejeição da continuação atual nessa amostra, mas não explica se a causa é mensagem, valor do remix ou intenção de apenas assistir. Acrescentar agora um CTA “for agencies” dividiria a atenção, contaminaria o experimento B2C existente e chamaria audiência de intenção empresarial sem evidência.

**GATE / PRÓXIMA DECISÃO:** **NO-GO para CTA, copy, evento ou landing B2B nova nessas páginas.** Preservar o experimento público existente; qualquer mudança futura deve primeiro ser formulada como hipótese B2C sobre o CTA único e comparada contra as 54 visões válidas, nunca como inferência B2B. Para `product_tool`, preservar o gate da seção 139 e só reabrir com volume externo suficiente para localizar abandono em `entrada → artefato → CTA creator|B2B → agency_bulk_page_viewed(entry=product_tool) → pack|checkout → pagamento`.
