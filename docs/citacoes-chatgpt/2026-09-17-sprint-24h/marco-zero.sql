...
from profiles p where p.created_at>='2026-09-04T03:00:00Z' and p.created_at<'2026-09-18T01:00:00Z'
), days as (select generate_series('2026-09-04'::date,'2026-09-17'::date,'1 day')::date as day_brt)
select current_timestamp as observed_at,'2026-09-18T01:00:00Z'::timestamptz as cutoff_exclusive, d.day_brt,
...
count(p.id) filter(where p.external and p.resolved_source='chatgpt' and exists(select 1 from videos v where v.user_id=p.id and v.status='completed' and v.created_at<p.created_at+interval '12 hours' and v.created_at<'2026-09-18T01:00:00Z')) as resolved_completed_within_12h,
count(p.id) filter(where p.external and p.resolved_source='chatgpt' and exists(select 1 from events e where e.user_id=p.id and e.name='checkout_started' and e.created_at>=p.created_at and e.created_at<'2026-09-18T01:00:00Z')) as resolved_checkout_people_to_cutoff,
count(p.id) filter(where p.external and p.resolved_source='chatgpt' and exists(select 1 from events e where e.user_id=p.id and e.name='payment_success' and e.created_at>=p.created_at and e.created_at<'2026-09-18T01:00:00Z')) as resolved_payment_event_people_unreconciled,
