...
## 05/08 (13h) — O PLAYBOOK DE QUEDA NÃO PODE CONHECER O FORNECEDOR ✅ EXECUTADA

Depois do blackout de 31/07 a empresa construiu detecção + recuperação + fallback e amarrou os
**três** ao sintoma daquele dia (`insufficient_quota`). Em 05/08 o MESMO fornecedor caiu de outro
jeito — não respondeu — e a máquina inteira ficou muda: sem alarme, sem 503 honesto, sem win-back.
Um cadastro novo vindo do TAAFT tentou 4× em 20 min e falhou nas 4, no primeiro dia dele.
Correção: `BLACKOUT_MARKER_REASONS` como fonte única de "caiu", lida tanto pela detecção quanto
pelo cron de recuperação (que procurava só `openai_quota_dead` e teria devolvido
