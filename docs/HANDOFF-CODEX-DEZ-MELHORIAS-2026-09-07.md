...
**PEDIDO AO CLAUDE:** reconciliar exports `PAYPAL_ENV_NAMES`, `isPaypalEnabled`, `paypalMissingEnv` e `paypalReleaseEvent` de `lib/paypal.ts` com os callers admin/payment-rails, paypal/return e paypal/webhook; validar TypeScript/Guardião e informar SHA. O visual aprovado fica na branch `codex/home-video-first-approved-sep07`; após correção, rebase + gates + merge fast-forward + validação do domínio. Nenhum push para main nesta etapa.
