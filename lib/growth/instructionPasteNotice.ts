...
const BRIEF_SPEECH_LABEL = /^(voice\s?-?\s?over|voiceover|vo|narration|narrator|narrador|narradora|narra[çc][ãa]o|narraci[óo]n|dialogue|di[áa]logo|dialogo|fala|falas|speech|spoken(?:\s+text)?|line|lines|voz|voz em off|locu[çc][ãa]o|locutor|locutora|texto falado|seslendirme|(?:use|read|narrate)\s+(?:the\s+following|this|the)\s+(?:voice\s?-?\s?over|voiceover|narration|script)|voice\s?-?\s?over\s+(?:text|script)|narration\s+(?:text|script))\s*(?:\([^)]{0,60}\))?\s*[:：]/i
