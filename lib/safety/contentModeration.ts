...
// notas, conta e o texto truncado — é o rastro de que o fundador precisa se tiver de agir sobre uma conta. Nenhum arquivo
// bloqueado é apagado aqui (decisão de quarentena é do fundador).

import { openai } from '@/lib/openai'
import { writeServerEvent } from '@/lib/serverEvents'
import { decideModeration, type ModerationDecision } from './moderationPolicy'

export type ModerationSurface = 'images' | 'animate' | 'footage' | 'ads_brief'
...
async function callModeration(input: ModerationInputPart[]) {
  const res = await openai.moderations.create(
    { model: 'omni-moderation-latest', input },
    { timeout: MODERATION_TIMEOUT_MS, maxRetries: 1 },
  )
  const result = res.results?.[0]
  if (!result) throw new Error('moderation returned no result')
  return result as unknown as { flagged?: boolean; categories?: Record<string, boolean>; category_scores?: Record<string, number> }
}
...
export async function moderateContent(args: {
  ...
  try {
    for (const batch of batches) {
      ...
      const decision = decideModeration(result, hasText ? text : null)
      if (decision.block && (!worst || (decision.minors && !worst.minors))) worst = decision
      if (worst?.minors) break
    }
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error)
    console.error(`[moderation] unavailable surface=${args.surface} stage=${args.stage}:`, message)
    return { ok: false, reason: 'unavailable', error: message.slice(0, 200) }
  }
