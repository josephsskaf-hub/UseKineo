...
const PLAN_INCLUDES: Record<CheckoutTier, string[]> = {
  ...
  basic: [
    ...
    `Around ${Math.floor(TIER_CREDITS.basic / 20)} Seedance 1.5 videos a month, or one Avatar with lip-sync (70 credits)`,
  ...
  pro: [
    ...
    `Cinematic Kling 2.5 at 1080p — about ${Math.floor(TIER_CREDITS.pro / 50)} premium videos per month`,
    `Or roughly ${Math.floor(TIER_CREDITS.pro / 20)} Seedance videos per month with the same credits`,
...
export const ENGINE_FACTS: EngineFact[] = [
  ...
  },
  {
    ...
    name: 'Veo 3.1',
    credits: creditCostFor('cinematic_veo'),
