...
import MyVideosClient from '@/app/(dashboard)/history/HistoryClient'
import { CREATOR_OFFER_PROFILE_COLUMNS, isPostFilmCreatorEligible } from '@/lib/growth/postFilmCreatorOffer'

// Both routes use the same authenticated read and the same functional video cards.

export default async function VideoCollection({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  return <MyVideosClient embedded={embedded} videos={videos ?? []} snapshotTime={Date.now()} loadError={Boolean(loadError)} creatorTrialEligible={creatorTrialEligible} />
}
